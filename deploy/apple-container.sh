...
MARIADB_CONTAINER="sub2api-apple-mariadb"
REDIS_CONTAINER="sub2api-apple-redis"
APP_VOLUME="sub2api-apple-data"
MARIADB_VOLUME="sub2api-apple-mariadb-data"
...
Commands:
  ...
  logs <service> [-f]   Show logs for app, mariadb, or redis
...
preflight_stack_ownership() {
    ...
    for resource_name in "${APP_CONTAINER}" "${REDIS_CONTAINER}" "${MARIADB_CONTAINER}"; do
    ...
    for resource_name in "${APP_VOLUME}" "${REDIS_VOLUME}" "${MARIADB_VOLUME}"; do
...
cmd_init() {
    local env_dir temp_file mariadb_secret jwt_secret totp_secret
    ...
    mariadb_secret="$(generate_secret)" || die "Failed to generate MariaDB password."
    ...
    [[ -n "${mariadb_secret}" && -n "${jwt_secret}" && -n "${totp_secret}" ]] || \
    ...
    chmod 600 "${temp_file}"
    replace_env_value MARIADB_PASSWORD "${mariadb_secret}" "${temp_file}"
...
prepare_environment() {
    ...
    MARIADB_IMAGE="$(read_env_value APPLE_CONTAINER_MARIADB_IMAGE mariadb:10.11.14)"
    ...
    MARIADB_USER="$(read_env_value MARIADB_USER sub2api)"
    MARIADB_PASSWORD="$(read_env_value MARIADB_PASSWORD)"
    MARIADB_DATABASE="$(read_env_value MARIADB_DATABASE sub2api)"
    ...
    [[ -n "${MARIADB_USER}" ]] || die "MARIADB_USER must not be empty."
    [[ -n "${MARIADB_DATABASE}" ]] || die "MARIADB_DATABASE must not be empty."
    if [[ -z "${MARIADB_PASSWORD}" || "${MARIADB_PASSWORD}" == "change_this_secure_password" ]]; then
        die "Set a secure MARIADB_PASSWORD in ${ENV_FILE}."
    fi

    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sub2api-apple.XXXXXX")"
    APP_ENV_FILE="${TEMP_DIR}/app.env"
    MARIADB_ENV_FILE="${TEMP_DIR}/mariadb.env"
    MARIADB_PROBE_ENV_FILE="${TEMP_DIR}/mariadb-probe.env"
...
EOF
    ...
    cat >"${MARIADB_PROBE_ENV_FILE}" <<EOF
MARIADB_PASSWORD=${MARIADB_PASSWORD}
EOF
...
EOF
    ...
    chmod 600 "${MARIADB_ENV_FILE}" "${MARIADB_PROBE_ENV_FILE}" "${REDIS_ENV_FILE}"
}

prepare_app_environment() {
    [[ -n "${MARIADB_ADDRESS}" && -n "${REDIS_ADDRESS}" ]] || \
...
DATABASE_HOST=${MARIADB_ADDRESS}
DATABASE_PORT=3306
DATABASE_USER=${MARIADB_USER}
DATABASE_PASSWORD=${MARIADB_PASSWORD}
DATABASE_DBNAME=${MARIADB_DATABASE}
...
probe_mariadb() {
    container exec --env-file "${MARIADB_PROBE_ENV_FILE}" \
        "${MARIADB_CONTAINER}" \
...
start_dependencies() {
    start_container_if_needed "${MARIADB_CONTAINER}"
    if ! wait_for_probe "MariaDB" 90 probe_mariadb; then
        show_failure_logs "${MARIADB_CONTAINER}"
...
cmd_up() {
    ...
    ensure_volume "${MARIADB_VOLUME}"
    ensure_volume "${REDIS_VOLUME}"
    ensure_image_available "${APP_IMAGE}"
    ensure_image_available "${MARIADB_IMAGE}"
    ...
    if [[ "${recreate}" == true ]]; then
        ...
        delete_container_if_present "${MARIADB_CONTAINER}"
    fi

    ensure_container "${MARIADB_CONTAINER}" create_mariadb_container
    ensure_container "${REDIS_CONTAINER}" create_redis_container
    start_dependencies
    MARIADB_ADDRESS="$(container_ipv4_address "${MARIADB_CONTAINER}")"
...
cmd_down() {
    ...
    stop_container_if_running "${MARIADB_CONTAINER}"
...
cmd_status() {
    ...
    print_container_status mariadb "${MARIADB_CONTAINER}"
    print_container_status redis "${REDIS_CONTAINER}"

    if [[ -f "${ENV_FILE}" ]]; then
        prepare_environment
        if container_is_running "${MARIADB_CONTAINER}" && probe_mariadb >/dev/null 2>&1; then
            printf '%-12s %s\n' "mariadb" "healthy"
        else
            printf '%-12s %s\n' "mariadb" "unhealthy"
            failed=1
...
cmd_logs() {
    ...
    case "${service}" in
        ...
        mariadb) container_name="${MARIADB_CONTAINER}" ;;
        redis) container_name="${REDIS_CONTAINER}" ;;
        *) die "Unknown service '${service}'. Use app, mariadb, or redis." ;;
    esac
...
cmd_pull() {
    ...
    info "Pulling ${MARIADB_IMAGE}..."
    container image pull --platform "${PLATFORM}" "${MARIADB_IMAGE}"
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        delete_volume_if_present "${MARIADB_VOLUME}"
