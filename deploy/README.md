...
Apple-silicon Macs running macOS 26 can run the complete Sub2API, MariaDB, and Redis stack with Apple `container` 1.1.0 or newer:
...
- Automatically generates secure secrets (JWT_SECRET, TOTP_ENCRYPTION_KEY, MARIADB_PASSWORD)
- Creates `.env` file with generated secrets
- Creates necessary data directories (data/, mariadb_data/, redis_data/)
- **Displays generated credentials** (MARIADB_PASSWORD, JWT_SECRET, etc.)
...
cp .env.example .env
chmod 600 .env
nano .env  # Set MARIADB_PASSWORD and other required variables
...
|---------|-------------|-----------|----------|
| **docker-compose.local.yml** | Local directories (./data, ./mariadb_data, ./redis_data) | ✅ Easy (tar entire directory) | Production, need frequent backups/migration |
...
### Database Migration Notes (MariaDB)
...
> **Note:** The `docker-deploy.sh` script automatically generates `JWT_SECRET`, `TOTP_ENCRYPTION_KEY`, and `MARIADB_PASSWORD` for you.
...
docker compose -f docker-compose.local.yml exec mariadb mariadb-admin ping
...
ls -la data/ mariadb_data/ redis_data/
```
...
2. **Database connection failed**: Check MariaDB is running and credentials are correct
