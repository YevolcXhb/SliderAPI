...
Sub2API can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published Sub2API, MariaDB, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates .env with random MariaDB, JWT, and TOTP secrets.
...
# Verifies MariaDB, Redis, and the application endpoint.
...
# Also recreate MariaDB and Redis containers, preserving their volumes.
./apple-container.sh up --recreate

# Stop containers while preserving all resources and data.
./apple-container.sh down

# Restart MariaDB, Redis, and Sub2API in dependency order.
...
./apple-container.sh logs mariadb -f
...
APPLE_CONTAINER_MARIADB_IMAGE=mariadb:10.11.14
APPLE_CONTAINER_REDIS_IMAGE=redis:8-alpine
```

The normal `up` command recreates the application container, so application environment changes are applied immediately. Use `up --recreate` when changing MariaDB or Redis container images or Redis runtime configuration. Persistent data remains in named volumes.

`MARIADB_USER`, `MARIADB_PASSWORD`, and `MARIADB_DATABASE` are applied only when MariaDB initializes an empty data volume. Changing them in `.env` and recreating the container does not change an existing database. Rotate a password with `ALTER USER`, and plan explicit migrations for user or database changes. To intentionally initialize a new empty database, first back up the old one and use `destroy --volumes`.
...
| `MARIADB_USER`, `MARIADB_PASSWORD`, `MARIADB_DATABASE` | MariaDB first initialization only |
| `REDIS_PASSWORD` | Applied to Redis and Sub2API |
| `DATABASE_PORT`, `REDIS_PORT` | Internal ports are fixed to 3306 and 6379 |
| `MARIADB_MAX_*`, `REDIS_MAXCLIENTS` | Not currently applied to the database/cache server |
...
| Type | Names |
|---|---|
| Containers | `sub2api-apple`, `sub2api-apple-mariadb`, `sub2api-apple-redis` |
| Network | `sub2api-apple` |
| Volumes | `sub2api-apple-data`, `sub2api-apple-mariadb-data`, `sub2api-apple-redis-data` |

The MariaDB volume is mounted at `/var/lib/mysql`, retaining MariaDB's default data directory. Sub2API and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After MariaDB and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts Sub2API. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.
...
# Logical MariaDB backup.
container exec sub2api-apple sh -c \
  'mariadb-dump -h "$DATABASE_HOST" -u "$DATABASE_USER" -p"$DATABASE_PASSWORD" "$DATABASE_DBNAME"' \
  > backups/sub2api.sql
...
container start sub2api-apple-mariadb
until container exec sub2api-apple-mariadb sh -c 'mariadb -h 127.0.0.1 -u "$MARIADB_USER" -p"$MARIADB_PASSWORD" -e "SELECT 1"'; do sleep 1; done
container copy backups/sub2api.sql sub2api-apple-mariadb:/tmp/sub2api.sql
container exec sub2api-apple-mariadb sh -c '
  mariadb -h 127.0.0.1 -u "$MARIADB_USER" -p"$MARIADB_PASSWORD" -e "DROP DATABASE IF EXISTS \`$MARIADB_DATABASE\`; CREATE DATABASE \`$MARIADB_DATABASE\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
  mariadb -h 127.0.0.1 -u "$MARIADB_USER" -p"$MARIADB_PASSWORD" "$MARIADB_DATABASE" < /tmp/sub2api.sql
  rm /tmp/sub2api.sql
'
