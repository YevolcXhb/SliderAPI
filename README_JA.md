...
**サブスクリプションクオータ分配のための AI API ゲートウェイプラットフォーム**

> MariaDB 10.11 ネイティブに再構築された Sub2API ゲートウェイ－ PostgreSQL 不依存、ikik 機能を完全統合、全 SQL を MariaDB 互換に書き換え。
...
## SliderAPI とは（MariaDB リワーク）

SliderAPI は **MariaDB 10.11 ネイティブ再構築** された Sub2API ゲートウェイです：

- **PostgreSQL 非依存のデータ層** — 全クエリ・マイグレーション・バックグラウンドジョブを MariaDB 10.11 対応に書き換え（SKIP LOCKED / FILTER / ILIKE / AT TIME ZONE / data-modifying CTE などを削除）。
- **ikik 機能の完全統合** — 管理画面・ユーザー画面、ショップとカードキー、出金、カープール、メール配信、各プラットフォーム OAuth、アカウント共有とチャネル監視を接続済み。フロントエンドは Go バイナリに埋め込み。
- **1 コマンドでデプロイ** — Go バイナリに Vue フロントエンドを埋め込み（`-tags embed`）。DB は MariaDB のみで、PostgreSQL 移行ツールは不要。
- **Docker イメージ**: `ghcr.io/yevolcxhb/sliderapi`（GitHub Actions がタグリリース時に自動ビルド）。

> 上流オリジナルは [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)。本リポジトリは上流機能を維持しつつ、データ層を MariaDB ネイティブ SQL に置き換えています。
...
MariaDB と Redis のコンテナを含む Docker Compose でデプロイします。
...
- セキュアな認証情報（JWT_SECRET、TOTP_ENCRYPTION_KEY、MARIADB_ROOT_PASSWORD）を自動生成
...
Apple シリコン搭載 Mac と macOS 26 では、Apple `container` 1.1.0 以降を使用して Sub2API、MariaDB、Redis の完全なスタックを実行できます:
