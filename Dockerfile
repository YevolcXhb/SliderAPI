...
# -----------------------------------------------------------------------------
# Stage 3: MariaDB Client (version-matched with docker-compose)
# -----------------------------------------------------------------------------
...
# mariadb-connector-c 提供 libmariadb（客户端库），mariadb-client 提供 mariadb/mariadb-dump CLI。
...
# Copy mariadb-dump and mariadb (client) from the same MariaDB image used in docker-compose.
# This ensures version consistency between backup tools and the database server.
# alpine 版 mariadb-client 已提供上述二进制；此处从官方 mariadb 镜像补齐以对齐版本。
