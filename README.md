...
<img src="assets/logo.svg" alt="SliderAPI Logo" width="128" />
...
> A MariaDB-10.11 native rework of the Sub2API gateway — PostgreSQL-free data layer, full ikik feature integration, and all SQL rewritten for MariaDB compatibility.
...
SliderAPI is an AI API gateway platform designed to distribute and manage API quotas from AI product subscriptions. Users can access upstream AI services through platform-generated API Keys, while the platform handles authentication, billing, load balancing, and request forwarding.


## Why SliderAPI (MariaDB Rework)

SliderAPI is a **MariaDB-10.11 native rework** of the Sub2API gateway:

- **PostgreSQL-free data layer** — every query, migration and background job was audited and rewritten for MariaDB 10.11 (`SKIP LOCKED` / `FILTER` / `ILIKE` / `AT TIME ZONE` / data-modifying CTEs and other PostgreSQL-only constructs removed).
- **Full ikik feature integration** — the ikik-era admin/user panels, store & card-key system, withdrawals, carpools, email broadcast, OAuth platforms, account sharing and channel monitoring are wired and served by the embedded frontend.
- **One command deployment** — the Go binary embeds the Vue frontend (`-tags embed`); MariaDB is the only database, no PostgreSQL migration tooling needed.
- **Docker image**: `ghcr.io/yevolcxhb/sliderapi` (built from this repository by GitHub Actions on tag release).

> The original upstream project is [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api). This repository keeps the upstream feature set while replacing its data layer with MariaDB-native SQL.
...
```bash
curl -sSL https://raw.githubusercontent.com/YevolcXhb/SliderAPI/main/deploy/install.sh | sudo bash
```
...
# Uninstall
curl -sSL https://raw.githubusercontent.com/YevolcXhb/SliderAPI/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
Deploy with Docker Compose, including MariaDB and Redis containers.
...
curl -sSL https://raw.githubusercontent.com/YevolcXhb/SliderAPI/main/deploy/docker-deploy.sh | bash
...
- Generates secure credentials (JWT_SECRET, TOTP_ENCRYPTION_KEY, MARIADB_ROOT_PASSWORD)
...
git clone https://github.com/YevolcXhb/SliderAPI.git
cd sub2api/deploy
...
# Generate MARIADB_ROOT_PASSWORD
openssl rand -hex 32
```
...
docker compose -f docker-compose.local.yml down
rm -rf data/ mariadb_data/ redis_data/
```
...
Apple-silicon Macs running macOS 26 can run the full Sub2API, MariaDB, and Redis stack with Apple `container` 1.1.0 or newer:
...
<a href="https://star-history.dera.page/#YevolcXhb/SliderAPI&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=YevolcXhb/SliderAPI&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=YevolcXhb/SliderAPI&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=YevolcXhb/SliderAPI&type=Date" />
 </picture>
</a>
