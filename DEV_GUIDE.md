...
| **数据库** | MariaDB 16 + Redis |
| **包管理** | 后端: go modules, 前端: **pnpm**（不是 npm） |

## 二、本地环境配置

### MariaDB 16 (Windows 服务)

| 配置项 | 值 |
|--------|-----|
| 端口 | 3306 |
| mariadb 路径 | `C:\Program Files\MariaDB\16\bin\mariadb.exe` |
| 閰嶇疆鏂囦欢 | `C:\Program Files\MariaDB\16\my.ini` |
| 数据库凭据 | user=`sub2api`, password=`sub2api`, dbname=`sub2api` |
| 超级用户 | user=`MariaDB`, password=`MariaDB` |
...
**解决**：将 SQL 写入文件，用 `mariadb -f` 执行：
```bash
# 错误示范（PowerShell 会吃掉 $）
mariadb -e "INSERT INTO users ... VALUES ('$2a$10$...')"

# 正确做法
echo "INSERT INTO users ... VALUES ('\$2a\$10\$...')" > temp.sql
mariadb -u sub2api -h 127.0.0.1 sub2api < temp.sql
```
...
**问题**：`mariadb -f "D:\中文路径\file.sql"` 报错找不到文件。

**解决**：复制到纯英文路径再执行：
```bash
cp "D:\中文路径\file.sql" "C:\temp.sql"
mariadb -u sub2api -h 127.0.0.1 sub2api < "C:\temp.sql"
```
...
**步骤**：
1. 修改 `C:\Program Files\MariaDB\16\my.ini`
   ```
...
3. 无密码登录并重置
   ```bash
   mariadb -u MariaDB -h 127.0.0.1
   ALTER USER sub2api WITH PASSWORD 'sub2api';
   ALTER USER MariaDB WITH PASSWORD 'MariaDB';
   ```
...
### 坑 7：Windows 上 mariadb 连 localhost 的 IPv6 问题

**问题**：mariadb 连 `localhost` 先尝试 IPv6 (::1)，可能报错后再回退 IPv4。
...
```bash
# 连接数据库
mariadb -u sub2api -h 127.0.0.1 sub2api

# 查看所有用户
mariadb -u MariaDB -h 127.0.0.1 -c "\du"

# 查看所有数据库
mariadb -u MariaDB -h 127.0.0.1 -c "\l"

# 执行 SQL 文件
mariadb -u sub2api -h 127.0.0.1 sub2api -f migration.sql
```
