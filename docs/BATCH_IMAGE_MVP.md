...
Sub2API Batch Image MVP provides asynchronous Gemini image batch generation through a unified API surface backed by Redis workers, MariaDB state, and provider-specific batch backends.
...
Redis is used for wakeups, retries, worker coordination, per-job locks, and download limiting. MariaDB remains the source of truth.
...
- No API key exposure.
- No image bytes/base64 in MariaDB.
- No base64 in logs.
