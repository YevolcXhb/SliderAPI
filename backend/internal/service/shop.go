...
WHERE id = ? AND deleted_at IS NULL`, newBalance, rechargeDebit, inviteDebit, shareDebit, userID); err != nil {
	...
	// MariaDB: no data-modifying CTE / UPDATE..RETURNING. Lock the row, compute
	// the per-bucket debit in Go, then update.
...
FOR UPDATE`, userID)
	...
	if err := rows.Scan(&rechargeBalance, &inviteBalance, &shareBalance); err != nil {
		_ = rows.Close()
		return 0, err
	}
...
WHERE id = ? AND deleted_at IS NULL`, amount, rechargeDebit, inviteDebit, shareDebit, userID); err != nil {
	...
	balance, err := queryShopWalletBalanceInTx(ctx, tx, userID)
	if err != nil {
		return 0, err
	}
	return balance, nil
...
func (s *ShopService) AdminDeleteCardKey(ctx context.Context, id int64) error {
	...
	if queryer, ok := sqlTxFromEntTx(tx); ok {
