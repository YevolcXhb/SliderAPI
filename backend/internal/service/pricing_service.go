...
var (
	...
	openAIGPT54FallbackPricing = &LiteLLMModelPricing{
		...
		CacheReadInputTokenCost:         2.5e-07, // ?.25 per MTok
...
type LiteLLMModelPricing struct {
	...
	// 否则 token 流量会被按 ? 计费。零值（false）表示条目具备 token 价格。
...
func (s *PricingService) matchByModelFamily(model string) *LiteLLMModelPricing {
	...
	families := []modelFamily{
		// Opus 5 与 Opus 4.8 同价（?/? per MTok）。定价数据缺失 claude-opus-5 时
		// 必须回退到 4.8，否则会掉进 "opus-4" 系列按 ?/? 计费（3 倍超收）。
