...
func (a *Account) GetOpenAIBaseURL() string {
	...
	// Kiro 没有公有默认 base_url：APIKey 账号必须显式配置，缺失时返回空串，
	// 让调用方给出"未配置 Kiro base URL"而不是误打到 api.openai.com。
	if a.IsKiro() {
...
func (a *Account) SupportsOpenAIEndpointCapability(capability OpenAIEndpointCapability) bool {
	...
	// Kiro 上游只提供 chat completions（无 responses/embeddings/live）。
	if a.IsKiro() {
...
const legacyOpenAIEndpointCapabilitiesCredentialKey = "endpoint_capabilities"
