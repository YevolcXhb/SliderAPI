...
func applyCodexOAuthTransformWithOptions(reqBody map[string]any, opts codexOAuthTransformOptions) codexTransformResult {
	...
	// 连接器/App 工具由上游以账号主人身份执行，共享 OAuth 账号时必须在别名收集前剥除，
	// 否则会把被删掉的工具名写进反向映射表。
...
// codexConnectorToolPrefixes 列出 ChatGPT 连接器工具的命名前缀（归一化后按 "_" 分隔匹配）。
var codexConnectorToolPrefixes = []string{
	"codex_apps",
}

// codexConnectorToolExactNames 列出连接器发现/枚举类工具的精确名称（已归一化）。
...
// normalizeCodexToolName 归一化工具名：小写并把 . - / 统一成 _，
// 这样 "codex_apps.github.get_repo"、"codex_apps-github"、"codex_apps/github" 等价。
...
// isCodexConnectorToolName 判断工具名是否属于 ChatGPT 连接器/App 工具。
// 前缀匹配要求整体相等或以 "前缀_" 开头，避免误伤 "my_codex_apps" 这类自定义工具。
...
// codexToolName 从 Responses/Chat Completions 两种工具形态里取出工具名。
...
// stripCodexConnectorTools 从 tools 中移除连接器工具并返回被移除的名称；
// 无移除时返回 nil 且不改动 reqBody。
...
// resetCodexToolChoiceIfRemoved 在 tool_choice 指向已移除工具时退回 "auto"，
// 否则上游会因为 tool_choice 找不到对应工具而 400。
