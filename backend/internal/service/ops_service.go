...
func (s *OpsService) UpdateErrorResolution(ctx context.Context, errorID int64, resolved bool, resolvedByUserID *int64) error {
	...
	return s.opsRepo.UpdateErrorResolution(ctx, errorID, resolved, resolvedByUserID, nil, nil)
}
...
func redactSensitiveJSON(v any) any {
	...
	case []any:
		...
		return out
	case string:
		// MariaDB does not accept NUL bytes in TEXT/VARCHAR columns, and
		// Postgres-format \u0000 escapes break MariaDB inserts. Strip NULs
		// from string content so queued request bodies stay insertable.
