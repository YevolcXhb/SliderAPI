...
func lockAuthPendingIdentityKeys(ctx context.Context, client *dbent.Client, keys ...string) (func(), error) {
	...
	normalized := normalizeAuthPendingIdentityLockKeys(keys...)
	if len(normalized) == 0 || client == nil || client.Driver() == nil {
		return release, nil
	}

	// MariaDB 10.11 使用 GET_LOCK 命名锁（会话级）；用 RELEASE_LOCK 显式释放。
	// 无 PG advisory lock；若并发路径已由 scoped key lock 串行化，GET_LOCK 是额外的
	// 跨进程防重入屏障，失败时回退为不放行并释放已有锁。
	// GET_LOCK 是 MariaDB 会话级命名锁；sqlite 单测没有该函数，仅在 MySQL 方言下执行。
	if client.Driver().Dialect() == dialect.MySQL {
		for _, key := range normalized {
			lockName := "sub2api_pa_" + strconv.FormatInt(authPendingIdentityAdvisoryLockHash(key), 10)
			var rows entsql.Rows
			if err := client.Driver().Query(ctx, "SELECT GET_LOCK(?, 10)", []any{lockName}, &rows); err != nil {
				release()
				return nil, err
			}
			// rows.Next() -> scan the GET_LOCK result (1=acquired).
			_ = rows.Close()
		}
	}
