...
const (
	deepseekFlashOffPeakInputPrice  = 2.2e-7  // ?.22 per MTok (cache miss)
	deepseekFlashOffPeakOutputPrice = 6.6e-7  // ?.66 per MTok
	deepseekFlashOffPeakCacheRead   = 7e-9    // ?.007 per MTok (cache hit)
	deepseekProOffPeakInputPrice    = 6.6e-7  // ?.66 per MTok (cache miss)
	deepseekProOffPeakOutputPrice   = 1.98e-6 // ?.98 per MTok
	deepseekProOffPeakCacheRead     = 2.2e-8  // ?.022 per MTok (cache hit)
)
...
func (s *BillingService) initFallbackPricing() {
	...
	s.fallbackPrices["claude-opus-4.5"] = &ModelPricing{
		...
		CacheReadPricePerToken:     0.5e-6,  // ?.50 per MTok
	...
	s.fallbackPrices["claude-sonnet-4"] = &ModelPricing{
		...
		CacheReadPricePerToken:     0.3e-6,  // ?.30 per MTok
		SupportsCacheBreakdown:     false,
	}
	...
	s.fallbackPrices["claude-3-5-haiku"] = &ModelPricing{
		...
		CacheReadPricePerToken:     0.1e-6,  // ?.10 per MTok
		SupportsCacheBreakdown:     false,
	}
	...
	s.fallbackPrices["claude-3-haiku"] = &ModelPricing{
		InputPricePerToken:         0.25e-6, // ?.25 per MTok
		OutputPricePerToken:        1.25e-6, // ?.25 per MTok
		CacheCreationPricePerToken: 0.3e-6,  // ?.30 per MTok
		CacheReadPricePerToken:     0.03e-6, // ?.03 per MTok
		SupportsCacheBreakdown:     false,
	}
	...
	// Claude 4.8 Opus / Claude Opus 5（标准 ?/?，Fast ?/? per MTok）。
	// 缺少这两条时 getFallbackPricing 会掉到 claude-3-opus（?/?），造成 3 倍超收。
	...
	s.fallbackPrices["gemini-3.1-pro"] = &ModelPricing{
		...
		CacheReadPricePerToken:     0.2e-6, // ?.20 per MTok
		SupportsCacheBreakdown:     false,
	}

	// Gemini 3.6 Flash (Google AI pricing: ?.50 input / ?.50 output /
	// ?.15 cached input per MTok). Antigravity's -high/-low/-medium/-tiered
	// aliases are matched below so unavailable remote pricing never records
	// token-bearing requests at ?.
	...
	s.fallbackPrices["gpt-5.4"] = &ModelPricing{
		...
		CacheReadPricePerToken:         0.25e-6, // ?.25 per MTok
		CacheReadPricePerTokenPriority: 0.5e-6,  // ?.5 per MTok
		SupportsCacheBreakdown:         false,
	...
	s.fallbackPrices["deepseek-v4-pro"] = &ModelPricing{
		InputPricePerToken:     deepseekProOffPeakInputPrice,  // ?.66 per MTok (cache miss, off-peak)
		OutputPricePerToken:    deepseekProOffPeakOutputPrice, // ?.98 per MTok
		CacheReadPricePerToken: deepseekProOffPeakCacheRead,   // ?.022 per MTok (cache hit)
		SupportsCacheBreakdown: false,
	}
	s.fallbackPrices["deepseek-v4-flash"] = &ModelPricing{
		InputPricePerToken:     deepseekFlashOffPeakInputPrice,  // ?.22 per MTok (cache miss, off-peak)
		OutputPricePerToken:    deepseekFlashOffPeakOutputPrice, // ?.66 per MTok
		CacheReadPricePerToken: deepseekFlashOffPeakCacheRead,   // ?.007 per MTok (cache hit)
		SupportsCacheBreakdown: false,
	}
	...
	// GLM-4.6 与 GLM-4.5 在 z.ai 国际版上定价一致；GLM-4.5 国内按 ¥0.8/¥2，汇率换算后约 ?.112/?.28，与国际版 ?.6/?.2 不同，本分支采用国际版 USD 口径与现有 Claude/GPT 一致。
	...
	s.fallbackPrices["glm-4.5-air"] = &ModelPricing{
		InputPricePerToken:     0.2e-6, // ?.20 per MTok
	...
	s.fallbackPrices["glm-4-32b-0414-128k"] = &ModelPricing{
		InputPricePerToken:     0.1e-6, // ?.10 per MTok
	...
	s.fallbackPrices["kimi-k3"] = &ModelPricing{
		InputPricePerToken:     3e-6,    // ?.00 per MTok (cache miss)
		OutputPricePerToken:    15e-6,   // ?.00 per MTok
		CacheReadPricePerToken: 0.30e-6, // ?.30 per MTok (cache hit)
		SupportsCacheBreakdown: false,
	}
	s.fallbackPrices["kimi-k2.6"] = &ModelPricing{
		InputPricePerToken:     0.95e-6, // ?.95 per MTok (cache miss)
		OutputPricePerToken:    4e-6,    // ?.00 per MTok
		CacheReadPricePerToken: 0.15e-6, // ?.15 per MTok (cache hit, ¥1.10)
		SupportsCacheBreakdown: false,
	}
	...
	s.fallbackPrices["minimax-m3"] = &ModelPricing{
		InputPricePerToken:     0.60e-6, // ?.60 per MTok (≤512K standard tier, 含 50% 永久折扣前原价 ?.20)
		OutputPricePerToken:    2.40e-6,
		CacheReadPricePerToken: 0.12e-6,
		SupportsCacheBreakdown: false,
	}
	s.fallbackPrices["minimax-m2.7"] = &ModelPricing{
		InputPricePerToken:     0.30e-6, // ?.30 per MTok
	...
	// 按量付费官方价 文本 ¥0.7/MTok、图片 ¥1.8/MTok；汇率口径 ÷7.14（与本表其他国产模型一致，¥1≈?.14）。
	// embedding 无 output，OutputPricePerToken 置 0。
	s.fallbackPrices["doubao-embedding-vision"] = &ModelPricing{
		InputPricePerToken:      0.098e-6, // ¥0.7/MTok ≈ ?.098（文本输入）
		ImageInputPricePerToken: 0.252e-6, // ¥1.8/MTok ≈ ?.252（图片输入）
		OutputPricePerToken:     0,
		SupportsCacheBreakdown:  false,
	}

	// xAI Grok 4.5: ? input / ?.30 cached input / ? output below 200k;
	// long-context rates are ? / ?.60 / ? (>=200k prompt tokens).
	...
	// xAI Grok 4.6: ? input / ?.50 cached input / ? output below 200k;
	// long-context rates are ? / ? / ? (>=200k prompt tokens).
	...
	// xAI Grok 4.3: ?.25 input / ?.20 cached / ?.50 output below 200k;
	// long-context rates are ?.50 / ?.40 / ?.
	...
	// Grok 4.20 variants share the official ?.25 / ?.20 / ?.50 card
	// (and ?.50 / ?.40 / ? long-context rates) with Grok 4.3.
	...
	// xAI Grok Build 0.1 (official docs: ? input / ?.20 cached input /
	// ? output per MTok). Composer is available only through Grok Build and
...
func (s *BillingService) GetModelPricing(model string) (*ModelPricing, error) {
	...
	if s.pricingService != nil {
		...
		// token 计费：直接返回会把 token 流量按 ? 计费。跳过后走 fallback，
...
const (
	...
	// Codex alpha/search 网页搜索单次默认价：OpenAI 官方 web search 定价 ?/1000 次。
	defaultWebSearchPricePerCall = 0.01

	// xAI server-side web/X search and code execution are ?/1000 calls.
...
func (s *BillingService) getDefaultImagePrice(model string, imageSize string) float64 {
	...
	// 如果没有找到价格，使用硬编码默认值（?.134，来自 gemini-3-pro-image-preview）
	if basePrice <= 0 {
