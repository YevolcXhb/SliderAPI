...
// 自动保留期归档路径（FindOldestUsageLogBefore/SnapshotUsageLogs/ExportUsageLogs）
// 不在本文件的用例覆盖范围内，仅补齐 UsageCleanupRepository 接口。
func (s *cleanupRepoStub) FindOldestUsageLogBefore(context.Context, time.Time) (*time.Time, error) {
	return nil, nil
}

func (s *cleanupRepoStub) SnapshotUsageLogs(context.Context, UsageCleanupFilters) error {
	return nil
}

func (s *cleanupRepoStub) ExportUsageLogs(context.Context, UsageCleanupFilters) (io.ReadCloser, error) {
