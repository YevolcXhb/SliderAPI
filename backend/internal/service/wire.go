...
// ProvideAccountService constructs AccountService with the repositories and runtime
// collaborators required by user-facing owned-account management (private proxies,
// share policies, exit probing and user-private-group provisioning).
func ProvideAccountService(
	accountRepo AccountRepository,
	groupRepo GroupRepository,
	proxyRepo ProxyRepository,
	sharePolicyRepo AccountSharePolicyRepository,
	...
	if sharePolicyRepo != nil {
		svc.SetAccountSharePolicyRepository(sharePolicyRepo)
	}
	if proxyProber != nil {
		svc.SetProxyProber(proxyProber)
	}
	if privateGroupProvisioner != nil {
