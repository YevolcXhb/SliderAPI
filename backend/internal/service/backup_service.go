...
const (
	...
	// independently, producing N concurrent mariadb-dumps against the same database,
...
type BackupRecord struct {
	...
	BackupType    string       `json:"backup_type"` // mariadb
...
func (s *BackupService) createCompressedBackupFile(ctx context.Context) (string, int64, error) {
	...
	if err != nil {
		return "", 0, fmt.Errorf("mariadb-dump: %w", err)
	}
...
const settingKeyUsageRetention = "usage_cleanup_auto_retention"
