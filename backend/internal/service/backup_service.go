...
func (s *BackupService) CreateBackup(ctx context.Context, triggeredBy string, expireDays int) (*BackupRecord, error) {
	...
	// 流式执行: mariadb-dump -> gzip -> S3 upload
	dumpReader, err := s.dumper.Dump(ctx)
	if err != nil {
		record.Status = "failed"
		record.ErrorMsg = fmt.Sprintf("mariadb-dump failed: %v", err)
		record.FinishedAt = time.Now().Format(time.RFC3339)
		_ = s.saveRecord(ctx, record)
		return record, fmt.Errorf("mariadb-dump: %w", err)
	}
...
func (s *BackupService) RestoreBackup(ctx context.Context, backupID string) error {
	...
	if record.BackupType != "" && record.BackupType != "mysql" {
		return infraerrors.BadRequest("BACKUP_TYPE_NOT_RESTORABLE", "only full mysql backups can be restored automatically")
	}
...
func (s *BackupService) StartRestore(ctx context.Context, backupID string) (*BackupRecord, error) {
	...
	if record.BackupType != "" && record.BackupType != "mysql" {
		return nil, infraerrors.BadRequest("BACKUP_TYPE_NOT_RESTORABLE", "only full mysql backups can be restored automatically")
	}
...
func (s *BackupService) executeRestore(record *BackupRecord, objectStore BackupObjectStore) {
	...
	if err := s.dumper.Restore(ctx, gzReader); err != nil {
		record.RestoreStatus = "failed"
		record.RestoreError = fmt.Sprintf("mariadb restore: %v", err)
		_ = s.saveRecord(context.Background(), record)
		return
	}
