...
func TestCalculateCost_BasicComputation(t *testing.T) {
	...
	// 使用 claude-sonnet-4 的回退价格：Input ?/MTok, Output ?/MTok
	tokens := UsageTokens{
		InputTokens:  1000,
		OutputTokens: 500,
	}
...
// 历史 bug:兜底表缺 glm-5.2 条目,使用记录按 ?.00/?.20 计费,比官方 ?.40/?.40 少收约 27%。
...
// output_tokens=439（全部图片输出）。官方定价：文本输入 ?/1M、图片输入 ?/1M、
// 文本输出 ?/1M、图片输出 ?/1M。修复前图片输入被并入文本价，单次偏低 ~6.6%。
func TestComputeTokenBreakdown_GptImage2ImageEditIssue4386(t *testing.T) {
	...
	require.InDelta(t, wantImageInput, cost.ImageInputCost, 1e-15, "图片输入按 ?/1M 独立计费")
	require.Zero(t, cost.OutputCost, "输出全部为图片，文本输出费用为 0")
	require.InDelta(t, wantImageOutput, cost.ImageOutputCost, 1e-15)
	require.InDelta(t, 0.016081, cost.TotalCost, 1e-9, "总额应为 ?.016081（修复前为 ?.015025）")
}
