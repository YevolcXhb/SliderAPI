...
func TestGetModelPricing_UnknownDeepseekMapsToFlash(t *testing.T) {
	// JSON 含 ? 占位条目（如旧 deepseek-v3-2-251201）：未知 deepseek-* 不再
	// fail-closed，统一按 flash 价兜底（2.2e-7/6.6e-7/7e-9），不得按 ? 计费。
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
...
func TestDeepseekPricingFileMatchesOfficialRates(t *testing.T) {
	...
	require.False(t, ok, "deepseek-v3-2-251201（? 占位条目）必须从价格表中移除")
