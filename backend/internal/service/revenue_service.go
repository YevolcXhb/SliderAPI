...
func (s *RevenueService) ListShareSettlements(ctx context.Context, params RevenueShareSettlementQueryParams) ([]RevenueShareSettlementItem, int64, error) {
	...
	query := fmt.Sprintf(`
		SELECT
			...
			ase.account_id,
			COALESCE(NULLIF(a.name, ''), CONCAT('Account #', ase.account_id)) AS account_name,
			...
			ase.platform_fee,
			(ase.platform_fee - ase.account_cost) AS platform_net_profit,
			ase.status,
			ase.created_at
		FROM account_share_settlement_entries ase
		LEFT JOIN users cu ON cu.id = ase.consumer_user_id
...
func revenueUserFilter(column string, userID *int64, placeholder int) (string, []any) {
	...
	return fmt.Sprintf(" AND %s = ?", column), []any{*userID}
}
...
func buildRevenueShareSettlementWhere(params RevenueShareSettlementQueryParams, status string) (string, []any) {
	...
	if status != "" {
		...
		conditions = append(conditions, "ase.status = ?")
	}
	if search := strings.TrimSpace(params.Search); search != "" {
		args = append(args, "%"+search+"%")
		placeholder := "?"
		conditions = append(conditions, fmt.Sprintf(`(
			LOWER(ase.request_id) LIKE LOWER(%s)
			OR LOWER(COALESCE(cu.email, '')) LIKE LOWER(%s)
			OR LOWER(COALESCE(cu.username, '')) LIKE LOWER(%s)
			OR LOWER(COALESCE(ou.email, '')) LIKE LOWER(%s)
			OR LOWER(COALESCE(ou.username, '')) LIKE LOWER(%s)
			OR LOWER(COALESCE(a.name, '')) LIKE LOWER(%s)
			OR LOWER(COALESCE(ak.name, '')) LIKE LOWER(%s)
...
func revenueBucketExpression(column, granularity string) string {
	if granularity == RevenueGranularityHour {
		return fmt.Sprintf("DATE_FORMAT(CONVERT_TZ(%s, '+00:00', ?), '%%Y-%%m-%%d %%H:00')", column)
	}
	return fmt.Sprintf("DATE_FORMAT(CONVERT_TZ(%s, '+00:00', ?), '%%Y-%%m-%%d')", column)
}
...
func (s *RevenueService) fillRevenueCashStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := `
		SELECT
			COALESCE(SUM(CASE WHEN paid_at >= ? AND paid_at < ? THEN pay_amount ELSE 0 END), 0) AS paid_amount,
			COALESCE(SUM(CASE WHEN paid_at >= ? AND paid_at < ? AND order_type = ? THEN pay_amount ELSE 0 END), 0) AS balance_paid_amount,
			COALESCE(SUM(CASE WHEN paid_at >= ? AND paid_at < ? AND order_type = ? THEN pay_amount ELSE 0 END), 0) AS subscription_paid_amount,
			COALESCE(SUM(CASE WHEN paid_at >= ? AND paid_at < ? THEN 1 ELSE 0 END), 0) AS paid_order_count,
			COALESCE(SUM(CASE WHEN refund_at >= ? AND refund_at < ? AND refund_amount > 0 THEN refund_amount ELSE 0 END), 0) AS refund_amount,
			COALESCE(SUM(CASE WHEN refund_at >= ? AND refund_at < ? AND refund_amount > 0 THEN 1 ELSE 0 END), 0) AS refund_order_count,
			COALESCE(SUM(CASE WHEN status = ? AND created_at >= ? AND created_at < ? THEN pay_amount ELSE 0 END), 0) AS pending_amount,
			COALESCE(SUM(CASE WHEN status = ? AND created_at >= ? AND created_at < ? THEN 1 ELSE 0 END), 0) AS pending_order_count
		FROM payment_orders
		WHERE ((paid_at >= ? AND paid_at < ?)
			OR (refund_at >= ? AND refund_at < ?)
			OR (status = ? AND created_at >= ? AND created_at < ?))
	`
	if userFilter != "" {
		query += userFilter
	}
	args := []any{
		...
		// balance_paid_amount
		params.StartTime, params.EndTime, payment.OrderTypeBalance,
		// subscription_paid_amount
		params.StartTime, params.EndTime, payment.OrderTypeSubscription,
		// paid_order_count
		params.StartTime, params.EndTime,
		// refund_amount
		params.StartTime, params.EndTime,
		...
		params.StartTime, params.EndTime,
		// pending_amount
		OrderStatusPending, params.StartTime, params.EndTime,
		// pending_order_count
		OrderStatusPending, params.StartTime, params.EndTime,
		// outer WHERE
		params.StartTime, params.EndTime,
	...
	trendQuery := fmt.Sprintf(`
		...
		FROM (
			SELECT %s AS bucket, COALESCE(SUM(pay_amount), 0) AS paid_amount, 0 AS refund_amount
			FROM payment_orders
			WHERE paid_at >= ? AND paid_at < ?
				%s
			GROUP BY 1
			UNION ALL
			SELECT %s AS bucket, 0 AS paid_amount, COALESCE(SUM(refund_amount), 0) AS refund_amount
			FROM payment_orders
			WHERE refund_at >= ? AND refund_at < ? AND refund_amount > 0
				%s
			GROUP BY 1
		) s
		GROUP BY bucket
...
func (s *RevenueService) fillRevenueRedeemCashStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	if hasRevenueFlag {
		typePredicate = "(type = ? OR (type = ? AND COALESCE(count_as_revenue, FALSE)))"
		args = append(args, AdjustmentTypeAdminBalance)
		userFilter, userArgs = revenueUserFilter("used_by", params.UserID, 7)
	}
	query := fmt.Sprintf(`
		SELECT
			COALESCE(SUM(value), 0) AS redeem_balance_amount,
	...
	trendTypePredicate := "type = ?"
	trendArgs := []any{params.Timezone, params.StartTime, params.EndTime, StatusUsed, RedeemTypeBalance}
	if hasRevenueFlag {
		trendTypePredicate = "(type = ? OR (type = ? AND COALESCE(count_as_revenue, FALSE)))"
...
func (s *RevenueService) fillRevenueUsageStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := fmt.Sprintf(`
		SELECT
			...
			COALESCE(SUM(actual_cost), 0) AS consumed_revenue,
	...
	trendArgs := []any{params.Timezone, params.StartTime, params.EndTime}
	trendArgs = append(trendArgs, trendUserArgs...)
	rows, err := s.entClient.QueryContext(ctx, trendQuery, trendArgs...)
	if err != nil {
...
func (s *RevenueService) fillRevenueUsageStatsFromSnapshots(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := fmt.Sprintf(`
		...
		combined AS (
			SELECT
				s.bucket_date,
				SUM(s.total_requests) AS requests,
				SUM(s.total_tokens) AS total_tokens,
				SUM(s.standard_cost) AS standard_cost,
				SUM(s.consumed_revenue) AS consumed_revenue,
				SUM(s.account_cost) AS account_cost
			...
			UNION ALL
			SELECT
				CONVERT_TZ(ul.created_at, '+00:00', 'Asia/Shanghai') AS bucket_date,
				COUNT(*) AS requests,
			...
			WHERE ul.created_at >= ? AND ul.created_at < ?
				AND NOT EXISTS (
					...
					WHERE sd.bucket_date = CONVERT_TZ(ul.created_at, '+00:00', 'Asia/Shanghai')
				)
				%s
			GROUP BY 1
		)
		SELECT
			...
			COALESCE(SUM(standard_cost), 0),
			COALESCE(SUM(consumed_revenue), 0),
	...
	trendQuery := fmt.Sprintf(`
		...
		combined AS (
			SELECT
				DATE_FORMAT(s.bucket_date, '%%Y-%%m-%%d') AS bucket,
				SUM(s.total_requests) AS requests,
				SUM(s.consumed_revenue) AS consumed_revenue,
				SUM(s.account_cost) AS account_cost
			FROM revenue_daily_dimension_snapshots s
			...
			UNION ALL
			SELECT
				DATE_FORMAT(CONVERT_TZ(ul.created_at, '+00:00', ?), '%%Y-%%m-%%d') AS bucket,
				COUNT(*) AS requests,
				COALESCE(SUM(ul.actual_cost), 0) AS consumed_revenue,
				COALESCE(SUM(COALESCE(ul.account_stats_cost, ul.total_cost) * COALESCE(ul.account_rate_multiplier, 1)), 0) AS account_cost
			FROM usage_logs ul
...
func (s *RevenueService) fillRevenueWalletBreakdownStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	pointsQuery := `
		SELECT
			COALESCE(SUM(CASE WHEN direction = 'debit' AND reason IN (?, ?) THEN amount ELSE 0 END), 0) AS points_consumed,
			COALESCE(SUM(CASE WHEN direction = 'credit' AND reason IN (?, ?, ?) THEN amount ELSE 0 END), 0) AS points_issued
		FROM points_ledger
		WHERE created_at >= ? AND created_at < ?
	`
	...
	trendBalanceQuery := fmt.Sprintf(`
		SELECT %s AS bucket, COALESCE(SUM(amount), 0) AS balance_consumed
		FROM user_balance_ledger
		WHERE created_at >= ? AND created_at < ?
			AND direction = 'debit'
			AND reason = ?
	...
	trendBalanceArgs := []any{params.Timezone, params.StartTime, params.EndTime, "usage_charge"}
...
func (s *RevenueService) fillRevenueAffiliateStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := `
		SELECT
			COALESCE(SUM(CASE WHEN action = ? THEN amount ELSE 0 END), 0) AS affiliate_rebate,
			COALESCE(SUM(CASE WHEN action = ? THEN amount ELSE 0 END), 0) AS affiliate_transfer,
			COALESCE(SUM(CASE WHEN action = ? THEN 1 ELSE 0 END), 0) AS affiliate_rebate_count
		FROM user_affiliate_ledger
		WHERE created_at >= ? AND created_at < ?
	`
	query += userFilter
	args := []any{revenueAffiliateActionAccrue, revenueAffiliateActionTransfer, revenueAffiliateActionAccrue, params.StartTime, params.EndTime}
	args = append(args, userArgs...)
	if err := s.querySingle(ctx, query, args,
	...
	trendQuery := fmt.Sprintf(`
		SELECT %s AS bucket, COALESCE(SUM(amount), 0) AS affiliate_rebate
		FROM user_affiliate_ledger
	...
	trendArgs := []any{params.Timezone, params.StartTime, params.EndTime, revenueAffiliateActionAccrue}
	trendArgs = append(trendArgs, trendUserArgs...)
	rows, err := s.entClient.QueryContext(ctx, trendQuery, trendArgs...)
	if err != nil {
...
func (s *RevenueService) fillRevenueShareStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := `
		SELECT
			COALESCE(SUM(consumer_charge), 0) AS consumer_charge,
			COALESCE(SUM(account_cost), 0) AS account_cost,
			COALESCE(SUM(owner_credit), 0) AS owner_credit,
	...
	trendQuery := fmt.Sprintf(`
		...
		ORDER BY 1
	`, bucketExpr, trendUserFilter)
	trendArgs := []any{params.Timezone, params.StartTime, params.EndTime, revenueShareStatusApplied}
	trendArgs = append(trendArgs, trendUserArgs...)
	rows, err := s.entClient.QueryContext(ctx, trendQuery, trendArgs...)
	if err != nil {
...
func (s *RevenueService) fillRevenueShareStatsFromSnapshots(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := fmt.Sprintf(`
		...
		combined AS (
			SELECT
				s.bucket_date,
				SUM(s.share_consumer_charge) AS consumer_charge,
				SUM(s.share_account_cost) AS account_cost,
				SUM(s.share_owner_credit) AS owner_credit,
				SUM(s.share_platform_fee) AS platform_fee,
				SUM(CASE WHEN s.share_owner_credit > 0 OR s.share_platform_fee > 0 THEN s.total_requests ELSE 0 END) AS settlement_count
			FROM revenue_daily_dimension_snapshots s
			WHERE s.bucket_date >= ? AND s.bucket_date < ?
				%s
			GROUP BY s.bucket_date
			UNION ALL
			SELECT
				CONVERT_TZ(ase.created_at, '+00:00', 'Asia/Shanghai') AS bucket_date,
				COALESCE(SUM(ase.consumer_charge), 0) AS consumer_charge,
				COALESCE(SUM(ase.account_cost), 0) AS account_cost,
				COALESCE(SUM(ase.owner_credit), 0) AS owner_credit,
				COALESCE(SUM(ase.platform_fee), 0) AS platform_fee,
				COUNT(*) AS settlement_count
			FROM account_share_settlement_entries ase
			WHERE ase.created_at >= ? AND ase.created_at < ? AND ase.status = ?
				AND ase.consumer_user_id <> ase.owner_user_id
				AND NOT EXISTS (
					SELECT 1
					FROM snapshot_days sd
					WHERE sd.bucket_date = CONVERT_TZ(ase.created_at, '+00:00', 'Asia/Shanghai')
				)
				%s
			GROUP BY 1
		)
		SELECT
			COALESCE(SUM(consumer_charge), 0),
			COALESCE(SUM(account_cost), 0),
			COALESCE(SUM(owner_credit), 0),
			COALESCE(SUM(platform_fee), 0),
			COALESCE(SUM(settlement_count), 0)
		FROM combined
	`, statsSnapshotUserFilter, statsSnapshotUserFilter, statsLiveUserFilter)
	...
	trendQuery := fmt.Sprintf(`
		...
		combined AS (
			...
			UNION ALL
			SELECT
				DATE_FORMAT(CONVERT_TZ(ase.created_at, '+00:00', ?), '%%Y-%%m-%%d') AS bucket,
				COALESCE(SUM(ase.owner_credit), 0) AS owner_credit,
				COALESCE(SUM(ase.platform_fee), 0) AS platform_fee
			FROM account_share_settlement_entries ase
			WHERE ase.created_at >= ? AND ase.created_at < ? AND ase.status = ?
...
func (s *RevenueService) fillRevenuePrivateGroupCommissionStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := `
		SELECT COALESCE(SUM(amount), 0)
		FROM user_balance_ledger
		WHERE created_at >= ? AND created_at < ? AND reason = ?
	`
	query += userFilter
	...
	trendUserFilter, trendUserArgs := revenueUserFilter("user_id", params.UserID, 5)
	trendQuery := fmt.Sprintf(`
		SELECT %s AS bucket, COALESCE(SUM(amount), 0) AS commission
		FROM user_balance_ledger
		WHERE created_at >= ? AND created_at < ? AND reason = ?
			%s
		GROUP BY 1
...
func (s *RevenueService) queryRevenueBreakdown(ctx context.Context, params RevenueQueryParams, kind revenueBreakdownKind) ([]RevenueBreakdownItem, error) {
	...
	case revenueBreakdownAccounts:
		selectExpr = "ul.account_id AS id, COALESCE(NULLIF(a.name, ''), CONCAT('Account #', ul.account_id)) AS name, COALESCE(NULLIF(a.platform, ''), '') AS secondary"
	...
	query := fmt.Sprintf(`
		SELECT
			...
			COALESCE(SUM(ul.actual_cost), 0) AS consumed_revenue,
			COALESCE(SUM(%s), 0) AS account_cost,
			COALESCE(SUM(ase.owner_credit), 0) AS share_owner_credit
		FROM usage_logs ul
		%s
...
func (s *RevenueService) queryRevenueBreakdownFromSnapshots(ctx context.Context, params RevenueQueryParams, kind revenueBreakdownKind) ([]RevenueBreakdownItem, error) {
	...
	if kind == revenueBreakdownModels {
		query = fmt.Sprintf(`
			...
			combined AS (
				SELECT
					...
					SUM(s.consumed_revenue) AS consumed_revenue,
					SUM(s.account_cost) AS account_cost,
					SUM(s.share_owner_credit) AS share_owner_credit
				FROM revenue_daily_dimension_snapshots s
				WHERE s.bucket_date >= ? AND s.bucket_date < ?
					%s
				GROUP BY %s
			...
			FROM (
				SELECT
					...
					COALESCE(SUM(consumed_revenue), 0) AS consumed_revenue,
					COALESCE(SUM(account_cost), 0) AS account_cost,
					COALESCE(SUM(share_owner_credit), 0) AS share_owner_credit
				FROM combined
				GROUP BY id, model_name
			) rolled_models
			ORDER BY rolled_models.consumed_revenue DESC, rolled_models.requests DESC
			LIMIT ?
		`, statsSnapshotUserFilter, snapshotSelectExpr, statsSnapshotUserFilter, snapshotGroupExpr, liveSelectExpr, statsLiveUserFilter, liveGroupExpr)
	} else {
		...
		query = fmt.Sprintf(`
			...
			LIMIT ?
		`, statsSnapshotUserFilter, snapshotSelectExpr, statsSnapshotUserFilter, snapshotGroupExpr, liveSelectExpr, statsLiveUserFilter, liveGroupExpr, nameExpr, secondaryExpr, joinExpr)
	}
...
func revenueBreakdownNameExpressions(kind revenueBreakdownKind) (string, string) {
	...
	case revenueBreakdownAccounts:
		return "COALESCE(NULLIF(a.name, ''), CONCAT('Account #', rolled.id))", "COALESCE(NULLIF(a.platform, ''), '')"
	default:
		return "''", "''"
	}
}
	...
	query := fmt.Sprintf(`
		...
		combined AS (
			SELECT
				...
				SUM(CASE WHEN s.share_owner_credit > 0 OR s.share_platform_fee > 0 THEN s.total_requests ELSE 0 END) AS requests,
				SUM(CASE WHEN s.share_owner_credit > 0 OR s.share_platform_fee > 0 THEN s.total_tokens ELSE 0 END) AS total_tokens,
				SUM(s.share_consumer_charge) AS consumer_charge,
				SUM(s.share_account_cost) AS account_cost,
				SUM(s.share_owner_credit) AS owner_credit,
				SUM(s.share_platform_fee) AS platform_fee
		...
		SELECT
			...
			rolled.platform_fee,
			CASE
				WHEN rolled.consumer_charge > 0 THEN (rolled.owner_credit / rolled.consumer_charge)
				ELSE 0
			END AS owner_share_ratio
		FROM rolled
