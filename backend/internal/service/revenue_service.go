...
func (s *RevenueService) ListShareSettlements(ctx context.Context, params RevenueShareSettlementQueryParams) ([]RevenueShareSettlementItem, int64, error) {
	...
	query := fmt.Sprintf(`
		SELECT
			...
			ase.account_id,
			COALESCE(NULLIF(a.name, ''), CONCAT('Account #', CAST(ase.account_id AS CHAR))) AS account_name,
			...
			CAST(ase.consumer_charge AS DOUBLE),
			CAST(ase.account_cost AS DOUBLE),
			CAST(ase.owner_share_ratio AS DOUBLE),
			CAST(ase.owner_credit AS DOUBLE),
			ase.invite_bound_at_snapshot,
			ase.invite_expires_at_snapshot,
			CAST(ase.invite_share_ratio AS DOUBLE),
			CAST(ase.invite_credit AS DOUBLE),
			CAST(ase.platform_share_ratio AS DOUBLE),
			CAST(ase.platform_fee AS DOUBLE),
...
func buildRevenueShareSettlementWhere(params RevenueShareSettlementQueryParams, status string) (string, []any) {
	...
	if search := strings.TrimSpace(params.Search); search != "" {
		...
		conditions = append(conditions, fmt.Sprintf(`(
			...
			OR COALESCE(cu.email, ''LOWER()) LIKE LOWER(%s)
			OR COALESCE(cu.username, ''LOWER()) LIKE LOWER(%s)
			OR COALESCE(ou.email, ''LOWER()) LIKE LOWER(%s)
			OR COALESCE(ou.username, ''LOWER()) LIKE LOWER(%s)
			OR COALESCE(a.name, ''LOWER()) LIKE LOWER(%s)
			OR COALESCE(ak.name, ''LOWER()) LIKE LOWER(%s)
...
func revenueBucketExpression(column, granularity string) string {
	if granularity == RevenueGranularityHour {
		return fmt.Sprintf("DATE_FORMAT(CONVERT_TZ(%s, @@session.time_zone, ?), '%%Y-%%m-%%d %%H:00:00')", column)
	}
	return fmt.Sprintf("DATE_FORMAT(CONVERT_TZ(%s, @@session.time_zone, ?), '%%Y-%%m-%%d')", column)
}
...
func (s *RevenueService) fillRevenueCashStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := `
		SELECT
			...
			SUM(CASE WHEN paid_at >= ? AND paid_at < ? THEN 1 ELSE 0 END) AS paid_order_count,
			COALESCE(SUM(CASE WHEN refund_at >= ? AND refund_at < ? AND refund_amount > 0 THEN refund_amount ELSE 0 END), 0) AS refund_amount,
			SUM(CASE WHEN refund_at >= ? AND refund_at < ? AND refund_amount > 0 THEN 1 ELSE 0 END) AS refund_order_count,
			COALESCE(SUM(CASE WHEN status = ? AND created_at >= ? AND created_at < ? THEN pay_amount ELSE 0 END), 0) AS pending_amount,
			SUM(CASE WHEN status = ? AND created_at >= ? AND created_at < ? THEN 1 ELSE 0 END) AS pending_order_count
		FROM payment_orders
...
func (s *RevenueService) redeemCodeRevenueFlagAvailable(ctx context.Context) bool {
	...
	if s.entClient.Driver().Dialect() == "sqlite3" {
...
func (s *RevenueService) fillRevenueUsageStatsFromSnapshots(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
		...
		combined AS (
			...
			WHERE ul.created_at >= ? AND ul.created_at < ?
				AND NOT EXISTS (
					...
					WHERE sd.bucket_date = (CONVERT_TZ(ul.created_at, @@session.time_zone, 'Asia/Shanghai'))
				)
				%s
			GROUP BY 1
		)
	...
	trendQuery := fmt.Sprintf(`
		...
		combined AS (
			SELECT
				DATE_FORMAT(CAST(s.bucket_date AS DATETIME), '%%Y-%%m-%%d') AS bucket,
				SUM(s.total_requests) AS requests,
				SUM(s.consumed_revenue) AS consumed_revenue,
				SUM(s.account_cost) AS account_cost
			FROM revenue_daily_dimension_snapshots s
			...
			UNION ALL
			SELECT
				DATE_FORMAT(CONVERT_TZ(ul.created_at, @@session.time_zone, ?), '%%Y-%%m-%%d') AS bucket,
				COUNT(*) AS requests,
				COALESCE(SUM(ul.actual_cost), 0) AS consumed_revenue,
				COALESCE(SUM(COALESCE(ul.account_stats_cost, ul.total_cost) * COALESCE(ul.account_rate_multiplier, 1)), 0) AS account_cost
			FROM usage_logs ul
...
func (s *RevenueService) fillRevenueAffiliateStats(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := `
		SELECT
			...
			SUM(CASE WHEN action = ? THEN 1 ELSE 0 END) AS affiliate_rebate_count
...
func (s *RevenueService) fillRevenueShareStatsFromSnapshots(ctx context.Context, params RevenueQueryParams, out *RevenueSummary, pointIndex map[string]int) error {
	...
	query := fmt.Sprintf(`
		...
		combined AS (
			...
			UNION ALL
			SELECT
				(CONVERT_TZ(ase.created_at, @@session.time_zone, 'Asia/Shanghai')) AS bucket_date,
			...
			WHERE ase.created_at >= ? AND ase.created_at < ? AND ase.status = ?
				...
				AND NOT EXISTS (
					SELECT 1
					FROM snapshot_days sd
					WHERE sd.bucket_date = (CONVERT_TZ(ase.created_at, @@session.time_zone, 'Asia/Shanghai'))
				)
				%s
			GROUP BY 1
		)
	...
	trendQuery := fmt.Sprintf(`
		...
		combined AS (
			...
			UNION ALL
			SELECT
				DATE_FORMAT(CONVERT_TZ(ase.created_at, @@session.time_zone, ?), '%%Y-%%m-%%d') AS bucket,
				COALESCE(SUM(ase.owner_credit), 0) AS owner_credit,
				COALESCE(SUM(ase.platform_fee), 0) AS platform_fee
			FROM account_share_settlement_entries ase
			WHERE ase.created_at >= ? AND ase.created_at < ? AND ase.status = ?
...
func (s *RevenueService) queryRevenueBreakdown(ctx context.Context, params RevenueQueryParams, kind revenueBreakdownKind) ([]RevenueBreakdownItem, error) {
	...
	case revenueBreakdownAccounts:
		selectExpr = "ul.account_id AS id, COALESCE(NULLIF(a.name, ''), CONCAT('Account #', CAST(ul.account_id AS CHAR))) AS name, COALESCE(NULLIF(a.platform, ''), '') AS secondary"
...
func revenueBreakdownNameExpressions(kind revenueBreakdownKind) (string, string) {
	...
	case revenueBreakdownAccounts:
		return "COALESCE(NULLIF(a.name, ''), CONCAT('Account #', CAST(rolled.id AS CHAR)))", "COALESCE(NULLIF(a.platform, ''), '')"
	default:
		return "''", "''"
	}
}
...
func (s *RevenueService) queryRevenueShareOwnerBreakdownFromSnapshots(ctx context.Context, params RevenueQueryParams) ([]RevenueShareOwnerBreakdownItem, error) {
	...
	query := fmt.Sprintf(`
		...
		SELECT
			...
			rolled.platform_fee,
			CASE
				WHEN rolled.consumer_charge > 0 THEN CAST((rolled.owner_credit / rolled.consumer_charge) AS DOUBLE)
				ELSE CAST(0 AS DOUBLE)
			END AS owner_share_ratio
		FROM rolled
