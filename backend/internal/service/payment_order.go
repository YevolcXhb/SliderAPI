...
func (s *PaymentService) invokeProvider(ctx context.Context, order *dbent.PaymentOrder, req CreateOrderRequest, cfg *PaymentConfig, limitAmount float64, payAmountStr string, payAmount float64, plan *dbent.SubscriptionPlan, sel *payment.InstanceSelection) (*CreateOrderResponse, error) {
	...
	if err != nil {
		...
		// 上游 CreatePayment 失败后无法确定远端是否已建单（超时/网络中断都会走到这里），
		// 因此包裹成 possiblyCreated，调用方（如商城）据此决定是否立刻释放占用的库存。
		if appErr := new(infraerrors.ApplicationError); errors.As(err, &appErr) {
	...
	_, err = s.entClient.PaymentOrder.UpdateOneID(order.ID).
		...
		Save(ctx)
	if err != nil {
		// 上游已经建单成功，只是本地回填失败：同样按 possiblyCreated 处理。
...
// paymentProviderOrderPossiblyCreatedError 标记「上游可能已经建单」的失败：
// 此时本地订单不能直接判定为未创建，否则会与上游账不平。
