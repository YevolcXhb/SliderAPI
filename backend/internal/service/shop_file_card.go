...
func (s *ShopService) insertFileCardKey(ctx context.Context, productID int64, storageKey, filename, contentType string, byteSize int64, sha string) (*ShopCardKeyDTO, error) {
	queryer, ok := sqlDBFromEntClient(s.entClient)
	if !ok {
		return nil, fmt.Errorf("shop file card insert requires QueryContext support")
	}
...
func (s *ShopService) listDeliveredFileCards(ctx context.Context, orderID int64) ([]ShopDeliveredFileDTO, error) {
	...
	return queryDeliveredFileCards(ctx, queryer, `
		...
		FROM shop_card_keys
		WHERE order_id = ? AND status = ? AND card_type = ?
		ORDER BY id ASC
...
// sqlDBFromEntClient extracts the underlying *sql.DB from an ent client so raw
// MariaDB SQL can run against the same connection pool. MariaDB-only fork: ent
// does not expose the *sql.DB publicly.
func sqlDBFromEntClient(client *dbent.Client) (*sql.DB, bool) {
	if client == nil {
		return nil, false
	}
	...
	if d, ok := driver.(interface{ DB() *sql.DB }); ok {
		return d.DB(), true
	}
	return nil, false
}
