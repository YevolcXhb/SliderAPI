...
func (s *PaymentService) tryClaimAffiliateRebateAudit(ctx context.Context, client *dbent.Client, orderID int64, baseAmount float64) (bool, error) {
	...
	if err != nil {
		// sqlite 单测没有 INSERT IGNORE；唯一约束冲突视为已认领。
		if client.Driver() != nil && client.Driver().Dialect() != dialect.MySQL && strings.Contains(strings.ToLower(err.Error()), "unique constraint failed") {
			return false, nil
		}
		return false, err
	}
...
func buildAffiliateRebateAuditClaimQuery(client *dbent.Client, orderID, detail string) (string, []any) {
	// MariaDB 10.11 不支持 PG 的 data-modifying CTE / ON DUPLICATE KEY UPDATE id=id RETURNING。
	// 这里用 INSERT IGNORE（唯一键冲突时跳过）并仅在新行插入时返回 affected>0，
	// INSERT ... ON DUPLICATE KEY UPDATE ... 会让受影响行数恒为 1，无法区分新插入。
	insertPrefix := "INSERT IGNORE INTO payment_audit_logs"
	if client.Driver() != nil && client.Driver().Dialect() != dialect.MySQL {
		insertPrefix = "INSERT INTO payment_audit_logs"
	}
	return fmt.Sprintf("%s (order_id, action, detail, operator, created_at)\nVALUES (?, 'AFFILIATE_REBATE_APPLIED', ?, 'system', CURRENT_TIMESTAMP)", insertPrefix), []any{orderID, detail}
}
...
// AdminManualFulfillOrder marks an externally verified order as paid and
// executes fulfillment. Ported from ikik fork.
