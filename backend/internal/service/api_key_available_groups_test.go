...
func TestAPIKeyService_GetAvailableGroups_PublicBalanceGroupIsSelectable(t *testing.T) {
	...
	svc := NewAPIKeyService(
		nil,
		&apiKeyAvailableGroupsUserRepoStub{user: &User{ID: userID, AllowedGroups: []int64{10}}},
...
// 下列方法仅为满足 UserRepository / UserSubscriptionRepository 接口；
// 可用分组查询路径不会触达它们，误调用直接 panic 以暴露测试假设变化。
