...
func (s *AuthService) applyProviderDefaultSettingsOnFirstBind(
	...
	grantSQL := `INSERT INTO user_provider_default_grants (user_id, provider_type, grant_reason)
VALUES (?, ?, ?)
ON DUPLICATE KEY UPDATE id=id`
	if client.Driver().Dialect() != dialect.MySQL {
		// sqlite 单测没有 ON DUPLICATE KEY UPDATE；唯一约束冲突视为已授权。
		grantSQL = `INSERT INTO user_provider_default_grants (user_id, provider_type, grant_reason)
VALUES (?, ?, ?)`
	}
	if err := client.Driver().Exec(
		...
		if client.Driver().Dialect() != dialect.MySQL && strings.Contains(strings.ToLower(err.Error()), "unique constraint failed") {
			return nil
		}
