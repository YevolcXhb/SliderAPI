...
// 定价：input ?/MTok, output ?/MTok, cache_read ?.3/MTok,
// 5m 写入 ?/MTok, 1h 写入 ?/MTok（SupportsCacheBreakdown=true）。
func TestBillingInvariant_CacheTier5mVs1h(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantInput:  0.003,  // 1000 × ?/MTok = 0.003
			wantOutput: 0.0075, // 500 × ?/MTok = 0.0075
			// 8000 × 4e-6 = 0.032; 4000 × 5e-6 = 0.020; 合计 0.052
			wantCacheCreation: 0.052,
			wantCacheRead:     0.0006, // 2000 × ?.3/MTok = 0.0006
...
func TestBillingInvariant_ServiceTierThenRateMultiplier(t *testing.T) {
	tests := []struct {
		...
		{
			// gpt-5.4 fallback 价：in ?.5/MTok, out ?/MTok, cache_write ?.5/MTok,
			// cache_read ?.25/MTok；flex 无显式价 → 0.5 倍 tier multiplier。
		...
		{
			// gpt-5.4 有显式 priority 价：in ?/MTok, out ?/MTok, cache_read ?.5/MTok。
