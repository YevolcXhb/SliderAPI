...
// validateUpdateReleaseSource 确认 release 元数据（页面与全部资产）都来自本仓库。
// GitHub API 被劫持或配置指向别的仓库时，自更新会把无关产物覆盖到运行中的二进制上。
...
// validateUpdateRepositoryURL 要求 URL 为 https://github.com/<githubRepo>/releases/...。
// 比 validateDownloadURL 更严格：后者只校验主机白名单（含资产 CDN），
// 这里额外把路径钉死到本仓库。
...
func (s *UpdateService) getFromCache(ctx context.Context) (*UpdateInfo, error) {
	...
	// 缓存可能是旧版本写入或被外部改写的，读取时重新校验来源，
	// 避免绕过 fetch 路径的仓库校验直接下载到非本仓库的产物。
