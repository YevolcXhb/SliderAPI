...
const DefaultCSPPolicy = "default-src 'self'; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://static.cloudflareinsights.com https://*.stripe.com https://*.airwallex.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https:; frame-src https://challenges.cloudflare.com https://*.stripe.com https://*.airwallex.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
func (d *DatabaseConfig) DSN() string {
	// MySQL/MariaDB DSN. The session time_zone is pinned to UTC so NOW() and
	// timestamp arithmetic behave consistently with the previous PostgreSQL
	// timestamptz (UTC) storage; multiStatements is required by the migration
	// runner which executes each migration file as a multi-statement batch.
	return fmt.Sprintf(
		"%s:%s@tcp(%s:%d)/%s?charset=utf8mb4&parseTime=true&loc=UTC&multiStatements=true&time_zone=%%27%%2B00%%3A00%%27",
...
func (d *DatabaseConfig) DSNWithTimezone(tz string) string {
	// The application always stores UTC datetimes. The app-level timezone is
	// applied at query time through CONVERT_TZ / application formatting, so the
	// session time_zone stays UTC regardless of tz.
	return d.DSN()
}
...
func setDefaults() {
	...
	viper.SetDefault("database.user", "ikik_api")
