...
// 兼容 PG 与 MySQL/MariaDB：通过 Dialect 字段选择方言；DSN() 与 DialectConfig() 同步返回对应驱动格式。
type DatabaseConfig struct {
	// Dialect 数据库方言：postgres | mysql（同时兼容 MySQL 8.0+ / MariaDB 10.5+/10.11+）。
	// 默认 postgres；可通过 DATABASE_DIALECT 环境变量覆盖。
	...
	// SSLMode 仅在 postgres 方言下生效。
	SSLMode string `mapstructure:"sslmode"`
	// Charset 仅在 mysql 方言下生效，默认 utf8mb4。
	Charset string `mapstructure:"charset"`
	// ParseTime 仅在 mysql 方言下生效，自动把 DATE/DATETIME 转为 time.Time。
	ParseTime bool `mapstructure:"parse_time"`
	// Loc 仅在 mysql 方言下生效，time.Location 字符串，默认 UTC。
...
// GetDialect 返回归一化后的方言枚举。
// 全量迁移后默认方言为 mysql（MySQL 8.0+ / MariaDB 10.5+/10.11+）；
// 空值/未识别值回退到 mysql；显式配置 postgres/postgresql 仍可识别（保留兼容分支）。
func (d *DatabaseConfig) GetDialect() dbdialect.Dialect {
	// MariaDB 10.11 is the only supported runtime database. The legacy
	// field remains decodable so old config files can be upgraded safely.
	return dbdialect.DialectMySQL
}

// DSNConfig 把当前 DatabaseConfig 转成 dbdialect.Config，供底层 driver 使用。
func (d *DatabaseConfig) DSNConfig() dbdialect.Config {
	return dbdialect.Config{
		...
		SSLMode:         d.SSLMode,
		Charset:         d.Charset,
		ParseTime:       d.ParseTime || d.GetDialect() == dbdialect.DialectMySQL,
		Loc:             d.Loc,
		MaxOpenConns:    d.MaxOpenConns,
		MaxIdleConns:    d.MaxIdleConns,
		ConnMaxLifetime: time.Duration(d.ConnMaxLifetimeMinutes) * time.Minute,
		ConnMaxIdleTime: time.Duration(d.ConnMaxIdleTimeMinutes) * time.Minute,
	}
}

func (d *DatabaseConfig) DSN() string {
	return d.DSNConfig().DSN()
}

// DSNWithTimezone 返回带时区的 DSN。MySQL/MariaDB 下时区通过 loc 参数控制；PostgreSQL 下追加 TimeZone 子句。
...
func setDefaults() {
	...
	// Database
	// 方言默认 mysql（本 fork 已全量迁移到 MySQL/MariaDB 10.11.14）。
	// 设为 postgres 可回退到 PostgreSQL（driver/DDL/占位符自动切换）。
	viper.SetDefault("database.dialect", "mysql")
	viper.SetDefault("database.host", "localhost")
	viper.SetDefault("database.port", 3306)
	viper.SetDefault("database.user", "sub2api")
	viper.SetDefault("database.password", "sub2api")
...
const OpenAIImagesResponsesReasoningEffortLow = "low"

const OpenAIImagesResponsesReasoningEffortMedium = "medium"

const OpenAIImagesResponsesReasoningEffortHigh = "high"

const OpenAIImagesResponsesReasoningEffortXHigh = "xhigh"

const OpenAIImagesResponsesReasoningEffortDefault = OpenAIImagesResponsesReasoningEffortMedium
