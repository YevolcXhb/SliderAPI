...
func TestConfigAddressHelpers(t *testing.T) {
	...
	dbCfg.Password = ""
	if strings.Contains(dbCfg.DSNWithTimezone("UTC"), ":secret@tcp(") {
		t.Fatalf("DatabaseConfig.DSNWithTimezone() should omit password when empty")
	}

	if !strings.Contains(dbCfg.DSNWithTimezone(""), "loc=UTC") {
		t.Fatalf("DatabaseConfig.DSNWithTimezone() should keep session timezone UTC")
	}
