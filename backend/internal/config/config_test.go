...
func TestLoadDefaultDatabaseSSLMode(t *testing.T) {
	...
	// 本 fork 默认方言为 mysql（MariaDB 10.11.14），sslmode 不适用，默认空串。
	if cfg.Database.Dialect != "mysql" {
		t.Fatalf("Database.Dialect = %q, want %q", cfg.Database.Dialect, "mysql")
	}
	if cfg.Database.SSLMode != "" {
		t.Fatalf("Database.SSLMode = %q, want empty (mysql default)", cfg.Database.SSLMode)
	}
}
