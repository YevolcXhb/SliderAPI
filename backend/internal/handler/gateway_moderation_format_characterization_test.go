...
func TestP2Characterization_OpenAIResponsesWSTurn2ModerationCloseError(t *testing.T) {
	...
	accountRepo := &p2CharWSAccountRepoStub{account: account}
	usageRepo := &p2CharWSUsageLogRepoStub{created: make(chan *service.UsageLog, 4)}
...
// p2CharWSAccountRepoStub 仅实现 WS 调度路径需要的最小账号查询。
// 其余方法经内嵌 nil AccountRepository 接口在意外调用时 panic（特征化测试纪律：
// 不掩饰未预期的 service 调用，直接 fail）。
type p2CharWSAccountRepoStub struct {
	service.AccountRepository
	account service.Account
}

func (r *p2CharWSAccountRepoStub) GetByID(_ context.Context, _ int64) (*service.Account, error) {
	return &r.account, nil
}

func (r *p2CharWSAccountRepoStub) ListSchedulableByPlatform(_ context.Context, platform string) ([]service.Account, error) {
...
func (r *p2CharWSAccountRepoStub) ListSchedulableByGroupIDAndPlatform(_ context.Context, _ int64, platform string) ([]service.Account, error) {
	if r.account.Platform != "" && r.account.Platform != platform {
		return nil, nil
	}
	return []service.Account{r.account}, nil
...
// p2CharWSUsageLogRepoStub 仅实现 Create 将日志发入 channel 供测试
// 断言；其余方法经内嵌 nil UsageLogRepository 接口 panic。
type p2CharWSUsageLogRepoStub struct {
	service.UsageLogRepository
	created chan *service.UsageLog
}

func (r *p2CharWSUsageLogRepoStub) Create(_ context.Context, log *service.UsageLog) (bool, error) {
	r.created <- log
	return true, nil
}
