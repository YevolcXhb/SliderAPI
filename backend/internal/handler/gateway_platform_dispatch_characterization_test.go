...
func p3CharAntigravityAccount(id, groupID int64, accountType string, creds, extra map[string]any) *service.Account {
	credentials := map[string]any{
		...
		"project_id":   fmt.Sprintf("project-%d", id),
	}
	for k, v := range creds {
		credentials[k] = v
	}
...
func p3CharNewHandler(t *testing.T, group *service.Group, accounts []*service.Account, settingValues map[string]string) *p3CharHandlerEnv {
	...
	gwSvc := service.NewGatewayService(
		...
		settingSvc,
		nil, nil, nil, nil, nil, // tlsFPProfileService / channelService / resolver / compositeResolver / balanceNotifyService
	...
	h := NewGatewayHandler(
		...
		concurrencySvc,
		billingCacheSvc,
		nil, nil, nil, nil, nil, nil, // usageService / apiKeyService / workerPool / errorPassthrough / contentModeration / userMsgQueue
		nil, // cfg
...
// Grok 视频计费与 reasoning 缓存不参与平台分发特征化测试，仅补齐 service.GatewayCache 接口。
func (c *p3CharStickyCache) SetGrokVideoPendingBilling(context.Context, string, []byte, time.Duration) error {
	return nil
}

func (c *p3CharStickyCache) GetGrokVideoPendingBilling(context.Context, string) ([]byte, error) {
	return nil, nil
}

func (c *p3CharStickyCache) ClaimGrokVideoBilled(context.Context, string, time.Duration) (bool, error) {
	return true, nil
}

func (c *p3CharStickyCache) ReleaseGrokVideoBilled(context.Context, string) error {
	return nil
}

func (c *p3CharStickyCache) SetReasoningContent(context.Context, string, string, time.Duration) error {
	return nil
}

func (c *p3CharStickyCache) GetReasoningContent(context.Context, string) (string, error) {
