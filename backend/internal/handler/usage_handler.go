...
// DashboardAccountSharing 返回当前用户「共享账号」维度的用量看板。
...
// parseUserDashboardTimeRangeStrict 解析看板时间范围，缺省最近 7 天；
// 与 parseUserUsageFilters 不同，这里对非法范围直接报错而不静默兜底。
...
// parseAccountSharingPagination 解析共享账号列表分页；上限 1000 行，
// 超限直接报错以避免一次性拉取过大结果集。
