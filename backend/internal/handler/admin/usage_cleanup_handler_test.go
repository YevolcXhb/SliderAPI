...
// 自动保留期归档路径不在 handler 用例范围内，仅补齐 service.UsageCleanupRepository 接口。
func (s *cleanupRepoStub) FindOldestUsageLogBefore(context.Context, time.Time) (*time.Time, error) {
	return nil, nil
}

func (s *cleanupRepoStub) SnapshotUsageLogs(context.Context, service.UsageCleanupFilters) error {
	return nil
}

func (s *cleanupRepoStub) ExportUsageLogs(context.Context, service.UsageCleanupFilters) (io.ReadCloser, error) {
