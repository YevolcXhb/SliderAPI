...
// markBatchRefreshAccountError 在批量刷新遇到上游 401 时把错误写回账号，
// 让前端能直接看到"凭证失效"而不是仅在本次响应里一闪而过。
// 非 401（参数错误、网络超时等）不落库，避免可恢复故障污染账号状态。
...
// batchRefreshErrorHTTPStatus 提取错误对应的 HTTP 状态：优先读取 infraerrors 携带的
// 状态码；上游驱动只在文本里带 401 时退化为字符串匹配。无法判定时返回 0。
func batchRefreshErrorHTTPStatus(err error) int {
	if err == nil {
		return 0
	}
	if status := infraerrors.Code(err); status != infraerrors.UnknownCode {
		return status
	}
...
// ModelProbeTestRequest is the body for POST /api/v1/admin/accounts/model-probe/test.
...
// ProbeModelList probes the upstream model list for an API key.
