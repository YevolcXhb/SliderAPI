...
func (r *accountBatchTaskRepository) ClaimNextPendingTask(ctx context.Context, staleRunningAfterSeconds int64) (*service.AccountBatchTask, error) {
	...
	// Use a transaction: claim the candidate row with FOR UPDATE, reset its
	// running items, mark the task running, then re-select the full task.
	...
	`, service.AccountBatchTaskStatusPending, nextID, service.AccountBatchTaskStatusRunning); err != nil {
		return nil, err
	}

	if _, err := tx.ExecContext(ctx, `
		UPDATE account_batch_tasks
		SET status = ?, started_at = COALESCE(started_at, NOW()), finished_at = NULL, error_message = NULL, updated_at = NOW()
		WHERE id = ?
	`, service.AccountBatchTaskStatusRunning, nextID); err != nil {
		return nil, err
	}
...
func (r *accountBatchTaskRepository) RefreshTaskProgress(ctx context.Context, taskID int64) (*service.AccountBatchTask, error) {
	...
	// MariaDB: no CTE + UPDATE..RETURNING. Compute counts then update in the
	// same transaction.
	...
	if err := row.Scan(&processed, &success, &failed); err != nil {
		return nil, err
	}
