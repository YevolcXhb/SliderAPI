...
` + where
	...
	duration.P99 = opsFloatToIntPtr(opsPercentileValue(durations, 0.99))
	if dCount > 0 {
		avg := dSum / float64(dCount)
		duration.Avg = opsFloatToIntPtr(&avg)
		v := int(dMax)
		duration.Max = &v
	}
	...
	ttft.P99 = opsFloatToIntPtr(opsPercentileValue(ttfts, 0.99))
	if tCount > 0 {
		avg := tSum / float64(tCount)
		ttft.Avg = opsFloatToIntPtr(&avg)
		v := int(tMax)
		ttft.Max = &v
	}
...
WITH usage_buckets AS (
  SELECT
    FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ul.created_at) / 60) * 60) AS bucket,
    COUNT(*) AS req_cnt,
...
error_buckets AS (
  SELECT FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(created_at) / 60) * 60) AS bucket, COUNT(*) AS err_cnt
  FROM ops_error_logs
  ` + errorWhere + `
...
combined AS (
  ...
  UNION ALL
  SELECT e.bucket,
         COALESCE(u.req_cnt, 0) + COALESCE(e.err_cnt, 0),
         COALESCE(u.token_cnt, 0)
  FROM error_buckets e
  LEFT JOIN usage_buckets u ON e.bucket = u.bucket
