...
func (r *opsRepository) queryUsageLatency(ctx context.Context, filter *service.OpsDashboardFilter, start, end time.Time) (duration service.OpsPercentiles, ttft service.OpsPercentiles, err error) {
	...
	q := `
SELECT
  PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY duration_ms) OVER () AS duration_p50,
  PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY duration_ms) OVER () AS duration_p90,
  PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY duration_ms) OVER () AS duration_p95,
  PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY duration_ms) OVER () AS duration_p99,
  AVG(CASE WHEN duration_ms IS NOT NULL THEN duration_ms END) OVER () AS duration_avg,
  MAX(duration_ms) OVER () AS duration_max,
  PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY first_token_ms) OVER () AS ttft_p50,
  PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY first_token_ms) OVER () AS ttft_p90,
  PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY first_token_ms) OVER () AS ttft_p95,
  PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY first_token_ms) OVER () AS ttft_p99,
  AVG(CASE WHEN first_token_ms IS NOT NULL THEN first_token_ms END) OVER () AS ttft_avg,
  MAX(first_token_ms) OVER () AS ttft_max
FROM usage_logs ul
` + join + `
` + where
...
func (r *opsRepository) queryErrorCounts(ctx context.Context, filter *service.OpsDashboardFilter, start, end time.Time) (
	...
	q := `
SELECT
  COALESCE(SUM(CASE WHEN COALESCE(status_code, 0) THEN 1 ELSE 0 END >= 400), 0) AS error_total,
  COALESCE(SUM(CASE WHEN COALESCE(status_code, 0) THEN 1 ELSE 0 END >= 400 AND is_business_limited), 0) AS business_limited,
  COALESCE(SUM(CASE WHEN COALESCE(status_code, 0) THEN 1 ELSE 0 END >= 400 AND NOT is_business_limited), 0) AS error_sla,
  COALESCE(SUM(CASE WHEN error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) THEN 1 ELSE 0 END NOT IN (429, 529)), 0) AS upstream_excl,
  COALESCE(SUM(CASE WHEN error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) THEN 1 ELSE 0 END = 429), 0) AS upstream_429,
  COALESCE(SUM(CASE WHEN error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) THEN 1 ELSE 0 END = 529), 0) AS upstream_529
FROM ops_error_logs
` + where
...
WITH usage_buckets AS (
  SELECT
    DATE_FORMAT(ul.created_at, '%Y-%m-%d %H:%i:00') AS bucket,
    COUNT(*) AS req_cnt,
...
error_buckets AS (
  SELECT DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:00') AS bucket, COUNT(*) AS err_cnt
  FROM ops_error_logs
  ` + errorWhere + `
