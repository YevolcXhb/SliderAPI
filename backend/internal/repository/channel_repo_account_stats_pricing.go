...
func (r *channelRepository) batchLoadAccountStatsPricingRules(ctx context.Context, channelIDs []int64) (map[int64][]service.AccountStatsPricingRule, error) {
	// 1. 查询规则
	query := fmt.Sprintf(`SELECT id, channel_id, name, group_ids, account_ids, sort_order, created_at, updated_at
		 FROM channel_account_stats_pricing_rules WHERE channel_id IN (%s) ORDER BY channel_id, sort_order, id`, sqlPlaceholders(len(channelIDs)))
	...
	for rows.Next() {
		...
		if len(groupIDsJSON) > 0 {
			_ = json.Unmarshal(groupIDsJSON, &rule.GroupIDs)
		}
		if len(accountIDsJSON) > 0 {
			_ = json.Unmarshal(accountIDsJSON, &rule.AccountIDs)
		}
...
func (r *channelRepository) batchLoadAccountStatsModelPricing(ctx context.Context, ruleIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	...
	query := fmt.Sprintf(`SELECT id, rule_id, platform, models, billing_mode, input_price, output_price,
	        cache_write_price, cache_read_price, image_output_price, per_request_price, created_at, updated_at
	 FROM channel_account_stats_model_pricing WHERE rule_id IN (%s) ORDER BY rule_id, id`, sqlPlaceholders(len(ruleIDs)))
...
func createAccountStatsPricingRuleTx(ctx context.Context, tx *sql.Tx, rule *service.AccountStatsPricingRule) error {
	// MariaDB 无 bigint[] 列：group_ids/account_ids 存为 JSON 数组字符串。
	groupIDsJSON, err := json.Marshal(rule.GroupIDs)
	if err != nil {
		return fmt.Errorf("marshal group ids: %w", err)
	}
	accountIDsJSON, err := json.Marshal(rule.AccountIDs)
	if err != nil {
	...
	res, err := tx.ExecContext(ctx,
		`INSERT INTO channel_account_stats_pricing_rules (channel_id, name, group_ids, account_ids, sort_order)
		 VALUES (?, ?, ?, ?, ?)`,
		rule.ChannelID, rule.Name, string(groupIDsJSON), string(accountIDsJSON), rule.SortOrder,
	)
	if err != nil {
		return fmt.Errorf("insert account stats pricing rule: %w", err)
	}
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT created_at, updated_at FROM channel_account_stats_pricing_rules WHERE id = ?`, rule.ID,
	).Scan(&rule.CreatedAt, &rule.UpdatedAt); err != nil {
		return fmt.Errorf("load account stats pricing rule timestamps: %w", err)
	}
...
func createAccountStatsModelPricingTx(ctx context.Context, tx *sql.Tx, ruleID int64, pricing *service.ChannelModelPricing) error {
	...
	// MariaDB 不支持 INSERT ... RETURNING，改用 LAST_INSERT_ID + 回查。
	res, err := tx.ExecContext(ctx,
	...
	pricing.ID, err = res.LastInsertId()
	if err != nil {
		return fmt.Errorf("insert account stats model pricing: %w", err)
	}
	if err = tx.QueryRowContext(ctx,
		`SELECT created_at, updated_at FROM channel_account_stats_model_pricing WHERE id = ?`, pricing.ID,
	).Scan(&pricing.CreatedAt, &pricing.UpdatedAt); err != nil {
		return fmt.Errorf("load account stats model pricing timestamps: %w", err)
	}
...
func createAccountStatsIntervalTx(ctx context.Context, tx *sql.Tx, iv *service.PricingInterval) error {
	...
	if err != nil {
		return fmt.Errorf("insert account stats pricing interval: %w", err)
	}
	iv.ID, err = res.LastInsertId()
	if err != nil {
		return fmt.Errorf("insert account stats pricing interval: %w", err)
	}
	return tx.QueryRowContext(ctx,
		`SELECT created_at, updated_at FROM channel_account_stats_pricing_intervals WHERE id = ?`, iv.ID,
...
func (r *channelRepository) batchLoadAccountStatsIntervals(ctx context.Context, pricingIDs []int64) (map[int64][]service.PricingInterval, error) {
	...
	query := fmt.Sprintf(`SELECT id, pricing_id, min_tokens, max_tokens, tier_label,
	        input_price, output_price, cache_write_price, cache_read_price,
	        per_request_price, sort_order, created_at, updated_at
	 FROM channel_account_stats_pricing_intervals
	 WHERE pricing_id IN (%s) ORDER BY pricing_id, sort_order, id`, sqlPlaceholders(len(pricingIDs)))
	args := make([]any, len(pricingIDs))
	for i, id := range pricingIDs {
		args[i] = id
	}
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
