...
func (r *opsRepository) InsertErrorLog(ctx context.Context, input *service.OpsInsertErrorLogInput) (int64, error) {
	...
	var id int64
	res, err := r.db.ExecContext(ctx, insertOpsErrorLogSQL, opsInsertErrorLogArgs(input)...)
	if err != nil {
		return 0, err
	}
...
SELECT
  ...
  e.group_id,
  COALESCE(g.name, ''),
  CASE WHEN e.client_ip IS NULL THEN NULL ELSE SUBSTRING_INDEX(e.client_ip, '/', 1) END,
...
func (r *opsRepository) BatchInsertSystemLogs(ctx context.Context, inputs []*service.OpsInsertSystemLogInput) (int64, error) {
	...
	// MariaDB 无 PG 的 COPY（pq.CopyIn），改为批量多行 INSERT。
	const cols = 13
	var inserted int64
	const chunkSize = 200
	for start := 0; start < len(inputs); start += chunkSize {
		...
		rowsPlaceholders := make([]string, 0, len(chunk))
		...
		for _, input := range chunk {
			...
			rowsPlaceholders = append(rowsPlaceholders, "("+strings.TrimSuffix(strings.Repeat("?, ", len(vals)), ", ")+")")
			args = append(args, vals...)
		}
		if len(rowsPlaceholders) == 0 {
			continue
		}
		...
		inserted += int64(len(rowsPlaceholders))
	}
...
// escapeLikePattern 转义 LIKE/LIKE 通配符（\ % _），避免用户输入被当作通配符。
...
func buildOpsErrorLogsWhere(filter *service.OpsErrorLogFilter) (string, []any) {
	...
	if len(filter.StatusCodes) > 0 {
		// MariaDB 兼容:status_code = ANY($n) → IN ($n, $n+1, ...),参数逐个展开。
		startIdx := len(args) + 1
		args = append(args, toAnySlice(filter.StatusCodes)...)
		clauses = append(clauses, "COALESCE(e.upstream_status_code, e.status_code, 0) IN ("+sqlInPlaceholders(startIdx, len(filter.StatusCodes))+")")
		...
		// MariaDB 兼容:NOT (status_code = ANY($n)) → NOT (status_code IN ($n, $n+1, ...)),参数逐个展开。
		startIdx := len(args) + 1
		args = append(args, toAnySlice(known)...)
		clauses = append(clauses, "NOT (COALESCE(e.upstream_status_code, e.status_code, 0) IN ("+sqlInPlaceholders(startIdx, len(known))+"))")
	}
	...
	if q := strings.TrimSpace(filter.Query); q != "" {
		...
		clauses = append(clauses, "(e.request_id LIKE ? OR e.client_request_id LIKE ? OR e.error_message LIKE ?)")
	}

	if userQuery := strings.TrimSpace(filter.UserQuery); userQuery != "" {
		like := "%" + userQuery + "%"
		args = append(args, like)
		clauses = append(clauses, "EXISTS (SELECT 1 FROM users u WHERE u.id = e.user_id AND u.email LIKE ?)")
	}
	...
	if m := strings.TrimSpace(filter.Model); m != "" {
		if filter.ModelFuzzy {
			...
			clauses = append(clauses, "COALESCE(e.requested_model, e.model, '') LIKE ?")
		} else {
			args = append(args, m)
	...
	if len(filter.ErrorPhasesAny) > 0 {
		// MariaDB 兼容:error_phase = ANY($n) → IN ($n, $n+1, ...),参数逐个展开。
		startIdx := len(args) + 1
		args = append(args, toAnySlice(filter.ErrorPhasesAny)...)
		clauses = append(clauses, "e.error_phase IN ("+sqlInPlaceholders(startIdx, len(filter.ErrorPhasesAny))+")")
	}
	if len(filter.ErrorTypesAny) > 0 {
		// MariaDB 兼容:error_type = ANY($n) → IN ($n, $n+1, ...),参数逐个展开。
		startIdx := len(args) + 1
		args = append(args, toAnySlice(filter.ErrorTypesAny)...)
		clauses = append(clauses, "e.error_type IN ("+sqlInPlaceholders(startIdx, len(filter.ErrorTypesAny))+")")
	}
...
func buildOpsSystemLogsWhere(filter *service.OpsSystemLogFilter) (string, []any, bool) {
	...
	if filter != nil {
		...
		if v := strings.TrimSpace(filter.Query); v != "" {
			like := "%" + v + "%"
			args = append(args, like)
			clauses = append(clauses, "(l.message LIKE ? OR COALESCE(l.request_id,'') LIKE ? OR COALESCE(l.client_request_id,'') LIKE ? OR COALESCE(l.extra,'') LIKE ?)")
			hasConstraint = true
		}
	}
