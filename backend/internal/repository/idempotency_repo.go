...
func (r *idempotencyRepository) CreateProcessing(ctx context.Context, record *service.IdempotencyRecord) (bool, error) {
	...
	err := withExecutorTx(ctx, r.sql, func(exec sqlExecutor) error {
		...
		`, record.Scope, record.IdempotencyKeyHash, record.RequestFingerprint, record.Status,
			record.LockedUntil, record.ExpiresAt); err != nil {
			return err
		}
		return scanSingleRow(ctx, exec, `
			...
			LIMIT 1
		`, []any{record.Scope, record.IdempotencyKeyHash}, &record.ID, &createdAt, &updatedAt)
	})
