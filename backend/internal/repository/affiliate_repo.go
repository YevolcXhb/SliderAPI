...
func (r *affiliateRepository) GetCurrentInviteSharePercent(ctx context.Context) (float64, error) {
	...
	rows, err := client.QueryContext(ctx, `
		SELECT CAST(invite_share_ratio AS DOUBLE)* 100
...
func (r *affiliateRepository) BindInviter(ctx context.Context, userID, inviterID int64) (bool, error) {
	...
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		var rebateDays int64
		err := scanSingleRow(txCtx, txClient,
			`SELECT COALESCE((
				SELECT CASE
					WHEN (value REGEXP '^[0-9]+$') THEN LEAST(CAST(value AS SIGNED), ?)
					ELSE 0
				END
				FROM settings
				WHERE `+"`key`"+` = ?
			...
			[]any{service.AffiliateRebateDurationDaysMax, service.SettingKeyAffiliateRebateDurationDays},
			&rebateDays)
		if err != nil {
			return fmt.Errorf("bind inviter: read rebate duration: %w", err)
		}
		if rebateDays < 0 {
			rebateDays = 0
		}
		res, err := txClient.ExecContext(txCtx,
			...
			SET inviter_id = ?,
				...
				invite_reward_expires_at = CASE
					...
					WHEN ? > 0 THEN NOW() + INTERVAL ? DAY
					ELSE NULL
				END,
				updated_at = NOW()
			WHERE user_id = ?
				AND inviter_id IS NULL`,
			inviterID, rebateDays, rebateDays, userID,
		)
		if err != nil {
...
func (r *affiliateRepository) AdminBindInviter(ctx context.Context, userID, inviterID int64, resetValidity bool) (*service.AffiliateSummary, error) {
	...
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		err = scanSingleRow(txCtx, txClient,
			...
			&rebateDays)
		if err != nil {
			return fmt.Errorf("admin bind inviter: read rebate duration: %w", err)
		}
		if rebateDays < 0 {
			rebateDays = 0
		}
...
SET inviter_id = ?,
    inviter_bound_at = CASE
        WHEN ? OR inviter_bound_at IS NULL THEN NOW()
        ELSE inviter_bound_at
    END,
    invite_bind_source = 'admin',
    invite_reward_expires_at = CASE
        WHEN ? OR inviter_bound_at IS NULL THEN
            CASE WHEN ? > 0 THEN NOW() + INTERVAL ? DAY ELSE NULL END
        ELSE invite_reward_expires_at
    END,
    updated_at = NOW()
WHERE user_id = ?`,
			inviterID, resetValidity, resetValidity, rebateDays, rebateDays, userID,
		)
		if err != nil {
...
SET aff_count = GREATEST(aff_count - 1, 0),
    updated_at = NOW()
WHERE user_id = ?`, oldInviterID); err != nil {
...
SET aff_count = aff_count + 1,
    updated_at = NOW()
WHERE user_id = ?`, inviterID); err != nil {
...
UPDATE user_affiliates
SET invite_reward_expires_at = invite_reward_expires_at + INTERVAL ? DAY,
    updated_at = NOW()
...
WHERE inviter_id = ?
  ...
  AND invite_reward_expires_at IS NOT NULL
  AND invite_reward_expires_at > NOW()`, req.ExtendDays, req.InviterUserID, Int64CSV(req.InviteeUserIDs))
		}
	default:
...
VALUES (?, 'accrue', ?, ?, ?, NOW() + INTERVAL ? HOUR, NOW(), NOW())`,
...
WHERE user_id = ?
  ...
  AND frozen_until <= NOW()`, []any{userID}, &thawed)
	if err != nil {
...
SELECT ua.user_id,
       ...
       COALESCE(u.status, ''),
       COALESCE(SUM(CASE WHEN ase.status = 'applied' AND ase.inviter_user_id = ? THEN ase.consumer_charge ELSE 0 END), 0) AS history_consumption,
       COALESCE(SUM(CASE WHEN ase.status = 'applied' AND ase.inviter_user_id = ? THEN ase.invite_credit ELSE 0 END), 0) AS total_rebate,
       COALESCE(SUM(CASE WHEN ase.status = 'applied' AND ase.inviter_user_id = ? AND ase.created_at >= ? AND ase.created_at < ? THEN ase.consumer_charge ELSE 0 END), 0) AS period_consumption,
       COALESCE(SUM(CASE WHEN ase.status = 'applied' AND ase.inviter_user_id = ? AND ase.created_at >= ? AND ase.created_at < ? THEN ase.invite_credit ELSE 0 END), 0) AS period_rebate
...
LIMIT ?`, inviterID, inviterID, inviterID, nullableTimeArg(query.PeriodStart), nullableTimeArg(query.PeriodEnd), inviterID, nullableTimeArg(query.PeriodStart), nullableTimeArg(query.PeriodEnd), inviterID, limit)
	if err != nil {
		return nil, 0, err
	}
	defer func() { _ = rows.Close() }()
...
func queryAffiliatePeriodRebate(ctx context.Context, client affiliateQueryExecer, inviterID int64, query service.AffiliateDetailQuery) (float64, error) {
	rows, err := client.QueryContext(ctx, `
SELECT COALESCE(SUM(invite_credit), 0)
...
INSERT IGNORE INTO user_affiliates (user_id, aff_code, created_at, updated_at)
...
SELECT user_id,
       ...
       aff_count,
       CAST(aff_quota AS DOUBLE),
       CAST(aff_frozen_quota AS DOUBLE),
       CAST(aff_history_quota AS DOUBLE),
       created_at,
       updated_at
FROM user_affiliates
...
SET aff_rebate_rate_percent = ?,
    updated_at = NOW()
WHERE FIND_IN_SET(user_id, ?)`, nullableArg(ratePercent), Int64CSV(userIDs))
		if err != nil {
