...
func thawFrozenQuotaTx(txCtx context.Context, txClient *dbent.Client, userID int64) (float64, error) {
	// MariaDB has no data-modifying CTE: select matured rows FOR UPDATE, then
	// clear frozen_until, then move the sum to the affiliate balance.
...
func (r *affiliateRepository) ListAffiliateInviteRecords(ctx context.Context, filter service.AffiliateRecordFilter) ([]service.AffiliateInviteRecord, int64, error) {
	...
	where, args := buildAffiliateRecordWhere(filter, "ua.created_at", []string{
		...
		"ua.inviter_id", "ua.user_id", "inviter_aff.aff_code",
	})
...
func (r *affiliateRepository) ListAffiliateRebateRecords(ctx context.Context, filter service.AffiliateRecordFilter) ([]service.AffiliateRebateRecord, int64, error) {
	...
	where, args := buildAffiliateRecordWhere(filter, "ual.created_at", []string{
		"inviter.email", "inviter.username", "invitee.email", "invitee.username",
		"po.id", "po.out_trade_no", "po.payment_type", "po.status",
	})
	baseJoin := `
...
func (r *affiliateRepository) BatchSetUserRebateRate(ctx context.Context, userIDs []int64, ratePercent *float64) error {
	...
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		args := []any{nullableArg(ratePercent)}
...
WHERE inviter_id = ?
  ...
  AND invite_reward_expires_at > NOW()`, req.ExtendDays, req.InviterUserID)
			...
			placeholders := make([]string, 0, len(req.InviteeUserIDs))
			args := make([]any, 0, 2+len(req.InviteeUserIDs))
			args = append(args, req.ExtendDays, req.InviterUserID)
			for _, id := range req.InviteeUserIDs {
...
WHERE inviter_id = ?
  ...
  AND invite_reward_expires_at IS NOT NULL
  AND invite_reward_expires_at > NOW()`, args...)
		}
	default:
...
// AdminBindInviter binds an admin-specified inviter for a user.
...
SET inviter_id = ?,
    ...
    invite_reward_expires_at = CASE
        WHEN ? OR inviter_bound_at IS NULL THEN
            CASE
                WHEN duration.days > 0 THEN NOW() + INTERVAL duration.days DAY
                ELSE NULL
            END
...
WHERE user_affiliates.user_id = ?`,
			inviterID, resetArg, resetArg, service.AffiliateRebateDurationDaysMax, service.SettingKeyAffiliateRebateDurationDays, userID,
		)
		if err != nil {
