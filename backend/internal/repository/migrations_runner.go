...
CREATE TABLE IF NOT EXISTS atlas_schema_revisions (
	...
	executed_at DATETIME(6) NOT NULL DEFAULT NOW(),
...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	"115_auth_identity_legacy_external_backfill.sql":          newMigrationChecksumCompatibilityRule("022ba6d1b6e71992f4565c9ce0f83229027bf3780f2d87b7327a69076fd052fb", "022aadd97bb53e755f0cf7a3a957e0cb1a1353b0c39ec4de3234acd2871fd04f", "4cf39e508be9fd1a5aa41610cbbebeb80385c9adda45bf78a706de9db4f1385f"),
	"116_auth_identity_legacy_external_safety_reports.sql":    newMigrationChecksumCompatibilityRule("126d31323840595471ebc3e941dd0a861b7c7f733e6bf86c180a243734fafcb7", "3178b88dd74a65d8c7a4f36d54b80c1b9318f3bce08a7b4651266b54d57c15e0", "b05146906c76c0c47d3385652b5341af7689aa6cd5ace72eae13ead83f2181ec", "07edb09fa8d04ffb172b0621e3c22f4d1757d20a24ae267b3b36b087ab72d488", "f7757bd929ac67ffb08ce69fa4cf20fad39dbff9d5a5085fb2adabb7607e5877"),
...
func preparePaymentOrdersOutTradeNoUniqueMigration(ctx context.Context, db *sql.DB) error {
	...
	if _, err := db.ExecContext(ctx, fmt.Sprintf("ALTER TABLE payment_orders DROP INDEX IF EXISTS %s", paymentOrdersOutTradeNoUniqueIndex)); err != nil {
...
func prepareOwnedAccountIdentityUniqueMigration(ctx context.Context, db *sql.DB) error {
	...
	for _, indexName := range ownedAccountIdentityUniqueIndexes {
		...
		if _, err := db.ExecContext(ctx, fmt.Sprintf("ALTER TABLE accounts DROP INDEX IF EXISTS %s", indexName)); err != nil {
	...
	rows, err := db.QueryContext(ctx, `
		WITH identities AS (
			SELECT
				...
				NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.chatgpt_account_id'))), '') AS identity_value,
				id
			FROM accounts
			WHERE deleted_at IS NULL
			  ...
			  AND platform = 'openai'
			  AND type = 'oauth'
			  AND NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.chatgpt_account_id'))), '') IS NOT NULL
			...
			SELECT
				...
				NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.chatgpt_user_id'))), '') AS identity_value,
				id
			FROM accounts
			WHERE deleted_at IS NULL
			  AND owner_user_id IS NOT NULL
			  AND platform = 'openai'
			  AND type = 'oauth'
			  AND NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.chatgpt_user_id'))), '') IS NOT NULL
			...
			SELECT
				...
				LOWER(COALESCE(NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(extra, '$.org_uuid'))), ''), NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.org_uuid'))), ''))) ||
					'|' ||
					LOWER(COALESCE(NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(extra, '$.account_uuid'))), ''), NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.account_uuid'))), ''))) AS identity_value,
				id
			FROM accounts
			WHERE deleted_at IS NULL
			  AND owner_user_id IS NOT NULL
			  ...
			  AND type = 'oauth'
			  AND COALESCE(NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(extra, '$.org_uuid'))), ''), NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.org_uuid'))), '')) IS NOT NULL
			  AND COALESCE(NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(extra, '$.account_uuid'))), ''), NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.account_uuid'))), '')) IS NOT NULL
			...
			SELECT
				...
				LOWER(COALESCE(NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.oauth_type'))), ''), 'code_assist')) ||
					'|' ||
					LOWER(NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.project_id'))), '')) AS identity_value,
				id
			FROM accounts
			WHERE deleted_at IS NULL
			  AND owner_user_id IS NOT NULL
			  AND platform = 'gemini'
			  AND type = 'oauth'
			  AND NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.project_id'))), '') IS NOT NULL
...
func indexIsInvalid(ctx context.Context, db *sql.DB, indexName string) (bool, error) {
	// MariaDB indexes are never left in an invalid state the way PostgreSQL
	// concurrent index builds can be; an existing index is always usable.
	// The *_notx migrations themselves drop stale legacy indexes idempotently.
	var exists int
...
func validateMigrationExecutionMode(name, content string) (bool, error) {
	...
	if strings.Contains(strings.ToUpper(content), "BEGIN") || strings.Contains(strings.ToUpper(content), "COMMIT") || strings.Contains(strings.ToUpper(content), "ROLLBACK") {
	...
	for _, stmt := range statements {
		...
		isAlterTable := strings.Contains(normalizedStmt, "ALTER TABLE")
		if !isCreateIndex && !isDropIndex && !isAlterTable {
			return false, errors.New("*_notx.sql currently only supports CREATE/DROP INDEX and ALTER TABLE statements")
		}
...
func splitSQLStatements(content string) []string {
	...
	for i := 0; i < len(content); i++ {
		c := content[i]
		if c == '-' && i+1 < len(content) && content[i+1] == '-' && !inSingle {
...
func pgAdvisoryLock(ctx context.Context, db *sql.DB) error {
	...
	for {
		var locked bool
		if err := db.QueryRowContext(ctx, "SELECT GET_LOCK(CONCAT('ikik_api_migrations_', ?), 30)", migrationsAdvisoryLockID).Scan(&locked); err != nil {
...
func pgAdvisoryUnlock(ctx context.Context, db *sql.DB) error {
	_, err := db.ExecContext(ctx, "SELECT RELEASE_LOCK(CONCAT('ikik_api_migrations_', ?))", migrationsAdvisoryLockID)
	if err != nil {
