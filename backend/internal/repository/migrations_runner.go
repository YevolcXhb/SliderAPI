...
// schemaMigrationsTablePG 是 PostgreSQL 方言的迁移记录表 DDL。
const schemaMigrationsTablePG = `
...
// schemaMigrationsTableMySQL 是 MySQL/MariaDB 方言的迁移记录表 DDL。
const schemaMigrationsTableMySQL = `
CREATE TABLE IF NOT EXISTS schema_migrations (
...
// atlasSchemaRevisionsTablePG 是 PostgreSQL 方言的 atlas 记录表 DDL。
const atlasSchemaRevisionsTablePG = `
...
// atlasSchemaRevisionsTableMySQL 是 MySQL/MariaDB 方言的 atlas 记录表 DDL。
// 注：partial_hashes 在 MariaDB 改为 TEXT（数组在 MariaDB 不支持），
// 多个 hash 用 ',' 连接；该字段在应用层已不再读取。
const atlasSchemaRevisionsTableMySQL = `
CREATE TABLE IF NOT EXISTS atlas_schema_revisions (
...
// schemaMigrationsTableDDL 根据方言返回对应的 DDL。
func schemaMigrationsTableDDL(d dbdialect.Dialect) string {
	if d == dbdialect.DialectMySQL {
		return schemaMigrationsTableMySQL
	}
	return schemaMigrationsTablePG
}

// atlasSchemaRevisionsTableDDL 根据方言返回对应的 DDL。
func atlasSchemaRevisionsTableDDL(d dbdialect.Dialect) string {
	if d == dbdialect.DialectMySQL {
		return atlasSchemaRevisionsTableMySQL
	}
	return atlasSchemaRevisionsTablePG
}

// recordMigrationInsertSQL 返回记录迁移的 INSERT 语句，占位符按方言切换（PG: ?/?；MySQL: ?/?）。
func recordMigrationInsertSQL(d dbdialect.Dialect) string {
	if d == dbdialect.DialectMySQL {
		return "INSERT INTO schema_migrations (filename, checksum) VALUES (?, ?)"
	}
	return "INSERT INTO schema_migrations (filename, checksum) VALUES (?, ?)"
}

// selectMigrationChecksumSQL 返回按 filename 查已应用迁移 checksum 的语句，占位符按方言切换。
func selectMigrationChecksumSQL(d dbdialect.Dialect) string {
	if d == dbdialect.DialectMySQL {
		return "SELECT checksum FROM schema_migrations WHERE filename = ?"
	}
	return "SELECT checksum FROM schema_migrations WHERE filename = ?"
}
...
// - 使用方言特定的锁机制确保多实例并发安全（PostgreSQL: advisory lock；MySQL/MariaDB: GET_LOCK）
//
// 参数：
//   - ctx: 上下文，用于超时控制和取消
//   - db: 数据库连接
//   - dialect: 数据库方言，决定锁/记录表/默认时间戳语法
//
// 返回：
//   - error: 迁移过程中的任何错误
func ApplyMigrations(ctx context.Context, db *sql.DB, dialect dbdialect.Dialect) error {
	if db == nil {
...
//
// 迁移执行流程：
//  1. 获取方言特定的分布式锁（PostgreSQL: advisory lock；MySQL/MariaDB: GET_LOCK）
//  2. 确保 schema_migrations 表存在（按方言建表）
//  3. 按文件名排序读取所有 .sql 文件
//  4. 对于每个迁移文件：
//     - 计算文件内容的 SHA256 校验和
...
//   - dialect: 数据库方言
func applyMigrationsFS(ctx context.Context, db *sql.DB, fsys fs.FS, dialect dbdialect.Dialect) error {
	if db == nil {
		return errors.New("nil sql db")
	}

	// 获取方言特定的分布式锁，确保多实例部署时只有一个实例执行迁移。
	lockConn, err := db.Conn(ctx)
	if err != nil {
		return fmt.Errorf("acquire migrations lock connection: %w", err)
	}
	defer func() { _ = lockConn.Close() }()
	if err := acquireMigrationsLock(ctx, lockConn, dialect); err != nil {
		return err
	}
	defer func() {
		// 无论迁移是否成功，都要释放锁。
		...
		defer cancel()
		_ = releaseMigrationsLock(unlockCtx, lockConn, dialect)
	}()

	// 创建迁移记录表（如果不存在）。
	// 该表记录所有已应用的迁移及其校验和。
	if _, err := lockConn.ExecContext(ctx, schemaMigrationsTableDDL(dialect)); err != nil {
		return fmt.Errorf("create schema_migrations: %w", err)
	}

	// 自动对齐 Atlas 基线（如果检测到 legacy schema_migrations 且缺失 atlas_schema_revisions）。
	if err := ensureAtlasBaselineAligned(ctx, lockConn, fsys, dialect); err != nil {
		return err
	}
	...
	for _, name := range files {
		...
		// 检查该迁移是否已经应用
		var existing string
		rowErr := lockConn.QueryRowContext(ctx, selectMigrationChecksumSQL(dialect), name).Scan(&existing)
		if rowErr == nil {
			// 迁移已应用，验证校验和是否匹配
		...
		nonTx, err := validateMigrationExecutionMode(name, content, dialect)
		if err != nil {
			return fmt.Errorf("validate migration %s: %w", name, err)
		}

		if nonTx {
			if err := prepareNonTransactionalMigration(ctx, lockConn, name, dialect); err != nil {
			...
			if _, err := lockConn.ExecContext(ctx, recordMigrationInsertSQL(dialect), name, checksum); err != nil {
		...
		// 执行迁移 SQL。
		// PostgreSQL: lib/pq 支持单次 Exec 执行多语句，直接整文件执行。
		// MySQL/MariaDB: go-sql-driver 默认不支持多语句，且 CREATE TRIGGER 的 BEGIN...END 内含分号，
		// 需用「触发器感知」的分割器逐条执行（正确处理 BEGIN...END / $$ 复合体）。
		if dialect == dbdialect.DialectMySQL {
			for i, stmt := range splitSQLStatementsMySQL(content) {
				trimmed := strings.TrimSpace(stmt)
		...
		// 记录迁移已完成，保存文件名和校验和
		if _, err := tx.ExecContext(ctx, recordMigrationInsertSQL(dialect), name, checksum); err != nil {
			_ = tx.Rollback()
...
func prepareNonTransactionalMigration(ctx context.Context, db migrationConnection, name string, dialect dbdialect.Dialect) error {
	// MySQL/MariaDB 没有 PostgreSQL 的“失效索引”（CONCURRENTLY 建失败残留）概念，
	// 且我们的 *_notx.sql 已去掉 CONCURRENTLY；因此 MySQL 下无需预清理，直接放行。
	if dialect == dbdialect.DialectMySQL {
		if name == paymentOrdersOutTradeNoUniqueMigration {
			return preparePaymentOrdersOutTradeNoUniqueMigration(ctx, db, dialect)
		}
		return nil
	}
	switch name {
...
func preparePaymentOrdersOutTradeNoUniqueMigration(ctx context.Context, db migrationConnection, dialect dbdialect.Dialect) error {
	...
	// MySQL/MariaDB 无 PostgreSQL 失效索引概念，跳过失效索引清理。
	if dialect == dbdialect.DialectMySQL {
		return nil
	}
...
func indexIsInvalid(ctx context.Context, db migrationConnection, indexName string) (bool, error) {
	// MariaDB 10.11 does not have the pg_index.indisvalid concept; indexes are
	// either present (in information_schema.statistics) or absent. Treat any
	// reported index as valid; callers rely on this only to recover from a
	// half-applied PostgreSQL CONCURRENTLY index, which does not happen on
	// MariaDB.
	var exists int
	err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = DATABASE() AND index_name = ?`, indexName).Scan(&exists)
	return false, err
}

func ensureAtlasBaselineAligned(ctx context.Context, db migrationConnection, fsys fs.FS, dialect dbdialect.Dialect) error {
	hasLegacy, err := tableExistsForDialect(ctx, db, "schema_migrations", dialect)
	if err != nil {
	...
	hasAtlas, err := tableExistsForDialect(ctx, db, "atlas_schema_revisions", dialect)
	if err != nil {
		return fmt.Errorf("check atlas_schema_revisions: %w", err)
	}
	if !hasAtlas {
		if _, err := db.ExecContext(ctx, atlasSchemaRevisionsTableDDL(dialect)); err != nil {
	...
	`
	if dialect == dbdialect.DialectMySQL {
		insertBaseline = "INSERT INTO atlas_schema_revisions (version, description, type, applied, total, executed_at, execution_time, hash) VALUES (?, ?, ?, 0, 0, NOW(), 0, ?)"
	}
	if _, err := db.ExecContext(ctx, insertBaseline, version, description, 1, hash); err != nil {
...
func tableExists(ctx context.Context, db migrationConnection, tableName string) (bool, error) {
	return tableExistsForDialect(ctx, db, tableName, dbdialect.DetectFromEnv())
}

// tableExistsForDialect 按方言查表是否存在。
//   - PostgreSQL: information_schema.tables WHERE table_schema = 'public'（占位符 ?）
//   - MySQL/MariaDB: WHERE table_schema = DATABASE()（占位符 ?）
func tableExistsForDialect(ctx context.Context, db migrationConnection, tableName string, dialect dbdialect.Dialect) (bool, error) {
	var exists bool
	if dialect == dbdialect.DialectMySQL {
...
func validateMigrationExecutionMode(name, content string, dialect dbdialect.Dialect) (bool, error) {
	normalizedName := strings.ToLower(strings.TrimSpace(name))
	upperContent := strings.ToUpper(content)
	nonTx := strings.HasSuffix(normalizedName, nonTransactionalMigrationSuffix)

	// MySQL/MariaDB：DDL 隐式提交、无 CONCURRENTLY 概念。
	//   - *_notx.sql 仍按「非事务、逐条执行」处理（nonTx=true），但内容是普通 DDL，
	//     不要求 CONCURRENTLY，也不套用 PG 专属的语句白名单校验。
	//   - 普通迁移无需 CONCURRENTLY 检查。
	if dialect == dbdialect.DialectMySQL {
		return nonTx, nil
	}
...
// splitSQLStatementsMySQL 是「触发器/存储过程感知」的 MySQL 语句分割器。
//
// go-sql-driver 默认一次 Exec 只能执行一条语句，因此含多条语句的迁移文件
// 必须逐条执行。但 CREATE TRIGGER ... BEGIN ... END 与 CREATE FUNCTION ... $$ ... $$
// 复合语句体内部含有分号，不能按顶层 ";" 粗暴分割。
//
// 本函数按字符扫描，跟踪：
//   - 单引号 / 双引号 / 反引号字符串字面量（含 ” 转义）
//   - 行注释 --  与块注释 /* */
//   - BEGIN ... END 复合体嵌套深度（触发器/存储过程体）
//   - $$ ... $$ dollar-quoted 体（兼容尚未改写的历史 PG 片段，视作整体）
//
// 仅在复合体外、且不在字符串/注释中的顶层 ";" 处切分。
func splitSQLStatementsMySQL(content string) []string {
	var out []string
	...
	var (
		...
		inBlockComment bool // /* ... */
		inDollar       bool // $$ ... $$
		beginDepth     int  // BEGIN...END 嵌套深度
	)

	upperWordBefore := func(pos, length int) string {
		start := pos - length
		if start < 0 {
			return ""
		}
		return strings.ToUpper(string(runes[start:pos]))
	}
	// isWordBoundary 判断某位置是否处于标识符边界（前后非字母数字下划线）。
	...
	for i := 0; i < n; i++ {
		...
		// 复合体深度跟踪（仅在标识符边界处匹配）。
		// 关键设计：只有 BEGIN 开启深度；只有「收尾一个 BEGIN 的 END」才减深度。
		//   - "CREATE TABLE IF NOT EXISTS" 里的 IF 不是复合体开启符，忽略。
		//   - "END IF / END LOOP / END WHILE / END CASE / END REPEAT" 收尾的是控制流，
		//     不改变 beginDepth（只消费 token），避免把过程体过早截断。
		//   - 光秃的 "END"（后跟 ; 或空白或结束）收尾一个 BEGIN，减深度。
		prevBoundary := i == 0 || !isIdentRune(runes[i-1])
		matchKw := func(kw string) bool {
			L := len(kw)
			if i+L > n {
				return false
			}
			if !strings.EqualFold(string(runes[i:i+L]), kw) {
				return false
			}
			after := i + L
			return after == n || !isIdentRune(runes[after])
		}
		if prevBoundary {
			// 复合体深度模型：
			//   开启（depth++）：BEGIN、CASE（表达式或语句 CASE 都以 END 收尾，需计一层）。
			//   IF/LOOP/WHILE/REPEAT 不计层（避免 "IF NOT EXISTS" 等 DDL 里的 IF 误增）。
			//   收尾：
			//     - "END CASE"  -> depth--（收尾语句 CASE）
			//     - "END IF/LOOP/WHILE/REPEAT" -> 不变（收尾未计层的控制流）
			//     - 光秃 "END"  -> depth--（收尾 BEGIN 或表达式 CASE）
			if matchKw("BEGIN") {
				beginDepth++
				buf.WriteString(string(runes[i : i+5]))
				i += 4
				continue
			}
			if matchKw("CASE") {
				beginDepth++
				buf.WriteString(string(runes[i : i+4]))
				i += 3
				continue
			}
			if matchKw("END") {
				j := i + 3
				for j < n && isASCIISpace(runes[j]) {
					j++
				}
				closer := ""
				for _, tk := range []string{"IF", "CASE", "LOOP", "WHILE", "REPEAT"} {
					L := len(tk)
					if j+L <= n && strings.EqualFold(string(runes[j:j+L]), tk) &&
						(j+L == n || !isIdentRune(runes[j+L])) {
						closer = tk
						break
					}
				}
				...
				case "":
					...
					continue
				case "CASE":
					// END CASE：收尾语句 CASE（曾在 CASE 处 ++）。
					if beginDepth > 0 {
						beginDepth--
					}
					buf.WriteString(string(runes[i : j+len(closer)]))
					i = j + len(closer) - 1
					continue
				default:
					// END IF/LOOP/WHILE/REPEAT：收尾未计层的控制流，不动深度。
					buf.WriteString(string(runes[i : j+len(closer)]))
					i = j + len(closer) - 1
					continue
				}
...
// advisoryLockConnection 抽象出锁操作所需的连接方法。
type advisoryLockConnection interface {
	QueryRowContext(ctx context.Context, query string, args ...any) *sql.Row
	ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error)
}

// migrationsLockName 是 MySQL/MariaDB GET_LOCK 使用的命名锁名。
const migrationsLockName = "sub2api_migrations"

// acquireMigrationsLock 获取方言特定的分布式锁：
//   - PostgreSQL: pg_try_advisory_lock（轮询直到成功）
//   - MySQL/MariaDB: GET_LOCK(name, timeout)（轮询直到成功）
func acquireMigrationsLock(ctx context.Context, db advisoryLockConnection, dialect dbdialect.Dialect) error {
	// MySQL/MariaDB 是唯一支持的方言；保留 dialect 参数仅为 API 稳定。
	return mysqlGetLock(ctx, db)
}

// releaseMigrationsLock 释放方言特定的分布式锁。
func releaseMigrationsLock(ctx context.Context, db advisoryLockConnection, dialect dbdialect.Dialect) error {
	return mysqlReleaseLock(ctx, db)
}

// mysqlGetLock 获取 MySQL/MariaDB 命名锁 GET_LOCK。
// 返回值：1=成功获取，0=超时，NULL=错误。这里用 5 秒超时轮询。
func mysqlGetLock(ctx context.Context, db advisoryLockConnection) error {
	ticker := time.NewTicker(migrationsLockRetryInterval)
	defer ticker.Stop()

	for {
		var res sql.NullInt64
		if err := db.QueryRowContext(ctx, "SELECT GET_LOCK(?, 5)", migrationsLockName).Scan(&res); err != nil {
...
// mysqlReleaseLock 释放 MySQL/MariaDB 命名锁。
func mysqlReleaseLock(ctx context.Context, db advisoryLockConnection) error {
	_, err := db.ExecContext(ctx, "SELECT RELEASE_LOCK(?)", migrationsLockName)
	if err != nil {
