...
func invalidateProxyProbeSnapshots(ctx context.Context, exec sqlExecutor, proxyID int64) ([]int64, error) {
	...
	err := withExecutorTx(ctx, exec, func(tx sqlExecutor) error {
		rows, err := tx.QueryContext(ctx, `
			...
			WHERE proxy_id = ?
				AND type = 'apikey'
				AND (
					(JSON_CONTAINS_PATH(extra, 'one', '$.upstream_billing_probe')
						AND JSON_EXTRACT(extra, '$.upstream_billing_probe') <> 'null')
					OR (platform IN ('openai', 'anthropic')
						AND JSON_CONTAINS_PATH(extra, 'one', '$.ollama_cloud_usage_snapshot')
						AND JSON_EXTRACT(extra, '$.ollama_cloud_usage_snapshot') <> 'null')
				)
		...
		if _, err := tx.ExecContext(ctx, `
			...
			SET extra = JSON_REMOVE(COALESCE(extra, '{}'), '$.upstream_billing_probe', '$.ollama_cloud_usage_snapshot'),
				updated_at = NOW()
			WHERE id IN (`+sqlPlaceholders(len(ids))+`)`, toAnySlice(ids)...); err != nil {
			return err
		}
		accountIDs = ids
		return nil
	...
	err := withExecutorTx(ctx, exec, func(tx sqlExecutor) error {
		...
		if target == nil {
			_, err = tx.ExecContext(ctx, `
				UPDATE accounts SET proxy_id=NULL, proxy_fallback_origin_id=?,
					extra=CASE
						WHEN type='apikey' AND JSON_CONTAINS_PATH(extra, 'one', '$.upstream_billing_probe')
						THEN JSON_REMOVE(extra, '$.upstream_billing_probe')
						ELSE extra
					END,
					updated_at=NOW()
				WHERE id IN (`+sqlPlaceholders(len(ids))+`)`, append([]any{proxyID}, toAnySlice(ids)...)...)
		} else {
			args := make([]any, 0, len(ids)+2)
			args = append(args, proxyID, *target)
			args = append(args, toAnySlice(ids)...)
			_, err = tx.ExecContext(ctx, `
				UPDATE accounts SET proxy_id=?, proxy_fallback_origin_id=?,
					extra=CASE
						WHEN type='apikey' AND JSON_CONTAINS_PATH(extra, 'one', '$.upstream_billing_probe')
						THEN JSON_REMOVE(extra, '$.upstream_billing_probe')
						ELSE extra
...
// ListOwnedByUserID lists proxies owned by a user with their bound account counts.
...
// GetOwnedByID returns a proxy owned by a specific user.
...
// CountByOwnerUserID counts proxies owned by a user.
...
// CountOwnedAccountsByProxyID counts non-deleted accounts owned by a user bound to a proxy.
func (r *proxyRepository) CountOwnedAccountsByProxyID(ctx context.Context, ownerUserID, proxyID int64) (int64, error) {
	var count int64
	if err := scanSingleRow(ctx, r.sql, `
		SELECT COUNT(*)
		FROM accounts
		WHERE owner_user_id = ? AND proxy_id = ? AND deleted_at IS NULL
