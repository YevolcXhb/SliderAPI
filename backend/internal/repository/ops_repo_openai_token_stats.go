...
func (r *opsRepository) GetOpenAITokenStats(ctx context.Context, filter *service.OpsOpenAITokenStatsFilter) (*service.OpsOpenAITokenStatsResponse, error) {
	...
	join, where, baseArgs, _ := buildUsageWhere(dashboardFilter, dashboardFilter.StartTime, dashboardFilter.EndTime, 1)
...
WITH stats AS (
  SELECT
    ...
    ROUND(AVG(CAST(ul.first_token_ms AS DECIMAL)), 2) AS avg_first_token_ms,
    COALESCE(SUM(ul.output_tokens), 0) AS total_output_tokens,
    COALESCE(ROUND(AVG(CAST(ul.duration_ms AS DECIMAL)), 0), 0) AS avg_duration_ms,
