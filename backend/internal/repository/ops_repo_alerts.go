...
func (r *opsRepository) UpdateAlertRule(ctx context.Context, input *service.OpsAlertRule) (*service.OpsAlertRule, error) {
	...
	updateSQL := `UPDATE ops_alert_rules SET name = ?, description = ?, enabled = ?, severity = ?, metric_type = ?, operator = ?, threshold = ?, window_minutes = ?, sustained_minutes = ?, cooldown_minutes = ?, notify_email = ?, filters = ?, updated_at = NOW() WHERE id = ?`
	selectSQL := `SELECT id, name, COALESCE(description, ''), enabled, COALESCE(severity, ''), metric_type, operator, threshold, window_minutes, sustained_minutes, cooldown_minutes, COALESCE(notify_email, true), filters, last_triggered_at, created_at, updated_at FROM ops_alert_rules WHERE id = ?`

	var out service.OpsAlertRule
	var filtersRaw []byte
	var lastTriggeredAt sql.NullTime

	if err := execUpdateReturning(
		ctx, r.db,
		updateSQL,
		[]any{strings.TrimSpace(input.Name), strings.TrimSpace(input.Description), input.Enabled, strings.TrimSpace(input.Severity), strings.TrimSpace(input.MetricType), strings.TrimSpace(input.Operator), input.Threshold, input.WindowMinutes, input.SustainedMinutes, input.CooldownMinutes, input.NotifyEmail, filtersArg, input.ID},
		selectSQL,
		[]any{input.ID},
		[]any{&out.ID, &out.Name, &out.Description, &out.Enabled, &out.Severity, &out.MetricType, &out.Operator, &out.Threshold, &out.WindowMinutes, &out.SustainedMinutes, &out.CooldownMinutes, &out.NotifyEmail, &filtersRaw, &lastTriggeredAt, &out.CreatedAt, &out.UpdatedAt},
	); err != nil {
		return nil, err
	}
...
func buildOpsAlertEventsWhere(filter *service.OpsAlertEventFilter) (string, []any) {
	...
	if platform := strings.TrimSpace(filter.Platform); platform != "" {
		...
		clauses = append(clauses, "JSON_UNQUOTE(JSON_EXTRACT(dimensions, '$.platform')) = $"+itoa(len(args)))
	}
	if filter.GroupID != nil && *filter.GroupID > 0 {
		args = append(args, fmt.Sprintf("%d", *filter.GroupID))
		clauses = append(clauses, "JSON_UNQUOTE(JSON_EXTRACT(dimensions, '$.group_id')) = $"+itoa(len(args)))
	}
