...
func (r *groupRepository) ExistsByIDs(ctx context.Context, ids []int64) (map[int64]bool, error) {
	...
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id
		FROM groups
		WHERE FIND_IN_SET(id, ?) AND deleted_at IS NULL
	`, Int64CSV(uniqueIDs))
	if err != nil {
		return nil, err
	}
...
func (r *groupRepository) GetAccountIDsByGroupIDs(ctx context.Context, groupIDs []int64) ([]int64, error) {
	...
	rows, err := r.sql.QueryContext(
		ctx,
		"SELECT DISTINCT account_id FROM account_groups WHERE FIND_IN_SET(group_id, ?) ORDER BY account_id",
		Int64CSV(groupIDs),
	)
	if err != nil {
		return nil, err
...
func (r *groupRepository) BindAccountsToGroup(ctx context.Context, groupID int64, accountIDs []int64) error {
	...
	_, err := r.sql.ExecContext(
		...
		`INSERT IGNORE INTO account_groups (account_id, group_id, priority, created_at)
		 SELECT gid, ?, 50, NOW() FROM JSON_TABLE(?, '$[*]' COLUMNS (gid BIGINT PATH '$')) jt;`,
		JSONInt64Slice(accountIDs),
		groupID,
	)
	if err != nil {
		return err
...
func (r *groupRepository) UpdateSortOrders(ctx context.Context, updates []service.GroupSortOrderUpdate) error {
	...
	if err := scanSingleRow(
		ctx,
		r.sql,
		`SELECT COUNT(*) FROM groups WHERE deleted_at IS NULL AND FIND_IN_SET(id, ?)`,
		[]any{Int64CSV(groupIDs)},
		&existingCount,
	); err != nil {
		return err
	}
	...
	args = append(args, Int64CSV(groupIDs))
	...
	query := fmt.Sprintf(`
		...
		SET sort_order = CASE id
			...
			ELSE sort_order
		END
		WHERE deleted_at IS NULL AND FIND_IN_SET(id, ?)
