...
func (r *groupRepository) CreateFromSource(ctx context.Context, groupIn *service.Group, sourceGroupID int64) error {
	...
	result, err := txClient.ExecContext(
		...
		`INSERT INTO account_groups (account_id, group_id, priority, created_at)
		 SELECT ag.account_id, ?, ag.priority, NOW()
...
func (r *groupRepository) DeleteCascade(ctx context.Context, id int64) ([]int64, error) {
	...
	if _, err := exec.ExecContext(ctx, "UPDATE composite_model_routes SET deleted_at = NOW() WHERE group_id = ? AND deleted_at IS NULL", id); err != nil {
		return nil, err
	}
...
func (r *groupRepository) loadAccountCounts(ctx context.Context, groupIDs []int64) (counts map[int64]groupAccountCounts, err error) {
	...
	// MariaDB 兼容:group_id = ANY(?) → IN (?, ?, ...),参数逐个展开。
	query := fmt.Sprintf(`SELECT ag.group_id,
	...
	GROUP BY ag.group_id`, groupAccountAvailableSQL, groupAccountTemporarilyLimitedSQL, sqlPlaceholders(len(groupIDs)))
...
func (r *groupRepository) GetAccountIDsByGroupIDs(ctx context.Context, groupIDs []int64) ([]int64, error) {
	...
	// MariaDB 兼容:group_id = ANY(?) → IN (?, ?, ...),参数逐个展开。
	query := fmt.Sprintf("SELECT DISTINCT account_id FROM account_groups WHERE group_id IN (%s) ORDER BY account_id", sqlPlaceholders(len(groupIDs)))
	args := make([]any, 0, len(groupIDs))
	for _, groupID := range groupIDs {
		args = append(args, groupID)
	}
...
func (r *groupRepository) BindAccountsToGroup(ctx context.Context, groupID int64, accountIDs []int64) error {
	...
	// MariaDB 兼容:原 PG 分支(INSERT ... SELECT unnest(?[]) ... ON CONFLICT)
	// 改为多行 INSERT ... ON DUPLICATE KEY UPDATE(忽略已存在的绑定)。
	rowPlaceholders := make([]string, 0, len(accountIDs))
	args := make([]any, 0, len(accountIDs)*4)
	for range accountIDs {
		rowPlaceholders = append(rowPlaceholders, "(?, ?, 50, NOW())")
	}
	for _, accountID := range accountIDs {
		args = append(args, accountID, groupID)
	}
	query := fmt.Sprintf(
		"INSERT INTO account_groups (account_id, group_id, priority, created_at) VALUES %s ON DUPLICATE KEY UPDATE account_id = account_id",
...
func (r *groupRepository) UpdateSortOrders(ctx context.Context, updates []service.GroupSortOrderUpdate) error {
	...
	existingQuery := fmt.Sprintf(`SELECT COUNT(*) FROM groups WHERE deleted_at IS NULL AND id IN (%s)`, sqlPlaceholders(len(groupIDs)))
	existingArgs := make([]any, 0, len(groupIDs))
	for _, id := range groupIDs {
		existingArgs = append(existingArgs, id)
	}
	if err := scanSingleRow(ctx, r.sql, existingQuery, existingArgs, &existingCount); err != nil {
		return err
	}
	...
	args := make([]any, 0, len(groupIDs)*2+len(groupIDs))
	...
	inPlaceholders := make([]string, len(groupIDs))
