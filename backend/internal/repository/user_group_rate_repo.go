...
func (r *userGroupRateRepository) SyncUserGroupRates(ctx context.Context, userID int64, rates map[int64]*float64) error {
	...
	if len(clearGroupIDs) > 0 {
		...
		// 展开后的占位符编号 = 2..1+len(clearGroupIDs)。
		ph := sqlInPlaceholders(2, len(clearGroupIDs))
		args := []any{userID}
		args = append(args, toAnySlice(clearGroupIDs)...)
		if _, err := r.sql.ExecContext(ctx, `
			UPDATE user_group_rate_multipliers
			SET rate_multiplier = NULL, updated_at = NOW()
		...
		if _, err := r.sql.ExecContext(ctx,
			`DELETE FROM user_group_rate_multipliers WHERE user_id = ? AND group_id IN (`+ph+`) AND rate_multiplier IS NULL AND rpm_override IS NULL`,
			args...); err != nil {
			return err
		}
	}
	...
	if len(upsertGroupIDs) > 0 {
		now := time.Now()
		// MariaDB 兼容:原 PG 分支(INSERT ... SELECT unnest(?[], ?[])
		// ... ON CONFLICT DO UPDATE)改为多行 INSERT ... ON DUPLICATE KEY UPDATE。
		rowPlaceholders := make([]string, 0, len(upsertGroupIDs))
		args := make([]any, 0, len(upsertGroupIDs)*5)
		for range upsertGroupIDs {
...
func (r *userGroupRateRepository) SyncGroupRateMultipliers(ctx context.Context, groupID int64, entries []service.GroupRateMultiplierInput) error {
	...
	if len(keepUserIDs) == 0 {
		...
		// MariaDB 兼容:user_id <> ALL(?) → user_id NOT IN (?, ?, ...),参数逐个展开;
		// 展开后的占位符编号 = 2..1+len(keepUserIDs)。
		ph := sqlInPlaceholders(2, len(keepUserIDs))
		args := []any{groupID}
		args = append(args, toAnySlice(keepUserIDs)...)
		if _, err := r.sql.ExecContext(ctx, `
			UPDATE user_group_rate_multipliers
			SET rate_multiplier = NULL, updated_at = NOW()
			WHERE group_id = ? AND user_id NOT IN (`+ph+`)
		`, args...); err != nil {
			return err
		}
	}
	...
	// MariaDB 兼容:原 PG 分支(INSERT ... SELECT unnest(?[], ?[])
	// ... ON CONFLICT DO UPDATE)改为多行 INSERT ... ON DUPLICATE KEY UPDATE。
	rowPlaceholders := make([]string, 0, len(userIDs))
...
func (r *userGroupRateRepository) SyncGroupRPMOverrides(ctx context.Context, groupID int64, entries []service.GroupRPMOverrideInput) error {
	...
	if len(clearUserIDs) > 0 {
		// MariaDB 兼容:user_id = ANY(?) → user_id IN (?, ?, ...),参数逐个展开。
		ph := sqlInPlaceholders(2, len(clearUserIDs))
		args := []any{groupID}
		args = append(args, toAnySlice(clearUserIDs)...)
		if _, err := r.sql.ExecContext(ctx, `
			UPDATE user_group_rate_multipliers
			SET rpm_override = NULL, updated_at = NOW()
			WHERE group_id = ? AND user_id IN (`+ph+`)
		`, args...); err != nil {
			return err
		}
	}
	...
	if len(upsertUserIDs) > 0 {
		...
		// MariaDB 兼容:原 PG 分支(INSERT ... SELECT unnest(?[], ?[])
		// ... ON CONFLICT DO UPDATE)改为多行 INSERT ... ON DUPLICATE KEY UPDATE。
		rowPlaceholders := make([]string, 0, len(upsertUserIDs))
		args := make([]any, 0, len(upsertUserIDs)*5)
		for range upsertUserIDs {
			rowPlaceholders = append(rowPlaceholders, "(?, ?, ?, ?, ?)")
		}
