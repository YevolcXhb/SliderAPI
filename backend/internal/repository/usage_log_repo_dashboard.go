...
func (r *usageLogRepository) fillDashboardUsageStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, now time.Time) error {
	...
	combinedStatsQuery := `
		...
		SELECT
			...
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN input_tokens ELSE NULL END), 0) AS total_input_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN output_tokens ELSE NULL END), 0) AS total_output_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN cache_creation_tokens ELSE NULL END), 0) AS total_cache_creation_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN cache_read_tokens ELSE NULL END), 0) AS total_cache_read_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN total_cost ELSE NULL END), 0) AS total_cost,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN actual_cost ELSE NULL END), 0) AS total_actual_cost,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN account_cost ELSE NULL END), 0) AS total_account_cost,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN duration_ms ELSE NULL END), 0) AS total_duration_ms,
			SUM(CASE WHEN created_at >= ? AND created_at < ? THEN 1 ELSE 0 END) AS today_requests,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN input_tokens ELSE NULL END), 0) AS today_input_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN output_tokens ELSE NULL END), 0) AS today_output_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN cache_creation_tokens ELSE NULL END), 0) AS today_cache_creation_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN cache_read_tokens ELSE NULL END), 0) AS today_cache_read_tokens,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN total_cost ELSE NULL END), 0) AS today_cost,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN actual_cost ELSE NULL END), 0) AS today_actual_cost,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN account_cost ELSE NULL END), 0) AS today_account_cost
		FROM scoped
	`
	var totalDurationMs int64
	if err := scanSingleRow(
...
func (r *usageLogRepository) GetUserDashboardStats(ctx context.Context, userID int64) (*UserDashboardStats, error) {
	...
	platformQuery := `
		SELECT
			...
			SUM(CASE WHEN ul.created_at >= ? THEN 1 ELSE 0 END) as today_requests,
			COALESCE(SUM(CASE WHEN ul.created_at >= ? THEN ul.input_tokens + ul.output_tokens + ul.cache_creation_tokens + ul.cache_read_tokens ELSE NULL END), 0) as today_tokens,
			COALESCE(SUM(CASE WHEN ul.created_at >= ? THEN ul.actual_cost ELSE NULL END), 0) as today_actual_cost
		FROM usage_logs ul
	...
	`
	rows, err := r.sql.QueryContext(ctx, platformQuery, userID, today, today, today)
	if err != nil {
		return nil, err
	}
	for rows.Next() {
