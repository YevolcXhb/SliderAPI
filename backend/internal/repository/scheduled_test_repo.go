...
func (r *scheduledTestPlanRepository) Update(ctx context.Context, plan *service.ScheduledTestPlan) (*service.ScheduledTestPlan, error) {
	// MariaDB has no UPDATE ... RETURNING: run the UPDATE and re-select inside a tx.
	...
	if _, err := tx.ExecContext(ctx, `
		...
		WHERE id = ?`, plan.ModelID, plan.CronExpression, plan.Enabled, plan.MaxResults, plan.AutoRecover, plan.NextRunAt, plan.ID); err != nil {
		return nil, err
	}
