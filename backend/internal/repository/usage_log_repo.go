...
const usageLogSingleInsertSQL = `
		INSERT IGNORE INTO usage_logs (
			user_id,
			api_key_id,
			account_id,
			request_id,
...
func (r *usageLogRepository) createSingle(ctx context.Context, sqlq sqlExecutor, log *service.UsageLog) (bool, error) {
	...
	res, err := sqlq.ExecContext(ctx, query, prepared.args...)
	if err != nil {
		return false, err
	}
	...
	err = scanSingleRow(ctx, sqlq, "SELECT id, created_at FROM usage_logs WHERE id = ?", []any{insertID}, &log.ID, &log.CreatedAt)
	if err != nil {
		return false, err
	}
	log.RateMultiplier = prepared.rateMultiplier
...
func (r *usageLogRepository) batchInsertUsageLogs(db *sql.DB, keys []string, preparedByKey map[string]usageLogInsertPrepared) (map[string]bool, map[string]usageLogBatchState, bool, error) {
	...
	for _, key := range keys {
		...
		res, err := db.ExecContext(ctx, usageLogSingleInsertSQL, prepared.args...)
		if err != nil {
		...
		if affected > 0 {
			...
			state.ID, err = res.LastInsertId()
			if err != nil {
				return nil, nil, true, err
			}
			if err := scanSingleRow(ctx, db, "SELECT created_at FROM usage_logs WHERE id = ?", []any{state.ID}, &state.CreatedAt); err != nil {
				return nil, nil, true, err
			}
		} else {
			insertedMap[key] = false
			if prepared.requestID != "" {
				if err := scanSingleRow(ctx, db, "SELECT id, created_at FROM usage_logs WHERE request_id = ? AND api_key_id = ?", []any{prepared.requestID, prepared.args[1]}, &state.ID, &state.CreatedAt); err != nil {
					return nil, nil, true, err
				}
			}
		}
...
func buildUsageLogBatchInsertQuery(keys []string, preparedByKey map[string]usageLogInsertPrepared) (string, []any) {
	...
	return buildUsageLogBestEffortInsertQuery(preparedList)
}

// buildUsageLogBatchInsertQueryLegacyPG is retained only as reference for the
// original PostgreSQL WITH ... INSERT ... RETURNING implementation. It is not
// used on MariaDB.
...
func execUsageLogInsertNoResult(ctx context.Context, sqlq sqlExecutor, prepared usageLogInsertPrepared) error {
	_, err := sqlq.ExecContext(ctx, usageLogSingleInsertSQL, prepared.args...)
	return err
}
...
func (r *usageLogRepository) fillDashboardUsageStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, now time.Time) error {
	...
	combinedStatsQuery := `
		...
		SELECT
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN 1 ELSE 0 END), 0) AS total_requests,
			...
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN 1 ELSE 0 END), 0) AS today_requests,
	...
	`
	var totalDurationMs int64
	combinedStatsArgs := []any{startUTC, endUTC, startUTC, endUTC}
	for i := 0; i < 9; i++ {
		combinedStatsArgs = append(combinedStatsArgs, startUTC, endUTC)
	}
	for i := 0; i < 8; i++ {
		combinedStatsArgs = append(combinedStatsArgs, todayUTC, todayEnd)
	}
	if err := scanSingleRow(
		ctx,
		r.sql,
	...
	activeUsersQuery := `
		...
		FROM scoped
	`
	if err := scanSingleRow(ctx, r.sql, activeUsersQuery, []any{todayUTC, todayEnd, todayUTC, todayEnd, todayUTC, todayEnd, hourStart, hourEnd}, &stats.ActiveUsers, &stats.HourlyActiveUsers); err != nil {
		return err
	}
...
func (r *usageLogRepository) fillDashboardTodayAndActiveStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, todayUTC, now time.Time) error {
	...
	activeUsersQuery := `
		...
		FROM scoped
	`
	return scanSingleRow(ctx, r.sql, activeUsersQuery, []any{todayUTC, todayEnd, todayUTC, todayEnd, todayUTC, todayEnd, hourStart, hourEnd}, &stats.ActiveUsers, &stats.HourlyActiveUsers)
}
	...
	query := `
		SELECT
			...
			CASE
				...
				ELSE COALESCE(SUM(total_duration_ms), 0) / SUM(total_requests)
			END as avg_duration_ms
		FROM (
			SELECT
				DATE_FORMAT(CAST(bucket_date AS DATETIME), '%Y-%m-%d') as date,
				total_requests,
				input_tokens as total_input_tokens,
				output_tokens as total_output_tokens,
			...
			UNION ALL
			SELECT
				DATE_FORMAT(CONVERT_TZ(created_at, @@session.time_zone, ?), '%Y-%m-%d') as date,
				COUNT(*) as total_requests,
				COALESCE(SUM(input_tokens), 0) as total_input_tokens,
				COALESCE(SUM(output_tokens), 0) as total_output_tokens,
			...
			WHERE user_id = ?
				...
				AND NOT EXISTS (
					SELECT 1
					FROM usage_daily_dimension_snapshots s
					WHERE s.bucket_date = (CONVERT_TZ(usage_logs.created_at, @@session.time_zone, 'Asia/Shanghai'))
...
func (r *usageLogRepository) GetAccountWindowStatsBatch(ctx context.Context, accountIDs []int64, startTime time.Time) (map[int64]*usagestats.AccountStats, error) {
	...
	query := `
		SELECT
			...
			COALESCE(SUM(kiro_credits), 0) as kiro_credits
		FROM usage_logs
		WHERE FIND_IN_SET(account_id, ?) AND created_at >= ?
		GROUP BY account_id
	`
	rows, err := r.sql.QueryContext(ctx, query, Int64CSV(accountIDs), startTime)
	if err != nil {
		return nil, err
	}
...
func (r *usageLogRepository) GetGeminiUsageTotalsBatch(ctx context.Context, accountIDs []int64, startTime, endTime time.Time) (map[int64]service.GeminiUsageTotals, error) {
	...
	query := `
		...
		FROM usage_logs
		WHERE FIND_IN_SET(account_id, ?) AND created_at >= ? AND created_at < ?
		GROUP BY account_id
	`
	rows, err := r.sql.QueryContext(ctx, query, Int64CSV(accountIDs), startTime, endTime)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (r *usageLogRepository) getUserAccountSharingAccountStats(ctx context.Context, userID int64, startTime, endTime time.Time, page, pageSize int) ([]usagestats.AccountSharingAccountStat, usagestats.AccountSharingSummary, usagestats.AccountSharingAccountPage, error) {
	query := `
		...
		ORDER BY
			ISNULL(p.sort_account_cost), p.sort_account_cost DESC,
			ISNULL(p.created_at), p.created_at DESC,
...
func (r *usageLogRepository) getUsageTrendFromAggregates(ctx context.Context, startTime, endTime time.Time, granularity string) (results []TrendDataPoint, err error) {
	...
	case "day":
		query = fmt.Sprintf(`
			SELECT
				DATE_FORMAT(CAST(bucket_date AS DATETIME), '%s') as date,
				total_requests as requests,
				input_tokens,
				output_tokens,
				cache_creation_tokens,
...
func (r *usageLogRepository) hydrateUsageLogPointsDeductions(ctx context.Context, byID map[int64]*service.UsageLog, ids []int64) error {
	rows, err := r.sql.QueryContext(ctx, `
		SELECT ref_id, COALESCE(SUM(amount), 0)
		FROM points_ledger
		WHERE ref_type = 'usage_log'
			AND reason = 'usage_charge'
			AND direction = 'debit'
			AND FIND_IN_SET(ref_id, ?)
		GROUP BY ref_id
	`, Int64CSV(ids))
	if err != nil {
		return err
...
func buildRawUsageStatsSnapshotConditions(filters UsageLogFilters) ([]string, []any) {
	...
	args := make([]any, 0, 11)
	if filters.UserID > 0 {
		conditions = append(conditions, "ul.user_id = ?")
		args = append(args, filters.UserID)
	}
	if filters.APIKeyID > 0 {
		conditions = append(conditions, "ul.api_key_id = ?")
		args = append(args, filters.APIKeyID)
	}
	if filters.AccountID > 0 {
		conditions = append(conditions, "ul.account_id = ?")
		args = append(args, filters.AccountID)
	}
	if filters.GroupID > 0 {
		conditions = append(conditions, "ul.group_id = ?")
		args = append(args, filters.GroupID)
	}
	if strings.TrimSpace(filters.Model) != "" {
		conditions = append(conditions, fmt.Sprintf("ul.%s = ?", rawUsageLogModelColumn))
	...
	if filters.BillingType != nil {
		conditions = append(conditions, "ul.billing_type = ?")
		args = append(args, int16(*filters.BillingType))
	}
	if filters.BillingMode != "" {
		conditions = append(conditions, "ul.billing_mode = ?")
		args = append(args, filters.BillingMode)
	}
	if filters.StartTime != nil {
		conditions = append(conditions, "ul.created_at >= ?")
		args = append(args, *filters.StartTime)
	}
	if filters.EndTime != nil {
		conditions = append(conditions, "ul.created_at < ?")
	...
	conditions = append(conditions, `NOT EXISTS (
		SELECT 1
		FROM usage_daily_dimension_snapshots s
		WHERE s.bucket_date = (CONVERT_TZ(ul.created_at, @@session.time_zone, 'Asia/Shanghai'))
	)`)
	return conditions, args
}
...
func buildSnapshotUsageStatsConditions(filters UsageLogFilters) ([]string, []any) {
	...
	if filters.APIKeyID > 0 {
		conditions = append(conditions, "s.api_key_id = ?")
		args = append(args, filters.APIKeyID)
	}
	if filters.AccountID > 0 {
		conditions = append(conditions, "s.account_id = ?")
		args = append(args, filters.AccountID)
	}
	if filters.GroupID > 0 {
		conditions = append(conditions, "s.group_id = ?")
		args = append(args, filters.GroupID)
	}
	if strings.TrimSpace(filters.Model) != "" {
		conditions = append(conditions, "s.model = ?")
		args = append(args, filters.Model)
	}
	if filters.RequestType != nil {
		...
		conditions = append(conditions, "s.stream_state = ?")
		args = append(args, streamState)
	}
	if filters.BillingType != nil {
		conditions = append(conditions, "s.billing_type = ?")
		args = append(args, int16(*filters.BillingType))
	}
	if filters.BillingMode != "" {
		conditions = append(conditions, "s.billing_mode = ?")
		args = append(args, filters.BillingMode)
	}
	if filters.StartTime != nil {
		conditions = append(conditions, "s.bucket_date >= ?")
...
func appendAliasedRequestTypeOrStreamWhereCondition(conditions []string, args []any, alias string, requestType *int16, stream *bool) ([]string, []any) {
	...
	if stream != nil {
		conditions = append(conditions, fmt.Sprintf("%s.stream = ?", alias))
		args = append(args, *stream)
	}
	return conditions, args
}
...
func buildSnapshotRequestTypeCondition(startArgIndex int, requestType int16) (string, []any) {
	...
	case service.RequestTypeSync:
		return fmt.Sprintf("(s.request_type = ? OR (s.request_type = %d AND s.stream_state = 0))", startArgIndex), []any{requestTypeArg}
	case service.RequestTypeStream:
		return fmt.Sprintf("(s.request_type = ? OR (s.request_type = %d AND s.stream_state = 1))", startArgIndex), []any{requestTypeArg}
	default:
		return "s.request_type = ?", []any{requestTypeArg}
	}
}
...
func shiftPostgresPlaceholders(query string, offset int) string {
	// MySQL/MariaDB uses positional ? placeholders; no shifting is needed.
	return query
}
...
func buildRequestTypeFilterConditionWithAlias(_ int, requestType int16, alias string) (string, []any) {
	...
	switch normalized {
	case service.RequestTypeSync:
		return fmt.Sprintf("(%srequest_type = ? OR (%srequest_type = ? AND %sstream = FALSE AND %sopenai_ws_mode = FALSE))", prefix, prefix, prefix, prefix), []any{requestTypeArg, int16(service.RequestTypeUnknown)}
	case service.RequestTypeStream:
		return fmt.Sprintf("(%srequest_type = ? OR (%srequest_type = ? AND %sstream = TRUE AND %sopenai_ws_mode = FALSE))", prefix, prefix, prefix, prefix), []any{requestTypeArg, int16(service.RequestTypeUnknown)}
	case service.RequestTypeWSV2:
		return fmt.Sprintf("(%srequest_type = ? OR (%srequest_type = ? AND %sopenai_ws_mode = TRUE))", prefix, prefix, prefix), []any{requestTypeArg, int16(service.RequestTypeUnknown)}
	default:
