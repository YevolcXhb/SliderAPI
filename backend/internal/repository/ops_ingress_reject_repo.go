...
ON DUPLICATE KEY UPDATE request_count = ops_ingress_reject_aggregates.request_count + VALUES(request_count),
              first_seen = LEAST(ops_ingress_reject_aggregates.first_seen, VALUES(first_seen)),
              last_seen = GREATEST(ops_ingress_reject_aggregates.last_seen, VALUES(last_seen)),
              updated_at = NOW()`)
...
func (r *opsRepository) ListIngressRejects(ctx context.Context, filter *service.OpsIngressRejectFilter) (*service.OpsIngressRejectList, error) {
	...
	query := fmt.Sprintf(`SELECT id,bucket_start,reject_reason,route_family,protocol,SUBSTRING_INDEX(client_ip, '/', 1),user_id,api_key_id,request_count,first_seen,last_seen
