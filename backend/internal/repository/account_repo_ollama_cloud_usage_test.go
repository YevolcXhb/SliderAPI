...
func expectOllamaCloudUsageGroupLock(
	...
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR UPDATE")).
		WithArgs(account.ID, account.Platform, account.Type, string(credentials), proxyID, apiKey).
...
func TestListOllamaCloudUsageGroupAccountsUsesOneStrictBatchQuery(t *testing.T) {
	...
	require.Contains(t, query, "JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.api_key')) IN (?)")
	require.Contains(t, query, "platform IN ('openai', 'anthropic')")
	require.Contains(t, query, "JSON_TYPE(JSON_EXTRACT(credentials, '$.api_key')) = 'STRING'")
	require.Contains(t, query, ollamaCloudBaseURLMatchesSQL("JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.base_url'))"))
...
func TestListDueOllamaCloudUsageAccountsFiltersOrdersAndLimits(t *testing.T) {
	...
	maxWait := time.Hour
	var capturedSQL string
	mock.ExpectQuery("SELECT a.id, grouped.group_last_used_at").
	...
	for _, clause := range []string{
		...
		"type = 'apikey'",
		ollamaCloudBaseURLMatchesSQL("JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.base_url'))"),
		"JSON_TYPE(JSON_EXTRACT(a.extra, '$.ollama_cloud_usage_session')) = 'STRING'",
		"JSON_CONTAINS(a.extra, 'true', '$.ollama_cloud_usage_auto_refresh')",
	...
	require.NotContains(t, normalized, "jsonb_")
	require.NotContains(t, normalized, "make_interval")
	require.NotContains(t, normalized, "NULLS FIRST")
...
func TestBulkUpdateOllamaIdentityCleanupIsValueConditional(t *testing.T) {
	...
	require.Contains(t, query, "NOT ("+ollamaCloudBaseURLMatchesSQL("JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.base_url'))"))
	require.Contains(t, query, ollamaCloudBaseURLMatchesSQL("JSON_UNQUOTE(JSON_EXTRACT(?, '$.base_url'))"))
	require.NotContains(t, query, "~*")
	require.Contains(t, query, "platform IN ('openai', 'anthropic') AND type = 'apikey'")
	require.Contains(t, query, "JSON_REMOVE(COALESCE(extra, JSON_OBJECT()), '$.ollama_cloud_usage_session', '$.ollama_cloud_usage_auto_refresh', '$.ollama_cloud_usage_snapshot')")
