...
func TestChannelMonitorV2ErrorAggregationCountsFinalUserErrorsOnly(t *testing.T) {
	query := strings.ToLower(channelMonitorV2ErrorClassifiedSQL)
	...
	require.Contains(t, query, "where rn = 1 and bucket_start >= ? and bucket_start < ?")
	require.Contains(t, strings.ToLower(channelMonitorV2ErrorMetricsInsertSQL), "upstream_affected_requests")
	require.Contains(t, query, "json_length(current_error.upstream_errors) > 0")
	// request_id dedup must be time-bounded (no full-history scan).
	require.Contains(t, query, "interval 90 minute")
	require.Contains(t, query, "current_error.created_at >= ? - interval 90 minute")
}
...
func TestChannelMonitorV2TierRetentionPolicy(t *testing.T) {
	...
	require.Contains(t, channelMonitorV2WatermarkSQL, "INTERVAL 90 DAY")
...
func TestChannelMonitorV2SQLTaxonomyContainsGoNeedles(t *testing.T) {
	sql := channelMonitorV2ErrorClassifiedSQL
	needles := []string{
		"blocked keyword",
		"invalid_api_key",
		"max_tokens",
...
func TestChannelMonitorV2CatalogFilterClearsMultiSelectDimensions(t *testing.T) {
	...
	require.Len(t, catalogArgs, 6) // start, end, platforms (2), groups (2)
...
// issue: ordinary viewer (RestrictGroups) must be scoped to the intersection of
// configured groups, allowed groups and explicit filter group ids — MariaDB 版断言。
func TestChannelMonitorV2WhereRestrictsOrdinaryViewerToAllowedConfiguredGroups(t *testing.T) {
	...
	where, args := channelMonitorV2Where(filter, cfg, "m")
	require.Contains(t, where, "m.group_id IN (?)")
	require.Len(t, args, 4) // start, end, platform, scoped group (4)
