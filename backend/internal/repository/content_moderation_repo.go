...
func buildContentModerationLogWhere(filter service.ContentModerationLogFilter) ([]string, []any) {
	...
	if search := strings.TrimSpace(filter.Search); search != "" {
		...
		where = append(where, "LOWER((l.request_id) LIKE LOWER(?) OR LOWER(l.user_email) LIKE LOWER(?) OR LOWER(l.api_key_name) LIKE LOWER(?) OR LOWER(l.model) LIKE LOWER(?) OR LOWER(l.input_excerpt) LIKE LOWER(?))")
	}
