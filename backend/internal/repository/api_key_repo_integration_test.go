...
func (s *APIKeyRepoSuite) TestDeleteWithAudit_TombstonesWithoutRetainingCredential() {
	...
	rows, err := s.repo.sql.QueryContext(s.ctx, `SELECT key, deleted_at FROM api_keys WHERE id = ?`, key.ID)
	s.Require().NoError(err)
	...
	auditRows, err := s.repo.sql.QueryContext(s.ctx,
		`SELECT COUNT(*) FROM deleted_api_key_audits WHERE api_key_id = ?`, key.ID)
	s.Require().NoError(err)
