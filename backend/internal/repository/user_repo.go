...
func (r *userRepository) GetLatestUsedAtByUserIDs(ctx context.Context, userIDs []int64) (map[int64]*time.Time, error) {
	...
	// MariaDB 兼容:user_id = ANY(?) → IN (?, ?, ...),参数逐个展开。
	query := fmt.Sprintf(`
		SELECT user_id, MAX(created_at) AS last_used_at
		FROM usage_logs
		WHERE user_id IN (%s)
		GROUP BY user_id
	`, sqlInPlaceholders(1, len(userIDs)))
...
func (r *userRepository) filterUsersByAttributes(ctx context.Context, attrs map[int64]string) ([]int64, error) {
	...
	for attrID, value := range attrs {
		clauses = append(clauses, "(attribute_id = ? AND value LIKE ?)")
...
func (r *userRepository) ApplyRedeemBalanceAdjustment(ctx context.Context, id int64, delta float64) error {
	const updateSQL = `
		UPDATE users
		SET balance = GREATEST(balance + ?, 0), updated_at = NOW()
...
func (r *userRepository) DeductAvailableBalance(ctx context.Context, id int64, amount float64) (deducted float64, err error) {
	...
	// MariaDB has no data-modifying CTE/UPDATE..RETURNING. Lock the row,
	// calculate the clamped deduction in Go, and update it in the same
	// transaction. This preserves the original atomic behavior.
	err = withExecutorTx(ctx, r.sql, func(exec sqlExecutor) error {
		var balance float64
		if err := scanSingleRow(ctx, exec,
			"SELECT balance FROM users WHERE id = ? AND deleted_at IS NULL FOR UPDATE",
			[]any{id}, &balance,
		); err != nil {
		...
		_, err := exec.ExecContext(ctx,
			"UPDATE users SET balance = balance - ?, updated_at = NOW() WHERE id = ? AND deleted_at IS NULL",
			deducted, id,
		)
		return err
	})
...
func (r *userRepository) AdjustBalance(ctx context.Context, id int64, delta float64) (service.BalanceChange, error) {
	...
	err := withExecutorTx(ctx, r.sql, func(exec sqlExecutor) error {
		...
		change = service.BalanceChange{Old: old, New: old + delta}
		if change.New < 0 {
			return service.ErrBalanceNegative
		}
		_, err := exec.ExecContext(ctx,
			"UPDATE users SET balance = ?, updated_at = NOW() WHERE id = ? AND deleted_at IS NULL",
			change.New, id,
		)
		return err
	})
...
func (r *userRepository) currentBalance(ctx context.Context, id int64) (balance float64, err error) {
	rows, err := clientFromContext(ctx, r.client).QueryContext(ctx,
		`SELECT balance FROM users WHERE id = ? AND deleted_at IS NULL`, id)
	if err != nil {
		return 0, err
	}
	defer func() {
...
func (r *userRepository) ApplyRedeemConcurrencyAdjustment(ctx context.Context, id int64, delta int) error {
	const updateSQL = `
		UPDATE users
		SET concurrency = GREATEST(concurrency + ?, 0), updated_at = NOW()
		WHERE id = ? AND deleted_at IS NULL
	`
	client := clientFromContext(ctx, r.client)
	result, err := client.ExecContext(ctx, updateSQL, delta, id)
...
func (r *userRepository) BatchSetConcurrency(ctx context.Context, userIDs []int64, value int) (int, error) {
	...
	query := fmt.Sprintf(
		"UPDATE users SET concurrency = ?, updated_at = NOW() WHERE id IN (%s) AND deleted_at IS NULL",
		sqlInPlaceholders(2, len(userIDs)),
	)
	args := []any{value}
	for _, userID := range userIDs {
		args = append(args, userID)
...
func (r *userRepository) BatchAddConcurrency(ctx context.Context, userIDs []int64, delta int) (int, error) {
	...
	// MariaDB 兼容:id = ANY(?) → IN (?, ?, ...),参数逐个展开。
	query := fmt.Sprintf(
		"UPDATE users SET concurrency = GREATEST(concurrency + ?, 0), updated_at = NOW() WHERE id IN (%s) AND deleted_at IS NULL",
		sqlInPlaceholders(2, len(userIDs)),
	)
	args := []any{delta}
	for _, userID := range userIDs {
...
func (r *userRepository) BatchUpdateLimits(ctx context.Context, userIDs []int64, concurrency, rpmLimit *int) (int, error) {
	...
	if rpmLimit != nil {
		...
		args = append(args, value)
		setClauses = append(setClauses, "rpm_limit = ?")
	}
	setClauses = append(setClauses, "updated_at = NOW()")
	// MariaDB 兼容:id = ANY($n) → IN ($n, $n+1, ...),参数逐个展开。
	inStart := len(args) + 1
	for _, userID := range userIDs {
		args = append(args, userID)
	}
	...
	query := fmt.Sprintf(
		...
		sqlInPlaceholders(inStart, len(userIDs)),
	)
	res, err := r.sql.ExecContext(ctx, query, args...)
	if err != nil {
