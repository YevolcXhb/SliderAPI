...
func (r *userRepository) GetLatestUsedAtByUserIDs(ctx context.Context, userIDs []int64) (map[int64]*time.Time, error) {
	...
	rows, err := r.sql.QueryContext(ctx, query, Int64CSV(userIDs))
	if err != nil {
		return nil, err
	}
...
SELECT id,
	...
	CAST(total_recharged AS DOUBLE),
	CAST(total_invite_income AS DOUBLE),
	CAST(total_share_income AS DOUBLE)
FROM users
WHERE FIND_IN_SET(id, ?)`, Int64CSV(ids))
	if err != nil {
		return err
	}
	defer func() { _ = rows.Close() }()
