...
func lockRepositoryScopedKeys(ctx context.Context, client *dbent.Client, exec sqlQueryExecutor, keys ...string) (func(), error) {
	...
	// MariaDB 10.11 使用 GET_LOCK 命名锁（会话级）；用 RELEASE_LOCK 显式释放。
	// lockName 需满足 MariaDB GET_LOCK 的标识符限制，这里用 "sub2api_"+ 哈希形式。
	// MariaDB 10.11 使用 GET_LOCK 命名锁（会话级）；用 RELEASE_LOCK 显式释放。
	// 仅对 MySQL/MariaDB 方言执行：sqlite 单元测试没有 GET_LOCK 函数。
	if client.Driver() == nil || client.Driver().Dialect() != dialect.MySQL {
		return release, nil
	}
	for _, key := range normalized {
		lockName := "sub2api_" + strconv.FormatInt(advisoryLockHash(key), 10)
		rows, err := exec.QueryContext(ctx, "SELECT GET_LOCK(?, 10)", lockName)
		if err != nil {
			release()
			return nil, err
		}
		...
		if !got.Valid || got.Int64 != 1 {
			release()
			_, _ = exec.ExecContext(ctx, "SELECT RELEASE_LOCK(?)", lockName)
			return nil, errors.New("failed to acquire advisory lock: " + lockName)
		}
	}
	return release, nil
}
