...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = ?, billing_mode = ?, input_price = ?, output_price = ?, cache_write_price = ?, cache_read_price = ?, fast_multiplier = ?, flex_multiplier = ?, image_input_price = ?, image_output_price = ?, per_request_price = ?, time_pricing = ?, platform = ?, updated_at = NOW()
		 WHERE id = ?`,
...
func (r *channelRepository) batchLoadModelPricing(ctx context.Context, channelIDs []int64) (map[int64][]service.ChannelModelPricing, error) {
	query := fmt.Sprintf(`SELECT id, channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, fast_multiplier, flex_multiplier, image_input_price, image_output_price, per_request_price, time_pricing, created_at, updated_at
		 FROM channel_model_pricing WHERE channel_id IN (%s) ORDER BY channel_id, id`, sqlPlaceholders(len(channelIDs)))
...
func setGroupIDsTx(ctx context.Context, exec dbExec, channelID int64, groupIDs []int64) error {
	...
	query := fmt.Sprintf(`INSERT INTO channel_groups (channel_id, group_id) VALUES (%s)`,
		strings.TrimSuffix(strings.Repeat("?, ?, ", len(groupIDs)), ", "))
	args := make([]any, 0, len(groupIDs)*2)
	for _, gid := range groupIDs {
		args = append(args, channelID, gid)
	}
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	if err = exec.QueryRowContext(ctx,
		`SELECT created_at, updated_at FROM channel_model_pricing WHERE id = ?`, pricing.ID,
	).Scan(&pricing.CreatedAt, &pricing.UpdatedAt); err != nil {
		return fmt.Errorf("load model pricing timestamps: %w", err)
	}
...
func createIntervalExec(ctx context.Context, exec dbExec, iv *service.PricingInterval) error {
	...
	if err != nil {
		return fmt.Errorf("insert pricing interval: %w", err)
	}
	...
	return exec.QueryRowContext(ctx,
		`SELECT created_at, updated_at FROM channel_pricing_intervals WHERE id = ?`, iv.ID,
...
// isUniqueViolation 检查 MariaDB/MySQL 唯一约束违反错误（错误码 1062）。
...
// escapeLike 转义 LIKE/LIKE 模式中的特殊字符
