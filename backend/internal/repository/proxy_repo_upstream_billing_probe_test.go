...
func TestProxyUpdateInvalidatesBoundProbeSnapshotsAndEnqueuesOutboxAtomically(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("UPDATE `proxies` SET `backup_proxy_id` = NULL WHERE `backup_proxy_id` = ?")).
		WithArgs(int64(9)).
		WillReturnResult(sqlmock.NewResult(0, 0))
	expectProxyUpdateReload(mock, 9, "new.example", "user", "pass")
	mock.ExpectQuery(`(?s)SELECT id FROM accounts.*JSON_CONTAINS_PATH\(extra, 'one', '\$.upstream_billing_probe'\).*JSON_EXTRACT\(extra, '\$.upstream_billing_probe'\).*JSON_CONTAINS_PATH\(extra, 'one', '\$.ollama_cloud_usage_snapshot'\).*FOR UPDATE`).
		WithArgs(int64(9)).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(int64(17)).AddRow(int64(18)))
	mock.ExpectExec(`(?s)UPDATE accounts SET extra = JSON_REMOVE\(COALESCE\(extra, '\{\}'\), '\$.upstream_billing_probe', '\$.ollama_cloud_usage_snapshot'\).*WHERE id IN \(\?, \?\)`).
...
func TestProxyUpdateRollsBackWhenProbeInvalidationOutboxFails(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT id FROM accounts.*JSON_CONTAINS_PATH\(extra, 'one', '\$.upstream_billing_probe'\).*FOR UPDATE`).
		WithArgs(int64(9)).
...
func expectProxyUpdateReload(mock sqlmock.Sqlmock, id int64, host, username, password string) {
	now := time.Now()
	mock.ExpectQuery("(?s)SELECT .* FROM `proxies` WHERE `id` = \\?").
		WithArgs(id).
