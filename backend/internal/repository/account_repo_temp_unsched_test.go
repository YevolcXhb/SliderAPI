...
func TestAccountRepository_GrokCredentialConditionalMutationsAreEligibleAndAtomicallyPropagated(t *testing.T) {
	...
	t.Run("permanent", func(t *testing.T) {
		...
		require.Contains(t, normalized, "UPDATE accounts AS a SET")
...
func TestAccountRepository_GrokCredentialCommitCarriesOutboxAcrossCallerCancellation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Len(t, exec.execQueries, 2, "state update and scheduler outbox must share one transaction")
			require.Contains(t, normalizeSQLWhitespace(strings.Join(exec.execQueries, " ")), "INSERT INTO scheduler_outbox")
		})
	}
}
...
func TestAccountRepository_SetGrokOAuthErrorIfCredentialsUnchanged_RequiresActiveExactCredentialMatch(t *testing.T) {
	...
	require.Len(t, exec.execQueries, 1, "no outbox insert when the CAS update matches zero rows")
	normalized := normalizeSQLWhitespace(exec.execQueries[0])
	require.Contains(t, normalized, "UPDATE accounts AS a SET")
	require.Contains(t, normalized, "platform = ?")
	require.Contains(t, normalized, "type = ?")
	require.Contains(t, normalized, "status = ?")
	require.Contains(t, normalized, "a.credentials = ?")
	require.Contains(t, normalized, "NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(a.credentials, '$.refresh_token'))), '') IS NULL")
...
func TestAccountRepository_SetGrokOAuthErrorIfCredentialsUnchanged_AppliedWritesOutbox(t *testing.T) {
	...
	require.Len(t, exec.execQueries, 2, "UPDATE then conditional outbox INSERT")
	allSQL := normalizeSQLWhitespace(strings.Join(exec.execQueries, " "))
	require.Contains(t, allSQL, "UPDATE accounts AS a SET")
	require.Contains(t, allSQL, "INSERT INTO scheduler_outbox (event_type, account_id, group_id, payload)")
	require.Contains(t, allSQL, "VALUES (?, ?, NULL, NULL)")
}
...
func TestAccountRepository_UpdateGrokOAuthCredentialsIfUnchanged_UsesExactAttemptStateAndAtomicOutbox(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, applied)
	require.Len(t, exec.execQueries, 2, "UPDATE then outbox INSERT")
	normalized := normalizeSQLWhitespace(exec.execQueries[0])
	require.Contains(t, normalized, "UPDATE accounts AS a SET")
	require.Contains(t, normalized, "credentials = ?")
	require.Contains(t, normalized, "credentials = ?")
...
func TestAccountRepository_ListOAuthRefreshCandidatePage_SQLFilter(t *testing.T) {
	...
	require.Contains(t, normalized, "TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.refresh_token'))) <> ''")
	...
	require.NotContains(t, normalized, "JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.expires_at'))")
	require.Len(t, capturedArgs, 7)
	require.Contains(t, capturedArgs, service.PlatformGrok)
