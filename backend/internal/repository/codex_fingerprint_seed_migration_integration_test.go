...
`).Scan(&apiKeyID))
	...
	for _, id := range []int64{offID, apiKeyID} {
		var hasSeed bool
		require.NoError(t, tx.QueryRowContext(ctx, `SELECT extra ? 'codex_fingerprint_seed' FROM accounts WHERE id = ?`, id).Scan(&hasSeed))
...
`, f.name, f.accountType, f.extra).Scan(&id))
	...
	t.Cleanup(func() {
		...
		_, _ = integrationDB.ExecContext(context.Background(), `DELETE FROM scheduler_outbox WHERE account_id IN (`+ph+`)`, args...)
		_, _ = integrationDB.ExecContext(context.Background(), `DELETE FROM accounts WHERE id IN (`+ph+`)`, args...)
	})
