...
func (r *channelMonitorV2Repository) GetConfig(ctx context.Context) (*service.ChannelMonitorV2Config, error) {
	...
	// MariaDB 兼容:group_ids / ignored_error_categories 以 JSON 列存储,
	// 原 PG 分支(pq.Array 扫描)已删除,统一走 MySQL 分支。
	var groupIDsJSON, ignoredJSON []byte
	err := r.db.QueryRowContext(ctx, `
		SELECT version, enabled, refresh_interval_seconds, platforms,
		       COALESCE(group_ids, JSON_ARRAY()),
		       COALESCE(ignored_error_categories, JSON_ARRAY()),
		...
		&groupIDsJSON, &ignoredJSON, &thresholds, &cfg.UpdatedAt, &cfg.UpdatedBy,
	)
	if err != nil {
		return nil, fmt.Errorf("get channel monitor v2 config: %w", err)
	}
	if err := json.Unmarshal(groupIDsJSON, &cfg.GroupIDs); err != nil {
		return nil, fmt.Errorf("decode channel monitor v2 group ids: %w", err)
	}
	if err := json.Unmarshal(ignoredJSON, &cfg.IgnoredErrorCategories); err != nil {
		return nil, fmt.Errorf("decode channel monitor v2 ignored categories: %w", err)
	}
...
func (r *channelMonitorV2Repository) UpdateConfig(ctx context.Context, cfg service.ChannelMonitorV2Config, expectedVersion int) (*service.ChannelMonitorV2Config, error) {
	...
	ignored, err := json.Marshal(cfg.IgnoredErrorCategories)
	if err != nil {
		return nil, err
	}
	cfg.HealthThresholds = service.NormalizeChannelMonitorV2HealthThresholds(cfg.HealthThresholds)
	...
	// MariaDB 兼容:原 PG 分支(UPDATE ... RETURNING + pq.Array)已删除,
	// 统一使用 MySQL 分支(UPDATE + RowsAffected + 回读 GetConfig)。
	result, err := r.db.ExecContext(ctx, `
		UPDATE channel_monitor_v2_config
		SET version = version + 1, enabled = ?, refresh_interval_seconds = ?,
		    platforms = ?, group_ids = ?, ignored_error_categories = ?,
		    health_thresholds = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP(6)
		WHERE id = 1 AND version = ?`,
		cfg.Enabled, cfg.RefreshIntervalSeconds, platforms, groupIDs, ignored, thresholds, cfg.UpdatedBy, expectedVersion,
	)
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("read channel monitor v2 update result: %w", err)
	}
	if changed == 0 {
...
func (r *channelMonitorV2Repository) loadChannelMonitorV2GroupInfo(ctx context.Context, groupIDs []int64) (map[int64]channelMonitorV2GroupInfo, error) {
	...
	query := fmt.Sprintf(`SELECT id, COALESCE(name, ''), lower(COALESCE(NULLIF(TRIM(platform), ''), 'unknown')) FROM groups WHERE id IN (%s) AND deleted_at IS NULL AND status = 'active'`, sqlPlaceholders(len(groupIDs)))
...
func (r *channelMonitorV2Repository) loadErrorDetails(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config) (map[string][]service.ChannelMonitorV2ErrorDetail, error) {
	...
	if len(platforms) > 0 {
		// MariaDB 兼容:platform = ANY($n) → IN (?, ?, ...),参数逐个展开。
		conditions = append(conditions, fmt.Sprintf("lower(COALESCE(NULLIF(TRIM(current_error.platform), ''), 'unknown')) IN (%s)", sqlPlaceholders(len(platforms))))
		args = append(args, toAnySlice(platforms)...)
	} else {
	...
	if groupScopeEmpty {
		...
		// MariaDB 兼容:group_id = ANY($n) → IN (?, ?, ...),参数逐个展开。
		conditions = append(conditions, fmt.Sprintf("COALESCE(current_error.group_id, 0) IN (%s)", sqlPlaceholders(len(groups))))
		args = append(args, toAnySlice(groups)...)
	}
	query := `SELECT
...
func (r *channelMonitorV2Repository) loadFacts(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, byBucket bool) ([]channelMonitorV2Fact, error) {
	...
	if byBucket {
		if bucketSeconds > 0 {
			...
			secs := int(filter.Bucket.Seconds())
			// MariaDB 无 date_bin：用 FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(col)/secs)*secs) 对齐到桶。
			bucketExpr = fmt.Sprintf("FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(m.bucket_start) / %d) * %d)", secs, secs)
			group = bucketExpr + "," + group
		}
	}
...
func (r *channelMonitorV2Repository) loadHistograms(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, userID int64, byBucket bool) ([]channelMonitorV2Histogram, error) {
	...
	if byBucket {
		if bucketSeconds > 0 {
			...
			secs := int(filter.Bucket.Seconds())
			// MariaDB 无 date_bin：用 FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(col)/secs)*secs) 对齐到桶。
			bucketExpr = fmt.Sprintf("FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(h.bucket_start) / %d) * %d)", secs, secs)
			group = bucketExpr + "," + group
		}
	}
...
func channelMonitorV2WhereWithRollup(filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, alias string) (string, []any, int) {
	...
	if bucketSeconds > 0 {
		...
		where += fmt.Sprintf(" AND %s.bucket_seconds = ?", alias)
	}
...
func channelMonitorV2Where(filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, alias string) (string, []any) {
	conditions := []string{alias + ".bucket_start >= ?", alias + ".bucket_start < ?"}
	args := []any{filter.Start, filter.End}
	platforms := channelMonitorV2EnabledPlatforms(cfg)
	if len(filter.Platforms) > 0 {
		platforms = intersectStrings(platforms, filter.Platforms)
	...
	if len(platforms) > 0 {
		// MariaDB 兼容:platform = ANY($n) → IN (?, ?, ...),参数逐个展开。
		conditions = append(conditions, fmt.Sprintf("%s.platform IN (%s)", alias, sqlPlaceholders(len(platforms))))
		args = append(args, toAnySlice(platforms)...)
	} else {
		conditions = append(conditions, "FALSE")
	}
	...
	if groupScopeEmpty {
		...
		// MariaDB 兼容:group_id = ANY($n) → IN (?, ?, ...),参数逐个展开。
		conditions = append(conditions, fmt.Sprintf("%s.group_id IN (%s)", alias, sqlPlaceholders(len(groups))))
		args = append(args, toAnySlice(groups)...)
	}
...
func (r *channelMonitorV2Repository) loadIgnoredErrorCounts(
	...
	if filter.Bucket > 0 && bucketSeconds == 0 {
		secs := int(filter.Bucket.Seconds())
		// MariaDB 无 date_bin：用 FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(col)/secs)*secs) 对齐到桶。
		bucketExpr = fmt.Sprintf("FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(e.bucket_start) / %d) * %d)", secs, secs)
		groupBy = bucketExpr + ", e.platform, e.model"
	}
	// MariaDB 兼容:error_category = ANY($n) → IN ($n, $n+1, ...),参数逐个展开。
	catIdx := len(args) + 1
	taxIdx := len(args) + 1 + len(cfg.IgnoredErrorCategories)
	args = append(args, toAnySlice(cfg.IgnoredErrorCategories)...)
	...
	query := fmt.Sprintf(
		`SELECT %s, e.platform, e.model, SUM(e.error_requests)
		 ...
		 AND e.error_category IN (%s) AND e.taxonomy_version = ?
		 GROUP BY %s`,
		bucketExpr, where, sqlInPlaceholders(catIdx, len(cfg.IgnoredErrorCategories)), taxIdx, groupBy,
	)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
...
func (r *channelMonitorV2Repository) loadIgnoredErrorCountsByPlatformModel(
	...
	query := fmt.Sprintf(
		...
		where, sqlInPlaceholders(catIdx, len(cfg.IgnoredErrorCategories)), taxIdx,
	)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
		return byPM, 0, err
...
func (r *channelMonitorV2Repository) loadIgnoredErrorCountsByMatrixKey(
	...
	query := fmt.Sprintf(
		`SELECT %s, e.platform, e.group_id, e.model, SUM(e.error_requests)
		 ...
		 AND e.error_category IN (%s) AND e.taxonomy_version = ?
		 GROUP BY %s`,
		bucketExpr, where, sqlInPlaceholders(catIdx, len(cfg.IgnoredErrorCategories)), taxIdx, groupSQL,
	)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
