...
func (r *auditLogRepository) BatchInsert(ctx context.Context, logs []*service.AuditLog) (int64, error) {
	...
	for start := 0; start < len(logs); start += chunkSize {
		...
		for _, log := range chunk {
			...
			vals := auditLogInsertValues(log)
			rowPl := make([]string, len(vals))
			for i := range vals {
				rowPl[i] = "?"
			}
			placeholders = append(placeholders, "("+strings.Join(rowPl, ",")+")")
			args = append(args, vals...)
		}
		if len(placeholders) == 0 {
			continue
		}
		query := `INSERT INTO audit_logs (` + auditLogInsertColumns + `) VALUES ` + strings.Join(placeholders, ",")
		if _, err := tx.ExecContext(ctx, query, args...); err != nil {
			return inserted, err
		}
		inserted += int64(len(placeholders))
	}
...
func buildAuditLogsWhere(filter *service.AuditLogFilter) (string, []any) {
	...
	if v := strings.TrimSpace(filter.ActorEmail); v != "" {
		...
		clauses = append(clauses, "l.actor_email LIKE $"+itoa(len(args)))
	}
	...
	if v := strings.TrimSpace(filter.Action); v != "" {
		args = append(args, "%"+escapeLikePattern(v)+"%")
		clauses = append(clauses, "l.action LIKE $"+itoa(len(args)))
	}
	...
	if v := strings.TrimSpace(filter.Query); v != "" {
		args = append(args, "%"+escapeLikePattern(v)+"%")
		idx := itoa(len(args))
		clauses = append(clauses, "(l.path LIKE $"+idx+" OR l.action LIKE $"+idx+" OR l.actor_email LIKE $"+idx+")")
	}
