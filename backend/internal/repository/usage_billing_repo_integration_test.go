...
func TestUsageBillingRepositoryApply_UpdatesAccountQuota(t *testing.T) {
	...
	var quotaUsed float64
	require.NoError(t, integrationDB.QueryRowContext(ctx, "SELECT COALESCE(CAST((JSON_UNQUOTE(JSON_EXTRACT(extra, '$.quota_used'))) AS DECIMAL), 0) FROM accounts WHERE id = ?", account.ID).Scan(&quotaUsed))
