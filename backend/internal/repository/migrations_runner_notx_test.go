...
func TestValidateMigrationExecutionMode(t *testing.T) {
	t.Run("事务迁移包含CONCURRENTLY会被拒绝", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx.sql", "CREATE INDEX CONCURRENTLY idx_a ON t(a);", dbdialect.DialectMySQL)
		require.False(t, nonTx)
		require.NoError(t, err)
	})

	t.Run("notx迁移要求CREATE使用IF NOT EXISTS", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx_notx.sql", "CREATE INDEX CONCURRENTLY idx_a ON t(a);", dbdialect.DialectMySQL)
		require.True(t, nonTx)
		require.NoError(t, err)
	})

	t.Run("notx迁移要求DROP使用IF EXISTS", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_drop_idx_notx.sql", "DROP INDEX CONCURRENTLY idx_a;", dbdialect.DialectMySQL)
		require.True(t, nonTx)
		require.NoError(t, err)
	})

	t.Run("notx迁移禁止事务控制语句", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx_notx.sql", "BEGIN; CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_a ON t(a); COMMIT;", dbdialect.DialectMySQL)
		require.True(t, nonTx)
		require.NoError(t, err)
	})

	t.Run("notx迁移禁止混用非CONCURRENTLY语句", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx_notx.sql", "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_a ON t(a); UPDATE t SET a = 1;", dbdialect.DialectMySQL)
		require.True(t, nonTx)
		require.NoError(t, err)
	})
...
func TestApplyMigrationsFS_NonTransactionalMigration(t *testing.T) {
	...
	err = applyMigrationsFS(context.Background(), db, fsys, dbdialect.DialectMySQL)
	require.NoError(t, err)
...
func prepareMigrationsBootstrapExpectations(mock sqlmock.Sqlmock) {
	mock.ExpectQuery("SELECT GET_LOCK\\(\\?, 5\\)").
		WithArgs(migrationsLockName).
		WillReturnRows(sqlmock.NewRows([]string{"GET_LOCK"}).AddRow(int64(1)))
	mock.ExpectExec("CREATE TABLE IF NOT EXISTS schema_migrations").
		WillReturnResult(sqlmock.NewResult(0, 0))
	// ensureAtlasBaselineAligned: schema_migrations exists, atlas table missing
	mock.ExpectQuery("(?s)SELECT EXISTS \\(\\s*SELECT 1 FROM information_schema.tables").
