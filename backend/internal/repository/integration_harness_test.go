...
func TestMain(m *testing.M) {
	...
	mariadbContainer, err := tcmariadb.Run(
		ctx,
		mariadbImage,
		tcmariadb.WithDatabase("sub2api_test"),
		tcmariadb.WithUsername("sub2api"),
		tcmariadb.WithPassword("sub2api"),
	)
	if err != nil {
	...
	if err != nil {
		log.Printf("failed to get mariadb dsn: %v", err)
		os.Exit(1)
	}
	...
	if err := ApplyMigrations(ctx, integrationDB, dbdialect.DialectMySQL); err != nil {
		log.Printf("failed to apply db migrations: %v", err)
		os.Exit(1)
	}

	// 创建 ent client 用于集成测试
	drv := entsql.OpenDB(testSchemaDialect(), integrationDB)
...
//
// SUB2API_TEST_MARIADB_IMAGE overrides the MariaDB image so the suite can be
// run against a different server version.
//
//	SUB2API_TEST_MARIADB_IMAGE=mariadb:10.6 go test -tags integration ./internal/repository/
func selectDockerImage(ctx context.Context, preferred string) string {
	if override := strings.TrimSpace(os.Getenv("SUB2API_TEST_MARIADB_IMAGE")); override != "" &&
		strings.HasPrefix(preferred, "mariadb:") {
		return override
	}
