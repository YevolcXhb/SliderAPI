...
func TestApplyMigrations_NilDB(t *testing.T) {
	err := ApplyMigrations(context.Background(), nil, dbdialect.DialectMySQL)
	require.Error(t, err)
...
func TestApplyMigrations_DelegatesToApplyMigrationsFS(t *testing.T) {
	...
	err = ApplyMigrations(context.Background(), db, dbdialect.DialectMySQL)
	require.Error(t, err)
...
func TestEnsureAtlasBaselineAligned(t *testing.T) {
	t.Run("skip_when_no_legacy_table", func(t *testing.T) {
		...
		err = ensureAtlasBaselineAligned(context.Background(), db, fstest.MapFS{}, dbdialect.DialectMySQL)
		require.NoError(t, err)
		require.NoError(t, mock.ExpectationsWereMet())
	})
	...
	t.Run("create_atlas_and_insert_baseline_when_empty", func(t *testing.T) {
		...
		err = ensureAtlasBaselineAligned(context.Background(), db, fsys, dbdialect.DialectMySQL)
		require.NoError(t, err)
		require.NoError(t, mock.ExpectationsWereMet())
	})
...
func TestApplyMigrationsFS_ReadMigrationError(t *testing.T) {
	...
	// latestMigrationBaseline fails on 001_bad.sql before the baseline insert.
	mock.ExpectExec("SELECT RELEASE_LOCK\\(\\?\\)").
		WithArgs(migrationsLockName).
		WillReturnResult(sqlmock.NewResult(0, 1))
	...
	err = applyMigrationsFS(context.Background(), db, fsys, dbdialect.DialectMySQL)
	require.Error(t, err)
	require.Contains(t, err.Error(), "001_bad.sql")
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestPgAdvisoryLockAndUnlock_ErrorBranches(t *testing.T) {
	...
	t.Run("unlock_exec_error", func(t *testing.T) {
		...
		err = mysqlReleaseLock(context.Background(), db)
		require.Error(t, err)
