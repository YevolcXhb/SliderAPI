...
func TestUsageCleanupRepositoryCreateTask(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, created_at, updated_at FROM usage_cleanup_tasks WHERE id = LAST_INSERT_ID").
...
func TestUsageCleanupRepositoryClaimNextPendingTaskNone(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery("(?s)SELECT id FROM usage_cleanup_tasks.*FOR UPDATE").
...
func TestUsageCleanupRepositoryDeleteUsageLogsBatchQueryError(t *testing.T) {
	...
	mock.ExpectQuery("(?s)SELECT id FROM usage_logs.*LIMIT").
		...
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(int64(1)).AddRow(int64(2)).AddRow(int64(3)).AddRow(int64(4)).AddRow(int64(5)))
	mock.ExpectQuery("DELETE FROM usage_logs").
		WithArgs(int64(1), int64(2), int64(3), int64(4), int64(5)).
		WillReturnError(sql.ErrConnDone)
	mock.ExpectRollback()
