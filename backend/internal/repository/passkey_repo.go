...
func (r *passkeyRepository) EnsureUserHandle(
	...
	if _, err := r.db.ExecContext(ctx, insert, userID, candidate); err != nil {
	...
	if dbdialect.IsMySQLDB(r.db) {
		...
		if execErr != nil {
			...
			if idErr != nil {
				...
				created.ID = id
				err = r.db.QueryRowContext(ctx, `SELECT name, created_at, updated_at FROM passkey_credentials WHERE id = ?`, id).Scan(&created.Name, &created.CreatedAt, &created.UpdatedAt)
			}
		}
	} else {
		err = r.db.QueryRowContext(ctx, `
...
func (r *passkeyRepository) UpdateCredential(
	...
	result, err := r.db.ExecContext(ctx, `
		...
		SET credential_data = ?, last_used_at = ?, updated_at = NOW()
