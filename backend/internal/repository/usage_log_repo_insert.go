...
func (r *usageLogRepository) createSingle(ctx context.Context, sqlq sqlExecutor, log *service.UsageLog) (bool, error) {
	...
	if affected == 0 && prepared.requestID != "" {
...
func buildUsageLogBatchInsertQuery(keys []string, preparedByKey map[string]usageLogInsertPrepared) (string, []any) {
	...
	_, _ = query.WriteString(`
		WITH input (
			...
			SELECT
				` + strings.Join([]string{
		"input_idx", "user_id", "api_key_id", "account_id", "request_id", "model",
		...
		"image_input_tokens", "image_input_cost",
		"input_cost", "output_cost", "cache_creation_cost", "cache_read_cost",
		"total_cost", "actual_cost", "rate_multiplier", "account_rate_multiplier",
		"billing_type", "request_type", "stream", "openai_ws_mode",
		...
		"cache_ttl_overridden", "long_context_billing_applied",
		"channel_id", "model_mapping_chain", "billing_tier", "billing_mode",
		"account_stats_cost", "session_id", "created_at",
	}, ",\n\t\t\t\t") + `
			FROM (VALUES `)
	...
	_, _ = query.WriteString(`
		...
		resolved AS (
			SELECT
				...
				input.request_id,
				input.api_key_id,
				COALESCE(existing.id, 0) AS id,
				COALESCE(existing.created_at, input.created_at) AS created_at,
				(existing.id IS NOT NULL) AS inserted
			FROM input
