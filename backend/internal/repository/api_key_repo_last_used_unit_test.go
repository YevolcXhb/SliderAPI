...
func TestLatestUsageLogIPsQueryUsesWindowFunction(t *testing.T) {
	query, args := latestUsageLogIPsQuery([]int64{11, 22}, dialect.MySQL)
	normalizedQuery := strings.Join(strings.Fields(query), " ")

	require.Contains(t, normalizedQuery, "ROW_NUMBER() OVER (PARTITION BY api_key_id ORDER BY created_at DESC, id DESC) AS rn")
