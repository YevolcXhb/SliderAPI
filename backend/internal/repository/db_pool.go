...
//
// DB pool lifetimes are clamped here. MariaDB (go-sql-driver/mysql) does not
// start background watchCancel goroutines like lib/pq, so the clamping is a
// safety net against idle-TCP drops by cloud proxies. Long-term the pool
// settings are driven from config (see applyDBPoolSettings).
package repository
