...
SELECT email, share_income_balance
FROM users
...
SET status = ?,
	...
	processed_at = NOW(),
	updated_at = NOW()
WHERE id = ?`, targetStatus, note, id); err != nil {
			return nil, err
		}
	} else {
		if _, err := tx.ExecContext(ctx, `
...
SET status = ?,
	...
	processed_at = NOW(),
	updated_at = NOW()
WHERE id = ?`, targetStatus, note, operatorID, id); err != nil {
			return nil, err
		}
	}
...
func buildWithdrawalWhere(params service.WithdrawalListParams, forceUser bool) (string, []any) {
	...
	if keyword := strings.TrimSpace(params.Keyword); keyword != "" {
		...
		clauses = append(clauses, "(LOWER(user_email) LIKE LOWER(?) OR CAST(id AS CHAR) LIKE ?)")
	}
	if len(clauses) == 0 {
		return "", args
	}
...
id, user_id, user_email, amount, fee_amount,
