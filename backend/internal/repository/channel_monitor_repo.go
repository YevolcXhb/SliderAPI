...
func (r *channelMonitorRepository) ListLatestForMonitorIDs(ctx context.Context, ids []int64) (map[int64][]*service.ChannelMonitorLatest, error) {
	...
	const q = `
		...
		FROM (
			...
			FROM channel_monitor_histories
			WHERE FIND_IN_SET(monitor_id, ?)
		) ranked
		WHERE rn = 1
		ORDER BY monitor_id, model
	`
	rows, err := r.db.QueryContext(ctx, q, Int64CSV(ids))
	if err != nil {
...
func (r *channelMonitorRepository) ListRecentHistoryForMonitors(
	...
	const q = `
		WITH targets AS (
		    SELECT jt.monitor_id, jt.model
		           FROM JSON_TABLE(?, '$[*]' COLUMNS (monitor_id BIGINT PATH '$[0]', model TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci PATH '$[1]')) jt
		),
		ranked AS (
		    SELECT h.monitor_id,
		           h.status,
	...
	`
	rows, err := r.db.QueryContext(ctx, q, mustInt64StringPairsValue(pairIDs, pairModels), perMonitorLimit)
	if err != nil {
...
func (r *channelMonitorRepository) ComputeAvailabilityForMonitors(ctx context.Context, ids []int64, windowDays int) (map[int64][]*service.ChannelMonitorAvailability, error) {
	...
	`
	rows, err := r.db.QueryContext(ctx, q, Int64CSV(ids), windowDays)
	if err != nil {
...
func (r *channelMonitorRepository) UpsertDailyRollupsFor(ctx context.Context, targetDate time.Time) (int64, error) {
	const q = `
		...
		FROM channel_monitor_histories
		WHERE checked_at >= ?
		  AND checked_at < ? + INTERVAL 1 DAY
		GROUP BY monitor_id, model
		ON DUPLICATE KEY UPDATE total_checks        = VALUES(total_checks),
...
func (r *channelMonitorRepository) UpdateAggregationWatermark(ctx context.Context, date time.Time) error {
	const q = `
		...
		VALUES (1, ?, NOW())
		ON DUPLICATE KEY UPDATE last_aggregated_date = VALUES(last_aggregated_date),
		    updated_at           = NOW()
	`
