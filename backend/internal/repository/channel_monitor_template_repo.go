...
func (r *channelMonitorRequestTemplateRepository) applyToMonitorsWithClient(
	...
	// MariaDB JSON：JSON_MERGE_PATCH 合并，JSON_EXTRACT 保留内部 operation ID。
	// 占位符顺序：? 模板 headers JSON，? 内部 key，? template_id，
	// ?..?+n-1 monitor id 列表，?+n provider，?+n api_mode。
	idPlaceholders := make([]string, len(monitorIDs))
	for i := range monitorIDs {
		idPlaceholders[i] = "?"
	}
	query := fmt.Sprintf(`
		UPDATE channel_monitors
		SET extra_headers = JSON_MERGE_PATCH(?,
			CASE
				WHEN JSON_EXTRACT(COALESCE(extra_headers, '{}'), CONCAT('$.', ?)) IS NOT NULL
				THEN JSON_OBJECT(?, JSON_EXTRACT(COALESCE(extra_headers, '{}'), CONCAT('$.', ?)))
				ELSE '{}'
			END
		)
		WHERE template_id = ?
		  AND id IN (%s)
		  AND provider = ?
		  AND api_mode = ?`, strings.Join(idPlaceholders, ", "))
	args := make([]any, 0, len(monitorIDs)+4)
	args = append(args, string(templateHeadersJSON), service.ChannelMonitorDuplicateOperationIDMetadataKey, id)
	for _, mid := range monitorIDs {
		args = append(args, mid)
	}
	args = append(args, string(tpl.Provider), defaultAPIModeRepo(tpl.APIMode))
