...
func (r *usageLogRepository) GetAccountWindowStatsBatch(ctx context.Context, accountIDs []int64, startTime time.Time) (map[int64]*usagestats.AccountStats, error) {
	...
	// MariaDB 兼容:account_id = ANY(?) → IN (?, ?, ...),参数逐个展开;
	// 展开后的占位符编号 = 1..len(accountIDs),startTime 占位符编号 = len(accountIDs)+1。
	query := fmt.Sprintf(`
		SELECT
			account_id,
			COUNT(*) as requests,
		...
		GROUP BY account_id
	`, sqlInPlaceholders(1, len(accountIDs)))
...
func (r *usageLogRepository) GetGeminiUsageTotalsBatch(ctx context.Context, accountIDs []int64, startTime, endTime time.Time) (map[int64]service.GeminiUsageTotals, error) {
	...
	// MariaDB 兼容:account_id = ANY(?) → IN (?, ?, ...),参数逐个展开;
	// startTime/endTime 占位符编号 = len(accountIDs)+1 / len(accountIDs)+2。
	query := fmt.Sprintf(`
		SELECT
			account_id,
...
func (r *usageLogRepository) GetBatchUserUsageStats(ctx context.Context, userIDs []int64, startTime, endTime time.Time) (map[int64]*BatchUserUsageStats, error) {
	...
	// startTime/endTime/today 占位符编号 = len(userIDs)+1 / +2 / +3。
	query := fmt.Sprintf(`
		SELECT
			ul.user_id,
			`+usageLogEffectivePlatformExpr+` as platform,
			COALESCE(SUM(CASE WHEN ul.created_at >= ? AND ul.created_at < ? THEN ul.actual_cost ELSE NULL END), 0) as total_cost,
	...
	`,
		sqlInPlaceholders(1, len(normalizedUserIDs)),
	)
	today := timezone.Today()
	args := make([]any, 0, len(normalizedUserIDs)+3)
...
func (r *usageLogRepository) GetBatchAPIKeyUsageStats(ctx context.Context, apiKeyIDs []int64, startTime, endTime time.Time) (map[int64]*BatchAPIKeyUsageStats, error) {
	...
	query := fmt.Sprintf(`
		SELECT
			api_key_id,
			COALESCE(SUM(CASE WHEN created_at >= ? AND created_at < ? THEN actual_cost ELSE NULL END), 0) as total_cost,
			COALESCE(SUM(CASE WHEN created_at >= ? THEN actual_cost ELSE NULL END), 0) as today_cost
		FROM usage_logs
		WHERE api_key_id IN (%s)
		  AND created_at >= LEAST(?, ?)
		GROUP BY api_key_id
	`,
		sqlInPlaceholders(1, len(normalizedAPIKeyIDs)),
	)
	today := timezone.Today()
	args := make([]any, 0, len(normalizedAPIKeyIDs)+3)
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	query := fmt.Sprintf(`
		...
		SELECT 1 AS inbound_grouped, 1 AS upstream_grouped,
			NULL AS inbound_endpoint, NULL AS upstream_endpoint,
			COUNT(*) AS requests,
		...
		FROM scoped
		UNION ALL
		SELECT 0 AS inbound_grouped, 1 AS upstream_grouped,
			inbound_endpoint, NULL AS upstream_endpoint,
			COUNT(*) AS requests,
			COALESCE(SUM(input_tokens), 0) AS input_tokens,
			COALESCE(SUM(output_tokens), 0) AS output_tokens,
			COALESCE(SUM(cache_creation_tokens), 0) AS cache_creation_tokens,
		...
		UNION ALL
		SELECT 1 AS inbound_grouped, 0 AS upstream_grouped,
			NULL AS inbound_endpoint, upstream_endpoint,
			COUNT(*) AS requests,
			COALESCE(SUM(input_tokens), 0) AS input_tokens,
			COALESCE(SUM(output_tokens), 0) AS output_tokens,
			COALESCE(SUM(cache_creation_tokens), 0) AS cache_creation_tokens,
		...
		UNION ALL
		SELECT 0 AS inbound_grouped, 0 AS upstream_grouped,
