...
ORDER BY bucket ASC`
	...
	// errorWhere appears twice in the query (error_buckets + switch_buckets), so each ?
	// consumes a separate arg — duplicate errorArgs accordingly.
	allArgs := make([]any, 0, len(usageArgs)+len(errorArgs)*2)
	allArgs = append(allArgs, usageArgs...)
	allArgs = append(allArgs, errorArgs...)
	allArgs = append(allArgs, errorArgs...)
...
combined AS (
  ...
  UNION ALL
  SELECT e.platform, 0, e.error_count, 0
  FROM error_totals e
  LEFT JOIN usage_totals u ON e.platform = u.platform
...
combined AS (
  ...
  UNION ALL
  SELECT e.group_id, COALESCE(g2.name, '') AS group_name, 0, e.error_count, 0
  FROM error_totals e
  LEFT JOIN usage_totals u ON e.group_id = u.group_id
  LEFT JOIN groups g2 ON g2.id = e.group_id
...
func opsBucketExprForUsage(bucketSeconds int) string {
	...
	case 3600:
		return "FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ul.created_at) / 3600) * 3600)"
	case 300:
		return "FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ul.created_at) / 300) * 300)"
	default:
		return "FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ul.created_at) / 60) * 60)"
	}
}
...
func opsBucketExprForError(bucketSeconds int) string {
	switch bucketSeconds {
	case 3600:
		return "FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(created_at) / 3600) * 3600)"
	case 300:
