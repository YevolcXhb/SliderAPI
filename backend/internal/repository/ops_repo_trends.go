...
switch_buckets AS (
  ...
  CROSS JOIN JSON_TABLE(
    COALESCE(NULLIF(upstream_errors, 'null'), '[]'),
...
combined AS (
  ...
  FROM usage_totals u
  LEFT JOIN error_totals e ON u.platform = e.platform
  UNION ALL
  SELECT COALESCE(u.platform, e.platform) AS platform,
         COALESCE(u.success_count, 0) AS success_count,
         COALESCE(e.error_count, 0) AS error_count,
         COALESCE(u.token_consumed, 0) AS token_consumed
  FROM error_totals e
  LEFT JOIN usage_totals u ON u.platform = e.platform
...
combined AS (
  SELECT COALESCE(u.group_id, e.group_id) AS group_id,
         ...
         COALESCE(u.token_consumed, 0) AS token_consumed
  FROM usage_totals u
  LEFT JOIN error_totals e ON u.group_id = e.group_id
  ...
  SELECT COALESCE(u.group_id, e.group_id) AS group_id,
         ...
         COALESCE(u.token_consumed, 0) AS token_consumed
  FROM error_totals e
  LEFT JOIN usage_totals u ON u.group_id = e.group_id
  LEFT JOIN groups g2 ON g2.id = COALESCE(u.group_id, e.group_id)
...
SELECT
  ...
  SUM(CASE WHEN COALESCE(status_code, 0) THEN 1 ELSE 0 END >= 400) AS error_total,
  SUM(CASE WHEN COALESCE(status_code, 0) THEN 1 ELSE 0 END >= 400 AND is_business_limited) AS business_limited,
  SUM(CASE WHEN COALESCE(status_code, 0) THEN 1 ELSE 0 END >= 400 AND NOT is_business_limited) AS error_sla,
  SUM(CASE WHEN error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) THEN 1 ELSE 0 END NOT IN (429, 529)) AS upstream_excl,
  SUM(CASE WHEN error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) THEN 1 ELSE 0 END = 429) AS upstream_429,
  SUM(CASE WHEN error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) THEN 1 ELSE 0 END = 529) AS upstream_529
FROM ops_error_logs
` + where + `
GROUP BY 1
ORDER BY 1 ASC`
