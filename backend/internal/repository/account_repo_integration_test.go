...
func (s *AccountRepoSuite) TestListWithFilters() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, page, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, tt.accType, tt.status, tt.search, tt.groupID, 0, tt.privacyMode)
			s.Require().NoError(err)
...
func TestGrokOAuthConditionalMutationRollsBackWhenOutboxInsertFails(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = integrationDB.ExecContext(context.Background(), "DELETE FROM scheduler_outbox WHERE account_id = ?", account.ID)
	...
	require.NoError(t, integrationDB.QueryRowContext(
		context.Background(),
		"SELECT COUNT(*) FROM scheduler_outbox WHERE account_id = ?",
		account.ID,
	).Scan(&outboxCount))
