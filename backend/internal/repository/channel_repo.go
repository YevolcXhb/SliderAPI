...
func (r *channelRepository) Create(ctx context.Context, channel *service.Channel) error {
	return r.runInTx(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx,
			`SELECT created_at, updated_at FROM channels WHERE id = ?`, channel.ID,
...
func (r *channelRepository) List(ctx context.Context, params pagination.PaginationParams, status, search string) ([]service.Channel, *pagination.PaginationResult, error) {
	...
	pageSize := params.Limit() // 约束�?[1, 100]
	page := params.Page
	if page < 1 {
		page = 1
	}
...
// batchLoadGroupIDs 批量加载多个渠道的分�?ID
func (r *channelRepository) batchLoadGroupIDs(ctx context.Context, channelIDs []int64) (map[int64][]int64, error) {
	query := fmt.Sprintf(`SELECT channel_id, group_id FROM channel_groups WHERE channel_id IN (%s)`, sqlPlaceholders(len(channelIDs)))
...
func (r *channelRepository) GetGroupsInOtherChannels(ctx context.Context, channelID int64, groupIDs []int64) ([]int64, error) {
	...
	groupIDArgs := args[:len(args)-1]
	args = append(args, channelID)
	query := fmt.Sprintf(`SELECT DISTINCT channel_id FROM channel_groups WHERE channel_id <> ? AND group_id IN (%s)`, sqlPlaceholders(len(groupIDArgs)))
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
...
// marshalModelMapping �?model mapping 序列化为嵌套 JSON 字节
// 格式：{"platform": {"src": "dst"}, ...}
...
// unmarshalModelMapping �?JSON 字节反序列化为嵌�?model mapping
...
func (r *channelRepository) GetGroupPlatforms(ctx context.Context, groupIDs []int64) (map[int64]string, error) {
	...
	query := fmt.Sprintf(`SELECT id, platform FROM groups WHERE id IN (%s)`, sqlPlaceholders(len(groupIDs)))
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
