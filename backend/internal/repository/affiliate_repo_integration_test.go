...
VALUES (?, ?, ?, ?, NOW(), NOW())`, u.ID, affCode, 12.34)
	...
	ledgerCount := querySingleInt(t, txCtx, client,
		"SELECT COUNT(*) FROM user_affiliate_ledger WHERE user_id = ? AND action = 'transfer'", u.ID)
...
func TestAffiliateRepository_AccrueQuota_ReusesOuterTransaction(t *testing.T) {
	...
	rows, err := integrationEntClient.QueryContext(ctx,
		"SELECT COUNT(*) FROM user_affiliates WHERE user_id IN (?, ?)",
...
func TestAffiliateRepository_TransferQuotaToBalance_EmptyQuota(t *testing.T) {
	...
	_, err := client.ExecContext(txCtx, `
INSERT INTO user_affiliates (user_id, aff_code, aff_quota, aff_history_quota, created_at, updated_at)
VALUES (?, ?, 0, 0, NOW(), NOW())`, u.ID, affCode)
	require.NoError(t, err)
...
WHERE user_id IN (?, ?, ?)`,
	...
	gotActive := querySingleTime(t, txCtx, client,
		"SELECT invite_reward_expires_at FROM user_affiliates WHERE user_id = ?", activeInvitee.ID)
	require.WithinDuration(t, activeExpiresAt.AddDate(0, 0, 7), gotActive, time.Second)

	gotExpired := querySingleTime(t, txCtx, client,
		"SELECT invite_reward_expires_at FROM user_affiliates WHERE user_id = ?", expiredInvitee.ID)
	require.WithinDuration(t, expiredExpiresAt, gotExpired, time.Second)

	permanentCount := querySingleInt(t, txCtx, client,
		"SELECT COUNT(*) FROM user_affiliates WHERE user_id = ? AND invite_reward_expires_at IS NULL", permanentInvitee.ID)
	require.Equal(t, 1, permanentCount)

	unboundCount := querySingleInt(t, txCtx, client,
		"SELECT COUNT(*) FROM user_affiliates WHERE user_id = ? AND invite_reward_expires_at IS NULL", unboundUser.ID)
