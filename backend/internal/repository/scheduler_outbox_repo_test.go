...
func TestSchedulerOutboxRepositoryDeleteConsumedUpToUsesBoundedCTE(t *testing.T) {
	...
	repo := &schedulerOutboxRepository{db: db}
	ids := make([]driver.Value, 0, 17)
	args := make([]driver.Value, 0, 17)
	for i := int64(1); i <= 17; i++ {
		ids = append(ids, i)
		args = append(args, i)
	}
	rowBuilder := sqlmock.NewRows([]string{"id"})
	for _, id := range ids {
		rowBuilder = rowBuilder.AddRow(id)
	}
	mock.ExpectBegin()
	mock.ExpectQuery("(?s)SELECT id FROM scheduler_outbox.*LIMIT").
		WithArgs(int64(42), 5000).
		WillReturnRows(rowBuilder)
...
func TestSchedulerOutboxRepositoryTryAcquireCleanupLock(t *testing.T) {
	...
	repo := &schedulerOutboxRepository{db: db}
	mock.ExpectQuery(regexp.QuoteMeta("SELECT GET_LOCK('sub2api_scheduler_outbox_cleanup', 0)")).
		WillReturnRows(sqlmock.NewRows([]string{"pg_try_advisory_lock"}).AddRow(true))
	mock.ExpectExec(regexp.QuoteMeta("SELECT RELEASE_LOCK('sub2api_scheduler_outbox_cleanup')")).
