...
func latestUsageLogIPsQuery(apiKeyIDs []int64, dialectName string) (string, []any) {
	// MariaDB 10.11 不支持 unnest + LATERAL,统一使用窗口函数/派生表模板;
	// 原 PG 分支(unnest(?[]) + LATERAL)已删除。
...
func (r *apiKeyRepository) IncrementQuotaUsedAndGetState(ctx context.Context, id int64, amount float64) (*service.APIKeyQuotaUsageState, error) {
	// MariaDB 不支持 UPDATE..RETURNING; 包成事务:UPDATE 后 SELECT 同一 WHERE 命中行。
...
func (r *apiKeyRepository) IncrementRateLimitUsage(ctx context.Context, id int64, cost float64) error {
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys SET
			...
			window_1d_start = CASE WHEN window_1d_start IS NULL OR window_1d_start + INTERVAL 24 HOUR <= NOW() THEN DATE(NOW()) ELSE window_1d_start END,
...
func (r *apiKeyRepository) ResetRateLimitWindows(ctx context.Context, id int64) error {
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys SET
			usage_5h = CASE WHEN window_5h_start IS NOT NULL AND window_5h_start + INTERVAL 5 HOUR <= NOW() THEN 0 ELSE usage_5h END,
			window_5h_start = CASE WHEN window_5h_start IS NOT NULL AND window_5h_start + INTERVAL 5 HOUR <= NOW() THEN NOW() ELSE window_5h_start END,
			usage_1d = CASE WHEN window_1d_start IS NOT NULL AND window_1d_start + INTERVAL 24 HOUR <= NOW() THEN 0 ELSE usage_1d END,
			window_1d_start = CASE WHEN window_1d_start IS NOT NULL AND window_1d_start + INTERVAL 24 HOUR <= NOW() THEN DATE(NOW()) ELSE window_1d_start END,
			usage_7d = CASE WHEN window_7d_start IS NOT NULL AND window_7d_start + INTERVAL 7 DAY <= NOW() THEN 0 ELSE usage_7d END,
			window_7d_start = CASE WHEN window_7d_start IS NOT NULL AND window_7d_start + INTERVAL 7 DAY <= NOW() THEN DATE(NOW()) ELSE window_7d_start END,
			updated_at = NOW()
		WHERE id = ? AND deleted_at IS NULL`,
		id)
	return err
