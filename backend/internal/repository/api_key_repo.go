...
func (r *apiKeyRepository) IncrementQuotaUsedAndGetState(ctx context.Context, id int64, amount float64) (*service.APIKeyQuotaUsageState, error) {
	...
	if err := scanSingleRow(ctx, r.sql, `
		SELECT quota_used, quota, `+"`key`"+`, status
		FROM api_keys
		WHERE id = ? AND deleted_at IS NULL
...
func (r *apiKeyRepository) IncrementRateLimitUsage(ctx context.Context, id int64, cost float64) error {
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys SET
			...
			window_1d_start = CASE WHEN window_1d_start IS NULL OR window_1d_start + INTERVAL 24 HOUR <= NOW() THEN DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:00') ELSE window_1d_start END,
...
func (r *apiKeyRepository) ResetRateLimitWindows(ctx context.Context, id int64) error {
	_, err := r.sql.ExecContext(ctx, `
		UPDATE api_keys SET
			...
			window_1d_start = CASE WHEN window_1d_start IS NOT NULL AND window_1d_start + INTERVAL 24 HOUR <= NOW() THEN DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:00') ELSE window_1d_start END,
			usage_7d = CASE WHEN window_7d_start IS NOT NULL AND window_7d_start + INTERVAL 7 DAY <= NOW() THEN 0 ELSE usage_7d END,
			window_7d_start = CASE WHEN window_7d_start IS NOT NULL AND window_7d_start + INTERVAL 7 DAY <= NOW() THEN DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:00') ELSE window_7d_start END,
			updated_at = NOW()
		WHERE id = ? AND deleted_at IS NULL`,
		id)
	return err
