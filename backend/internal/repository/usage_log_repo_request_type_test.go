...
func TestAppendUsageLogBillingModeWhereCondition(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantCondition: "(billing_mode = ? OR ((billing_mode IS NULL OR billing_mode = '') AND COALESCE(image_count, 0) > 0))",
		},
		{
			name:          "video remains exact",
			billingMode:   string(service.BillingModeVideo),
			wantCondition: "billing_mode = ?",
		},
		{
...
func TestAppendUsageLogBillingModeWhereConditionWithAlias(t *testing.T) {
	...
	require.Equal(t, []string{"(ul.billing_mode = ? OR ((ul.billing_mode IS NULL OR ul.billing_mode = '') AND COALESCE(ul.image_count, 0) > 0))"}, conditions)
	require.Equal(t, []any{string(service.BillingModeImage)}, args)
}

func TestAppendUsageLogBillingModeQueryFilter(t *testing.T) {
	query, args := appendUsageLogBillingModeQueryFilter("SELECT * FROM usage_logs WHERE user_id = ?", []any{int64(42)}, string(service.BillingModeToken), "")

	require.Equal(t, "SELECT * FROM usage_logs WHERE user_id = ? AND (billing_mode = ? OR ((billing_mode IS NULL OR billing_mode = '') AND COALESCE(image_count, 0) <= 0))", query)
...
func TestUsageLogRepositoryListWithFiltersRequestID(t *testing.T) {
	...
	mock.ExpectQuery("SELECT .* FROM usage_logs WHERE request_id = \\? ORDER BY id DESC LIMIT \\? OFFSET \\?").
...
func TestUsageLogRepositoryListWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("SELECT .* FROM usage_logs WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\? ORDER BY id DESC LIMIT \\? OFFSET \\?").
		WithArgs("gpt-5", 21, 0).
		WillReturnRows(sqlmock.NewRows([]string{"id"}))
...
func TestUsageLogRepositoryGetUsageTrendWithUsageFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("AND COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\?").
...
func TestUsageLogRepositoryGetUserModelStatsUsesRequestedModel(t *testing.T) {
	...
	mock.ExpectQuery("(?s)SELECT\\s+COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) as model,.*WHERE created_at >= \\? AND created_at < \\?\\s+AND user_id = \\?.*GROUP BY COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) ORDER BY total_tokens DESC").
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\?.*UNION ALL").
		WithArgs("gpt-5").
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped",
			"upstream_grouped",
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE \\(request_type = \\? OR \\(request_type = 0 AND stream = FALSE AND openai_ws_mode = FALSE\\)\\).*UNION ALL").
		WithArgs(requestType).
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped",
			"upstream_grouped",
...
func TestUsageLogRepositoryGetGroupStatsWithUsageFiltersAppliesRequestedModelFilter(t *testing.T) {
	...
	mock.ExpectQuery("AND COALESCE\\(NULLIF\\(TRIM\\(ul.requested_model\\), ''\\), ul.model\\) = \\?").
		WithArgs(start, end, "gpt-5").
		WillReturnRows(sqlmock.NewRows([]string{
			"group_id", "group_name", "requests", "total_tokens",
			"cost", "actual_cost", "account_cost",
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs.*UNION ALL").
		WillReturnRows(sqlmock.NewRows([]string{
