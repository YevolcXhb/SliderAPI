...
const (
	conditionalBalanceDeductSQL = `(?s)UPDATE users\s+SET balance = balance - \?,\s+updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL AND balance >= \?`
	overdraftBalanceDeductSQL   = `(?s)UPDATE users\s+SET balance = balance - \?,\s+updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL`
	reserveBatchImageHoldSQL    = `(?s)UPDATE users\s+SET balance = balance - \?,\s+frozen_balance = COALESCE\(frozen_balance, 0\) \+ \?,\s+updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL AND balance >= \?`
	captureBatchImageHoldSQL    = `(?s)UPDATE users\s+SET balance = balance\s+\+ CASE WHEN \? > \? THEN \? - \? ELSE 0 END\s+- CASE WHEN \? > \? THEN \? - \? ELSE 0 END,\s+frozen_balance = COALESCE\(frozen_balance, 0\) - \?,\s+updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL AND COALESCE\(frozen_balance, 0\) >= \?`
	releaseBatchImageHoldSQL    = `(?s)UPDATE users\s+SET balance = balance \+ \?,\s+frozen_balance = COALESCE\(frozen_balance, 0\) - \?,\s+updated_at = NOW\(\)\s+WHERE id = \? AND deleted_at IS NULL AND COALESCE\(frozen_balance, 0\) >= \?`
	userExistsForBillingSQL     = `(?s)SELECT 1\s+FROM users\s+WHERE id = \? AND deleted_at IS NULL`
)
...
func TestReleaseUsageBillingBatchImageBalance_ReturnsFrozenToAvailable(t *testing.T) {
	...
	tx, err := db.BeginTx(ctx, nil)
	require.NoError(t, err)
	mock.ExpectQuery(`SELECT 1\s+FROM usage_billing_dedup\s+WHERE request_id = \? AND api_key_id = \?`).
...
func TestReleaseUsageBillingBatchImageBalance_SkipsWhenHoldNeverReserved(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT 1\s+FROM usage_billing_dedup_archive\s+WHERE request_id = \? AND api_key_id = \?`).
		WithArgs(service.BatchImageHoldRequestID("imgbatch_phantom"), int64(7)).
		WillReturnError(sql.ErrNoRows)
	mock.ExpectCommit()
