...
func RunCLI() error {
	...
	fmt.Println("── Database Configuration (MariaDB 10.11) ──")

	for {
		cfg.Database.Host = promptString(reader, "MariaDB Host", "localhost")
	...
	for {
		cfg.Database.Port = promptInt(reader, "MariaDB Port", 3306)
	...
	for {
		cfg.Database.User = promptString(reader, "MariaDB User", "sub2api")
	...
	cfg.Database.Password = promptPassword("MariaDB Password")
