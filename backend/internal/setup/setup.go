...
type DatabaseConfig struct {
	// Dialect 数据库方言：postgres | mysql（兼容 MySQL/MariaDB）。
...
func (d *DatabaseConfig) toDBCfg() dbdialect.Config {
	...
	return dbdialect.Config{
		...
		SSLMode:   d.SSLMode,
		Charset:   d.Charset,
		ParseTime: d.ParseTime || dia == dbdialect.DialectMySQL,
		Loc:       d.Loc,
	}
}
...
func TestDatabaseConnection(cfg *DatabaseConfig) error {
	...
	dbc := cfg.toDBCfg()
	driverName := dbc.Dialect.Driver()

	db, err := sql.Open(driverName, defaultDSN)
	if err != nil {
		return fmt.Errorf("failed to connect to %s: %w", dbc.Dialect, err)
	}
	...
	defer func() {
		...
		if err := db.Close(); err != nil {
			logger.LegacyPrintf("setup", "failed to close %s connection: %v", dbc.Dialect, err)
		}
	}()
	...
	// Check if target database exists (dialect-specific).
	var exists bool
	...
	targetDB, err := sql.Open(driverName, targetDSN)
	if err != nil {
...
func initializeDatabase(cfg *SetupConfig) error {
	dbc := cfg.Database.toDBCfg()
	db, err := sql.Open(dbc.Dialect.Driver(), dbc.DSN())
	if err != nil {
		return err
	}
	...
	defer cancel()
	return repository.ApplyMigrations(migrationCtx, db, dbc.Dialect)
}
...
func createAdminUser(cfg *SetupConfig) (bool, string, error) {
	...
	// 占位符：postgres 用 $1；mysql 用 ?。
	placeholder := "$1"
	if dbc.Dialect == dbdialect.DialectMySQL {
		placeholder = "?"
	}
	...
	if err := db.QueryRowContext(ctx, fmt.Sprintf("SELECT COUNT(1) FROM users WHERE role = %s", placeholder), service.RoleAdmin).Scan(&adminUsers); err != nil {
		return false, "", err
	}
	...
	// INSERT 占位符按方言切换
	insertSQL := fmt.Sprintf(`INSERT INTO users (email, password_hash, role, balance, concurrency, status, created_at, updated_at)
...
func AutoSetupFromEnv() error {
	...
	cfg := &SetupConfig{
		Database: DatabaseConfig{
			Dialect:  getEnvOrDefault("DATABASE_DIALECT", "mysql"),
			Host:     getEnvOrDefault("DATABASE_HOST", "localhost"),
			Port:     getEnvIntOrDefault("DATABASE_PORT", 3306),
			User:     getEnvOrDefault("DATABASE_USER", "sub2api"),
