...
func TestDatabaseConnection(cfg *DatabaseConfig) error {
	defaultDSN := fmt.Sprintf(
		"%s:%s@tcp(%s:%d)/?charset=utf8mb4&parseTime=true&loc=UTC&multiStatements=true&time_zone=%%27%%2B00%%3A00%%27",
	...
	var exists int
	row := db.QueryRowContext(ctx, "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = ?", cfg.DBName)
	if err := row.Scan(&exists); err != nil {
		return fmt.Errorf("failed to check database existence: %w", err)
	}

	if exists == 0 {
		// cfg.DBName is validated by the caller via validateDBName().
