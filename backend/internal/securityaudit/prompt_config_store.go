...
func (m *ConfigManager) Save(ctx context.Context, req UpdateConfigRequest, actorID int64) (PublicConfig, error) {
	...
	defer func() {
		_, _ = tx.ExecContext(context.Background(), "SELECT RELEASE_LOCK(?)", "sub2api_prompt_audit_config_"+strconv.FormatInt(promptAuditConfigLockKey, 10))
		_ = tx.Rollback()
	}()
	if _, err := tx.ExecContext(ctx, "SELECT GET_LOCK(?, 5)", "sub2api_prompt_audit_config_"+strconv.FormatInt(promptAuditConfigLockKey, 10)); err != nil {
		return PublicConfig{}, err
	}
	current := DefaultStorageConfig()
	var raw string
	err = tx.QueryRowContext(ctx, `SELECT value FROM settings WHERE key=? FOR UPDATE`, SettingKeyPromptAuditConfig).Scan(&raw)
