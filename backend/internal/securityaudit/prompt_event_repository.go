...
func (r *PostgreSQLRepository) DeleteEventsByIDs(ctx context.Context, ids []int64) (*DeleteResult, error) {
	...
	// MariaDB 10.5+ 支持 DELETE ... RETURNING 但与 PG 的 ANY($1) 数组参数不同，
	// 改用 IN (?, ?, ...) 展开。RETURNING 保留（MariaDB 支持）。
	ph := strings.TrimSuffix(strings.Repeat("?, ", len(ids)), ", ")
	rows, err := tx.QueryContext(ctx, `DELETE FROM prompt_audit_events WHERE id IN (`+ph+`) RETURNING job_id`, anySlice(ids)...)
	if err != nil {
		return nil, err
	}
...
func (r *PostgreSQLRepository) DeleteEventsByFilter(ctx context.Context, filter EventFilter, snapshotMaxID int64, batchSize int) (*DeleteResult, error) {
	...
	for {
		...
		// MariaDB has no data-modifying CTE (WITH ... DELETE ... USING ... RETURNING):
		// select ids FOR UPDATE SKIP LOCKED, then DELETE ... WHERE id IN (...) RETURNING.
		selRows, err := tx.QueryContext(ctx, `
			SELECT e.id FROM prompt_audit_events e`+where+
			` AND e.id <= ? ORDER BY e.id LIMIT ? FOR UPDATE`, args...)
		if err != nil {
			_ = tx.Rollback()
			return nil, err
		}
		...
		for selRows.Next() {
			var id int64
			if err := selRows.Scan(&id); err != nil {
				_ = selRows.Close()
				_ = tx.Rollback()
				return nil, err
			}
...
func deleteOrphanJobs(ctx context.Context, tx *sql.Tx, jobIDs []int64) (int64, error) {
	...
	ph := strings.TrimSuffix(strings.Repeat("?, ", len(jobIDs)), ", ")
	result, err := tx.ExecContext(ctx, `DELETE FROM prompt_audit_jobs j
		WHERE j.id IN (`+ph+`) AND j.status <> 'processing'
		AND NOT EXISTS (SELECT 1 FROM prompt_audit_events e WHERE e.job_id=j.id)`, anySlice(jobIDs)...)
	if err != nil {
		return 0, err
	}
...
// anySlice 把 []int64 转成 []any，供 IN (?, ?, ...) 参数展开。
func anySlice(v []int64) []any {
	out := make([]any, len(v))
	for i, n := range v {
		out[i] = n
	}
