...
func (i *ResponsesInputItem) UnmarshalJSON(data []byte) error {
	...
	*i = ResponsesInputItem{}
	...
	// arguments 既可能是 OpenAI 规范的 JSON 字符串，也可能是 Codex 等新客户端
	// 直接给出的 JSON 对象。对象形态按原始 JSON 文本保存，调用方统一走
	// normalizeResponsesArguments 归一化。
	if arguments := bytes.TrimSpace(wire.Arguments); len(arguments) > 0 && !bytes.Equal(arguments, []byte("null")) {
