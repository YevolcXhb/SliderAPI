...
func responsesFunctionOutputToAnthropicContent(item ResponsesInputItem) json.RawMessage {
	...
	// 走到这里说明 output 既不是可识别的分片数组，也没有可用的 text/image 块：
	// 优先提取纯文本，提取不到时保留原始 JSON 文本（与历史行为一致）。
	text := extractResponsesOutputText(item.outputRaw)
	if text == "" {
		text = item.Output
	}
...
// extractResponsesOutputText 把 Responses 的 function_call_output.output 归一化为纯文本。
// output 可能是字符串、[]ResponsesContentPart，或其它无法识别的 JSON；无可提取文本时返回 ""，
// 由调用方决定降级策略。
...
// normalizeResponsesArguments 把 Responses 的 function_call.arguments 归一化为
// Anthropic tool_use.input 可用的 JSON 对象。入参既可能是 JSON 字符串（OpenAI 规范），
// 也可能是对象的原始 JSON 文本（Codex 等新客户端）；非法 JSON 统一降级为 {}，
// 避免把坏 JSON 透传给上游导致 400。
