...
-- [MariaDB] 数据修改型 CTE + UPDATE...FROM -> UPDATE JOIN 派生表。
UPDATE usage_logs ul
JOIN (
    SELECT id
    FROM (
        SELECT id, ROW_NUMBER() OVER (PARTITION BY api_key_id, request_id ORDER BY id) AS rn
        FROM usage_logs
        WHERE request_id IS NOT NULL
    ) t
    WHERE t.rn > 1
) r ON ul.id = r.id
SET ul.request_id = NULL;
...
CREATE TABLE IF NOT EXISTS billing_usage_entries (
    ...
    applied TINYINT(1) NOT NULL DEFAULT TRUE,
