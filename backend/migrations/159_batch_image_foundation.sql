CREATE TABLE IF NOT EXISTS batch_image_jobs (
    ...
    output_expires_at DATETIME(6),
    input_deleted_at DATETIME(6),
    output_deleted_at DATETIME(6),
...
CREATE INDEX IF NOT EXISTS batch_image_jobs_idempotency_key_idx ON batch_image_jobs (idempotency_key);  -- [MariaDB] 去掉部分索引 WHERE
CREATE UNIQUE INDEX IF NOT EXISTS batch_image_jobs_manifest_hash_uq ON batch_image_jobs (manifest_hash);  -- [MariaDB] 去掉部分索引 WHERE
...
CREATE UNIQUE INDEX IF NOT EXISTS batch_image_events_job_event_hash_uq ON batch_image_events (job_id, event_hash);  -- [MariaDB] 去掉部分索引 WHERE
