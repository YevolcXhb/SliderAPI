...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN usage_logs.video_count IS '视频生成数量；>0 表示本行是视频生成用量';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN usage_logs.video_resolution IS '计费用视频分辨率 480p/720p/1080p';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN usage_logs.video_duration_seconds IS '提交时请求的视频时长（秒），按秒计费的乘数';
...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN groups.video_price_480p IS '480p 视频生成每秒单价 (USD/s)，Grok 平台使用';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN groups.video_price_720p IS '720p 视频生成每秒单价 (USD/s)，Grok 平台使用';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN groups.video_price_1080p IS '1080p 视频生成每秒单价 (USD/s)，Grok 平台使用';