...
func TestMigration118DoesNotForceOverwriteAuthSourceGrantDefaults(t *testing.T) {
	...
	// MariaDB: ON CONFLICT (key) DO NOTHING -> ON DUPLICATE KEY UPDATE `key` = `key`
	require.True(t, strings.Contains(sql, "ON DUPLICATE KEY UPDATE `key` = `key`"))
...
func TestMigration119DefersPaymentIndexRolloutToOnlineFollowup(t *testing.T) {
	...
	// MariaDB: CONCURRENTLY 与部分索引 WHERE 已移除，退化为普通唯一索引。
	...
	// MariaDB: ALTER INDEX ... RENAME TO -> ALTER TABLE ... RENAME INDEX ... TO ...
...
func TestMigration124BackfillsLegacyOIDCSecurityFlagsSafely(t *testing.T) {
	...
	// MariaDB: ON CONFLICT (key) DO NOTHING -> ON DUPLICATE KEY UPDATE `key` = `key`
	// MariaDB: 派生表中 `key` 列名有歧义，改用 value = value 空更新。
	require.Contains(t, sql, "ON DUPLICATE KEY UPDATE value = value")
...
func TestMigration134AddsAffiliateLedgerAuditFieldsWithoutJSONCast(t *testing.T) {
	...
	// MariaDB: substring(x FROM regex) -> REGEXP_SUBSTR；派生表别名 ual -> ual2。
	require.Contains(t, sql, "REGEXP_SUBSTR(")
	require.Contains(t, sql, `"rebateAmount"`)
	require.Contains(t, sql, "COUNT(*) OVER (PARTITION BY ra.order_id) AS order_match_count")
	require.Contains(t, sql, "COUNT(*) OVER (PARTITION BY ual2.id) AS ledger_match_count")
...
func TestMigration151AddsAccountAutoPauseExpiryPartialIndex(t *testing.T) {
	...
	sql := string(content)
	// MariaDB: 移除 CONCURRENTLY 与部分索引 WHERE，退化为普通索引。
...
func TestMigration154AddsSparkShadowColumnsAndConstraintsWithoutHotIndexes(t *testing.T) {
	...
	// MariaDB: CHECK 不能引用 AUTO_INCREMENT 列，禁自指改用触发器。
	require.Contains(t, sql, "trg_accounts_parent_not_self")
	...
	// MariaDB: 无 NOT VALID / VALIDATE CONSTRAINT，约束建时即校验。
...
func TestMigration173AllowsCyberBlockedUsageRequestType(t *testing.T) {
	...
	// MariaDB: 去掉 NOT VALID。
	require.Contains(t, sql, "CHECK (request_type IN (0, 1, 2, 3, 4))")
}
