...
-- [MariaDB] ALTER COLUMN ... DROP NOT NULL -> MODIFY COLUMN 去掉 NOT NULL
ALTER TABLE user_group_rate_multipliers
    MODIFY COLUMN rate_multiplier DECIMAL(10,4) NULL;

-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN user_group_rate_multipliers.rate_multiplier IS '专属计费倍率；NULL 表示沿用分组默认倍率。';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN user_group_rate_multipliers.rpm_override IS '专属 RPM 上限；NULL 表示沿用分组默认；0 表示该用户在此分组不受 RPM 限制。';
