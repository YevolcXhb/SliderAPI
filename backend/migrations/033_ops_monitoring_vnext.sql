...
CREATE TABLE IF NOT EXISTS ops_error_logs (
    ...
    account_id BIGINT,
    group_id BIGINT,
    client_ip VARCHAR(45),  -- [MariaDB] PG inet -> VARCHAR(45)（容纳 IPv6）
    ...
    stream TINYINT(1) NOT NULL DEFAULT false,
    user_agent TEXT,
    ...
    is_business_limited TINYINT(1) NOT NULL DEFAULT false,
    ...
    request_body_truncated TINYINT(1) NOT NULL DEFAULT false,
    request_body_bytes INT,

    -- Retryability flags (best-effort classification)
    is_retryable TINYINT(1) NOT NULL DEFAULT false,
    retry_count INT NOT NULL DEFAULT 0,

    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)
);

-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE ops_error_logs IS 'Ops error logs (vNext). Stores sanitized error details and request_body for retries (errors only).';
...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE ops_retry_attempts IS 'Audit table for ops retries (client retry / pinned upstream retry).';
...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE ops_system_metrics IS 'Ops system/request metrics snapshots (vNext). Used for dashboard overview and realtime rates.';
...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE ops_job_heartbeats IS 'Ops background jobs heartbeats (vNext).';
...
CREATE TABLE IF NOT EXISTS ops_alert_events (
    ...
    email_sent TINYINT(1) NOT NULL DEFAULT false,
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)
);
...
-- [MariaDB 重写] MariaDB 不支持表达式索引；PG 的 COALESCE(...) 唯一索引改为
--   STORED 生成列 + 对生成列建唯一索引（等价 NULL 归一）。
ALTER TABLE ops_metrics_hourly
    ADD COLUMN platform_key VARCHAR(32) AS (COALESCE(platform, '')) STORED,
    ADD COLUMN group_id_key BIGINT AS (COALESCE(group_id, 0)) STORED;
CREATE UNIQUE INDEX IF NOT EXISTS idx_ops_metrics_hourly_unique_dim
    ON ops_metrics_hourly (bucket_start, platform_key, group_id_key);
...
CREATE INDEX IF NOT EXISTS idx_ops_metrics_hourly_platform_bucket
    ON ops_metrics_hourly (platform, bucket_start DESC);  -- [MariaDB] 去掉部分索引 WHERE

CREATE INDEX IF NOT EXISTS idx_ops_metrics_hourly_group_bucket
    ON ops_metrics_hourly (group_id, bucket_start DESC);  -- [MariaDB] 去掉部分索引 WHERE

-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE ops_metrics_hourly IS 'vNext hourly pre-aggregated ops metrics (overall/platform/group).';
...
CREATE INDEX IF NOT EXISTS idx_ops_metrics_daily_platform_bucket
    ON ops_metrics_daily (platform, bucket_date DESC);  -- [MariaDB] 去掉部分索引 WHERE

CREATE INDEX IF NOT EXISTS idx_ops_metrics_daily_group_bucket
    ON ops_metrics_daily (group_id, bucket_date DESC);  -- [MariaDB] 去掉部分索引 WHERE

-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE ops_metrics_daily IS 'vNext daily pre-aggregated ops metrics (overall/platform/group).';
...
CREATE INDEX IF NOT EXISTS idx_ops_error_logs_group_time
    ON ops_error_logs (group_id, created_at DESC);  -- [MariaDB] 去掉部分索引 WHERE

CREATE INDEX IF NOT EXISTS idx_ops_error_logs_account_time
    ON ops_error_logs (account_id, created_at DESC);  -- [MariaDB] 去掉部分索引 WHERE
...
CREATE INDEX IF NOT EXISTS idx_ops_system_metrics_platform_time
    ON ops_system_metrics (platform, created_at DESC);  -- [MariaDB] 去掉部分索引 WHERE

CREATE INDEX IF NOT EXISTS idx_ops_system_metrics_group_time
    ON ops_system_metrics (group_id, created_at DESC);  -- [MariaDB] 去掉部分索引 WHERE
...
CREATE INDEX IF NOT EXISTS idx_ops_retry_attempts_source_error
    ON ops_retry_attempts (source_error_id, created_at DESC);  -- [MariaDB] 去掉部分索引 WHERE
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_ops_retry_attempts_unique_active
    ON ops_retry_attempts (source_error_id);  -- [MariaDB] 去掉部分索引 WHERE

-- ============================================
-- 2) Optional: pg_trgm + trigram indexes for fuzzy search
-- ============================================

-- [MariaDB 重写] PostgreSQL 使用 pg_trgm + GIN 三元组索引做子串模糊搜索；
-- MariaDB 无 pg_trgm，改用 InnoDB FULLTEXT 索引（MATCH ... AGAINST）。
-- FULLTEXT 与 trigram 语义不同（按词而非任意子串），仅作模糊检索加速；
-- 应用层 LIKE '%x%' 兜底查询仍可用。
CREATE FULLTEXT INDEX IF NOT EXISTS idx_ops_error_logs_request_id_ft
    ON ops_error_logs (request_id);
CREATE FULLTEXT INDEX IF NOT EXISTS idx_ops_error_logs_client_request_id_ft
    ON ops_error_logs (client_request_id);
CREATE FULLTEXT INDEX IF NOT EXISTS idx_ops_error_logs_error_message_ft
    ON ops_error_logs (error_message);
...
ALTER TABLE ops_alert_rules
    ADD COLUMN IF NOT EXISTS notify_email TINYINT(1) NOT NULL DEFAULT true;
...
-- - Keep inserts idempotent via ON DUPLICATE KEY UPDATE `name` = `name`.
--
-- Notes:
...
INSERT INTO ops_alert_rules (
    ...
    true, 'error_rate', '>', 5.0, 5, 5, 'P1', true, 20, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
...
INSERT INTO ops_alert_rules (
    ...
    true, 'success_rate', '<', 95.0, 5, 5, 'P0', true, 15, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
INSERT INTO ops_alert_rules (
    ...
    true, 'p99_latency_ms', '>', 3000.0, 5, 10, 'P2', true, 30, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
INSERT INTO ops_alert_rules (
    ...
    true, 'p95_latency_ms', '>', 2000.0, 5, 10, 'P2', true, 30, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
INSERT INTO ops_alert_rules (
    ...
    true, 'cpu_usage_percent', '>', 85.0, 5, 10, 'P2', true, 30, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
INSERT INTO ops_alert_rules (
    ...
    true, 'memory_usage_percent', '>', 90.0, 5, 10, 'P1', true, 20, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
INSERT INTO ops_alert_rules (
    ...
    true, 'concurrency_queue_depth', '>', 100.0, 5, 5, 'P1', true, 20, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
INSERT INTO ops_alert_rules (
    ...
    true, 'error_rate', '>', 20.0, 1, 1, 'P0', true, 15, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
) ON DUPLICATE KEY UPDATE `name` = `name`;
...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN ops_system_metrics.redis_conn_total IS 'Redis pool total connections (go-redis PoolStats.TotalConns).';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN ops_system_metrics.redis_conn_idle IS 'Redis pool idle connections (go-redis PoolStats.IdleConns).';
