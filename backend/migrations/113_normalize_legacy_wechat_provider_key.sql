...
SET
    ...
    metadata = JSON_MERGE_PATCH(COALESCE(ai.metadata, JSON_OBJECT()), JSON_OBJECT(
...
SET
    provider_key = 'wechat-main',
    metadata = JSON_MERGE_PATCH(COALESCE(channel.metadata, JSON_OBJECT()), JSON_OBJECT(
        'legacy_provider_key', 'wechat',
        'normalized_by_migration', '113_normalize_legacy_wechat_provider_key'
    )),
    updated_at = CURRENT_TIMESTAMP(6)
