...
ALTER TABLE ops_error_logs
  ADD COLUMN IF NOT EXISTS resolved TINYINT(1) NOT NULL DEFAULT false;
...
CREATE INDEX IF NOT EXISTS idx_ops_error_logs_unresolved_time
  ON ops_error_logs (created_at DESC);  -- [MariaDB] 去掉部分索引 WHERE
...
ALTER TABLE ops_retry_attempts
  ADD COLUMN IF NOT EXISTS success TINYINT(1);
...
ALTER TABLE ops_retry_attempts
  ADD COLUMN IF NOT EXISTS response_truncated TINYINT(1) NOT NULL DEFAULT false;
