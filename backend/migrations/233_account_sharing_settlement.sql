...
CREATE INDEX IF NOT EXISTS idx_account_share_policies_lookup
    ON account_share_policies (enabled, scope_type, scope_id, platform, effective_at DESC);  -- [MariaDB] partial index WHERE removed

CREATE INDEX IF NOT EXISTS idx_account_share_policies_created_by_admin
    ON account_share_policies (created_by_admin_id, created_at DESC);  -- [MariaDB] partial index WHERE removed
...
CREATE INDEX IF NOT EXISTS idx_accounts_owner_user_id
    ON accounts (owner_user_id);  -- [MariaDB] partial index WHERE removed

CREATE INDEX IF NOT EXISTS idx_accounts_public_share
    ON accounts (share_mode, share_status, platform);  -- [MariaDB] partial index WHERE removed

CREATE INDEX IF NOT EXISTS idx_accounts_share_policy_id
    ON accounts (share_policy_id);  -- [MariaDB] partial index WHERE removed
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_balance_ledger_unique_usage_reason
    ON user_balance_ledger (user_id, direction, reason, ref_type, ref_id);  -- [MariaDB] partial index WHERE removed
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_account_share_settlement_usage_log_unique
    ON account_share_settlement_entries (usage_log_id);  -- [MariaDB] partial index WHERE removed
