...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_model_pricing.billing_mode IS '计费模式：token（按 token 区间计费）、per_request（按次计费）、image（图片计费）';
...
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON TABLE channel_pricing_intervals IS '渠道定价区间：支持按 token 区间、按次分层、图片分辨率分层';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.min_tokens IS '区间下界（含），token 模式使用';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.max_tokens IS '区间上界（不含），NULL 表示无上限';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.tier_label IS '层级标签，按次/图片模式使用（如 1K、2K、4K、HD）';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.input_price IS 'token 模式：每 token 输入价';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.output_price IS 'token 模式：每 token 输出价';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.cache_write_price IS 'token 模式：缓存写入价';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.cache_read_price IS 'token 模式：缓存读取价';
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN channel_pricing_intervals.per_request_price IS '按次/图片模式：每次请求价格';
