...
func TestChannelMonitorQuotaModeMigration(t *testing.T) {
	...
	// MariaDB: pg_get_constraintdef + position() -> check_constraints.check_clause + INSTR()
	require.Contains(t, sql, "INSTR(monitor_def, 'kimi') = 0")
	require.Contains(t, sql, "INSTR(template_def, 'kimi') = 0")
	...
	// 历史表配额快照列。
	require.Contains(t, sql, "ADD COLUMN IF NOT EXISTS quota JSON")
	...
	require.Contains(t, sql, "ON DUPLICATE KEY UPDATE `key` = `key`")
}
