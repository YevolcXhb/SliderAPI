...
-- [MariaDB 重写] DO $$ + pg_constraint -> 临时存储过程 + information_schema.check_constraints。
DROP PROCEDURE IF EXISTS _mig108_add_signup_check;
CREATE PROCEDURE _mig108_add_signup_check()
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints
        WHERE constraint_schema = DATABASE() AND constraint_name = 'users_signup_source_check'
    ) THEN
        ALTER TABLE users
    ...
    END IF;
END;
CALL _mig108_add_signup_check();
DROP PROCEDURE _mig108_add_signup_check;

CREATE TABLE IF NOT EXISTS auth_identities (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    provider_type VARCHAR(20) NOT NULL,
    provider_key VARCHAR(191) NOT NULL,   -- [MariaDB] 入唯一索引，TEXT 改 VARCHAR(191)
    provider_subject VARCHAR(191) NOT NULL, -- [MariaDB] 入唯一索引
...
CREATE TABLE IF NOT EXISTS auth_identity_channels (
    ...
    provider_type VARCHAR(20) NOT NULL,
    provider_key VARCHAR(191) NOT NULL,   -- [MariaDB] 入唯一索引
    channel VARCHAR(20) NOT NULL,
    channel_app_id VARCHAR(191) NOT NULL, -- [MariaDB] 入唯一索引
    channel_subject VARCHAR(191) NOT NULL, -- [MariaDB] 入唯一索引
    metadata JSON NOT NULL DEFAULT ('{}'),
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
...
CREATE TABLE IF NOT EXISTS identity_adoption_decisions (
    ...
    adopt_display_name TINYINT(1) NOT NULL DEFAULT FALSE,
    adopt_avatar TINYINT(1) NOT NULL DEFAULT FALSE,
