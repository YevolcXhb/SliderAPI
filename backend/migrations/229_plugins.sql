...
CREATE INDEX IF NOT EXISTS idx_sub2api_plugin_bindings_enabled_scope
    ON sub2api_plugin_bindings(platform, account_type, capability);  -- [MariaDB] 去掉部分索引 WHERE

CREATE UNIQUE INDEX IF NOT EXISTS idx_sub2api_plugin_bindings_one_enabled_scope
    ON sub2api_plugin_bindings(capability, platform, account_type);  -- [MariaDB] 去掉部分索引 WHERE
