...
ADD COLUMN is_count_tokens TINYINT(1) NOT NULL DEFAULT FALSE;

-- Add comment
-- [MariaDB: COMMENT ON 已禁用] COMMENT ON COLUMN ops_error_logs.is_count_tokens IS '是否为 count_tokens 请求的错误（用于统计过滤）';
...
ON ops_error_logs(is_count_tokens);  -- [MariaDB] 去掉部分索引 WHERE
