...
ALTER TABLE channel_monitors
    ADD COLUMN IF NOT EXISTS extra_headers      JSON       NOT NULL DEFAULT ('{}');
ALTER TABLE channel_monitors
...
-- 约束 + 外键（幂等）。[MariaDB 重写] DO $$ -> 临时存储过程；部分索引 WHERE 已移除。
DROP PROCEDURE IF EXISTS _mig128_add_constraints;
CREATE PROCEDURE _mig128_add_constraints()
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints
        WHERE constraint_schema = DATABASE() AND constraint_name = 'channel_monitors_body_mode_check'
    ) THEN
        ALTER TABLE channel_monitors
    ...
    IF NOT EXISTS (
        ...
        WHERE constraint_schema = DATABASE() AND constraint_name = 'channel_monitors_template_id_fkey'
          AND table_name = 'channel_monitors' AND constraint_type = 'FOREIGN KEY'
    ) THEN
        ALTER TABLE channel_monitors
    ...
    END IF;
END;
CALL _mig128_add_constraints();
DROP PROCEDURE _mig128_add_constraints;
