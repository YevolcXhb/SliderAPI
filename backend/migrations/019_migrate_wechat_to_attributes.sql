...
SELECT 'wechat', '微信', '用户微信号', 'text', ('[]'), false, ('{}'), '请输入微信号', 0, true, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
WHERE NOT EXISTS (
...
-- [MariaDB] 数据修改型 CTE (WITH ordered ... UPDATE ... FROM) 不支持 -> UPDATE JOIN 派生表。
UPDATE user_attribute_definitions target
JOIN (
    SELECT id, ROW_NUMBER() OVER (ORDER BY display_order, id) - 1 AS new_order
    FROM user_attribute_definitions
    WHERE deleted_at IS NULL
) ordered ON target.id = ordered.id
SET target.display_order = ordered.new_order;
...
-- [MariaDB] UPDATE ... FROM -> UPDATE JOIN。
UPDATE users u
