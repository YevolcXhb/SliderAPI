...
ALTER TABLE users
    ...
    ADD COLUMN IF NOT EXISTS prefer_points_billing TINYINT(1) NOT NULL DEFAULT 0;

ALTER TABLE shop_products
    ADD COLUMN IF NOT EXISTS allow_points_payment TINYINT(1) NOT NULL DEFAULT 0;
...
CREATE INDEX IF NOT EXISTS idx_points_ledger_operator_time
    ON points_ledger (operator_user_id, created_at DESC);  -- [MariaDB] partial index WHERE removed

CREATE UNIQUE INDEX IF NOT EXISTS idx_points_ledger_unique_ref_reason
    ON points_ledger (user_id, direction, reason, ref_type, ref_id);  -- [MariaDB] partial index WHERE removed