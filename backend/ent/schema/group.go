...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("description").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "text", dialect.MySQL: "text"}),
		field.Float("rate_multiplier").
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)", dialect.MySQL: "decimal(10,4)"}).
			Default(1.0),
		...
		// Kiro 模拟缓存配置，仅对 Kiro 平台生效
		field.Bool("kiro_cache_emulation_enabled").
			Default(false).
			Comment("是否启用 Kiro 模拟缓存（仅对 kiro 平台生效）"),
		field.Bool("kiro_auto_sticky_enabled").
			Default(true).
			Comment("是否启用 Kiro 自动会话粘性路由（仅对 kiro 平台生效）"),
		field.Int("kiro_sticky_session_ttl_seconds").
			Default(3600).
			Comment("Kiro 自动会话粘性对 TTL（秒），仅对 kiro 平台生效"),
		field.Float("kiro_cache_emulation_ratio").
			SchemaType(map[string]string{dialect.Postgres: "decimal(5,4)", dialect.MySQL: "decimal(5,4)"}).
			Default(1.0).
			Comment("Kiro 模拟缓存命中率（范围 0-1），仅对 kiro 平台生效"),
		field.String("kiro_endpoint_mode").
			MaxLen(8).
			Default("q").
			Comment("Kiro 网关 endpoint：q=AWS Q (q.{region}.amazonaws.com), krs=Kiro Runtime Service (runtime.us-east-1.kiro.dev)"),
		...
		field.Float("daily_limit_usd").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,8)", dialect.MySQL: "decimal(20,8)"}),
		...
		field.JSON("video_model_prices", map[string]map[string]float64{}).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb", dialect.MySQL: "json"}).
