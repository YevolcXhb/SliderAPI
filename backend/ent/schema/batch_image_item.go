...
func (BatchImageItem) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("prompt_preview").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text", dialect.MySQL: "text"}),
		...
		field.String("error_message").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text", dialect.MySQL: "text"}),
		field.Float("billed_amount").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)", dialect.MySQL: "decimal(20,10)"}),
		...
		field.Time("indexed_at").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "timestamptz", dialect.MySQL: "datetime(6)"}),
	}
}
