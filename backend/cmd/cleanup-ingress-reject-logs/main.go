...
func main() {
	...
	if *execute && deleted > 0 {
		fmt.Println("cleanup complete; run ANALYZE TABLE ops_error_logs during normal maintenance")
	}
}
	...
	for {
		...
		if execute && len(ids) > 0 {
			...
			result, err := db.ExecContext(ctx,
				`DELETE FROM ops_error_logs WHERE id IN (`+strings.Join(placeholders, ", ")+`) AND created_at < ?`, append(args, before)...)
			if err != nil {
				return nil, scanned, matched, deleted, err
			}
