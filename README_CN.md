...
> 基于 MariaDB 10.11 原生重构的 Sub2API 网关——彻底移除 PostgreSQL 数据层，完整集成 ikik 功能，全部 SQL 重写为 MariaDB 兼容。
...
## 为什么是 SliderAPI（MariaDB 重构版）

SliderAPI 是基于 **MariaDB 10.11 原生重构** 的 Sub2API 网关：

- **彻底去 PostgreSQL 数据层** —— 所有查询、迁移和后台任务都已审计并重写为 MariaDB 10.11 写法（移除 SKIP LOCKED / FILTER / ILIKE / AT TIME ZONE / data-modifying CTE 等 PostgreSQL 专用语法）。
- **完整集成 ikik 功能** —— ikik 时代的管理端/用户端、商城与卡密、提现、拼车、邮件广播、各平台 OAuth、账号共享与渠道监控均已接线，前端嵌入后端二进制中。
- **单命令部署** —— Go 二进制嵌入 Vue 前端（`-tags embed`）；数据库仅需 MariaDB，无需 PostgreSQL 迁移工具。
- **Docker 镜像**：`ghcr.io/yevolcxhb/sliderapi`（由本仓库 GitHub Actions 打 tag 时自动构建）。

> 上游原始项目为 [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)。本仓库保留上游功能集，但将数据层替换为 MariaDB 原生 SQL。
...
使用 Docker Compose 部署，包含 MariaDB 和 Redis 容器。
...
- 自动生成安全凭证（JWT_SECRET、TOTP_ENCRYPTION_KEY、MARIADB_ROOT_PASSWORD）
- 创建 `.env` 文件并填充自动生成的密钥
...
Apple 芯片 Mac 在 macOS 26 上可使用 Apple `container` 1.1.0 或更高版本运行完整的 Sub2API、MariaDB 和 Redis：
